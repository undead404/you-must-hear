---
category: genre
date: 2025-12-05T05:17:45.952Z
permalink: tags/thrashcore
title: thrashcore
---

## thrashcore

Last updated: <time datetime="2025-12-05T05:17:45.952Z">Fri Dec 05 2025 07:17:45 GMT+0200 (Eastern European Standard Time)</time>

### Description

Thrashcore fuses the breakneck speed of hardcore punk with the aggressive riffing of thrash metal, creating an intense, chaotic sound built on extremely short songs and relentless energy. Emerging from the early 1980s underground, it strips both genres down to their rawest elements, favoring visceral impact over technical complexity. The style embodies pure sonic aggression and DIY ethos, often clocking in at under a minute per track.

### 100 chronological releases

<table>
  <thead>
    <tr>
      <th>Place</th>
      <th>Cover</th>
      <th>Date</th>
      <th>Artist</th>
      <th>Album</th>
      <th>Tags</th>
      <th>Other places</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>2</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6b7d76f9eed64b299efd4b559454067b.png" alt="Cover for Dirty Rotten EP" style="max-width:120px; height:auto;" />
      </td>
      <td>1983-03</td>
      <td>D.R.I.</td>
      <td>Dirty Rotten EP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>3</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fae46175584344a896caf44cee1f836b.png" alt="Cover for Drop Dead" style="max-width:120px; height:auto;" />
      </td>
      <td>1984</td>
      <td>Siege</td>
      <td>Drop Dead</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
            <li style="font-size: calc(1rem * 52 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a> # 17</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>27</td>
      <td>
          <img src="https://i.discogs.com/eJ7Uq-uqsyRJwuHz10s6uB4qOLHzXJhLJs8jTkA3lcQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIzODEy/NjItMTU2NzU0Njg2/NC0yMzI3LmpwZWc.jpeg" alt="Cover for Violent Pacification" style="max-width:120px; height:auto;" />
      </td>
      <td>1984</td>
      <td>Dirty rotten imbeciles</td>
      <td>Violent Pacification</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>23</td>
      <td>
          <img src="https://i.discogs.com/8MQSrBFp4uKuQsbQNhMQC5_3TS80fitH6K00dEPnaVE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzNjkw/NTU3LTE1NTkwOTQ2/NDUtNjYwNS5qcGVn.jpeg" alt="Cover for Dealing With It" style="max-width:120px; height:auto;" />
      </td>
      <td>1985-03</td>
      <td>D.R.I.</td>
      <td>Dealing With It</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>99</td>
      <td>
          <img src="https://i.discogs.com/cl38xf0qIc99S3y5kGBo_zu_J9vRalmBxhzqxYYN7Mw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTk5Njcz/MjMtMTQ4OTM3MTM1/Ni02NDQyLmpwZWc.jpeg" alt="Cover for Violent Death" style="max-width:120px; height:auto;" />
      </td>
      <td>1985-10-21</td>
      <td>Genocide</td>
      <td>Violent Death</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>81</td>
      <td>
          <img src="https://i.discogs.com/_QdmGuKDOciZpPrsS4l-P8-KrNqndmUjUIxoarEpUEc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI1NDQ0/MTItMTI4OTczNDEy/MC5qcGVn.jpeg" alt="Cover for Killing Babies Is Tight" style="max-width:120px; height:auto;" />
      </td>
      <td>1986</td>
      <td>Electro Hippies</td>
      <td>Killing Babies Is Tight</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>92</td>
      <td>
          <img src="https://i.discogs.com/CAeCw-ngaIpSi2gFgnEEVh0xOGV6END3f19fTNI3tcg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE0ODI4/Njk4LTE1ODIzOTA1/NDgtNjY2MS5qcGVn.jpeg" alt="Cover for From Enslavement to Obliteration (Demo)" style="max-width:120px; height:auto;" />
      </td>
      <td>1986</td>
      <td>Napalm Death</td>
      <td>From Enslavement to Obliteration (Demo)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>60</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f187982a92eee30a6afe2b6654713d34.png" alt="Cover for Convicted" style="max-width:120px; height:auto;" />
      </td>
      <td>1986-07-26</td>
      <td>Cryptic Slaughter</td>
      <td>Convicted</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a> # 16</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>48</td>
      <td>
          <img src="https://i.discogs.com/4HM6mfJrg48WyiGpTKLgNAVx8jR42gk3hxaNsCzUVGw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE3MDM1/MDkyLTE2MTEyNTI1/MzMtMzE2OS5qcGVn.jpeg" alt="Cover for 9 Shocks Terror" style="max-width:120px; height:auto;" />
      </td>
      <td>1987</td>
      <td>Lip Cream</td>
      <td>9 Shocks Terror</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>66</td>
      <td>
          <img src="https://i.discogs.com/qphm8V64fR1uhQDzufr5VWlnidBP7Qgfu9jwc2yYWMw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIyNzI0/MDYtMTQ3NjE3OTQ2/OC0yMDc1LmpwZWc.jpeg" alt="Cover for Olho Por Olho" style="max-width:120px; height:auto;" />
      </td>
      <td>1989</td>
      <td>Olho Seco</td>
      <td>Olho Por Olho</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 15</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 15</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>68</td>
      <td>
          <img src="https://i.discogs.com/7--SWIItRW0cz4KnwHDG8mzyXXhHMtn9GH2lROu5czc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQ1MTc2/LTE0ODUzMjczODQt/MTYyMi5qcGVn.jpeg" alt="Cover for Thrash Night" style="max-width:120px; height:auto;" />
      </td>
      <td>1989</td>
      <td>S.O.B.</td>
      <td>Thrash Night</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>84</td>
      <td>
          <img src="https://i.discogs.com/4rHj-DRgmIvgooDuI0A7hOK_GiS5jPOxyqdheUxbRzo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTk0Mjcx/NS0xNjY1MjE5MDMz/LTkwNDUuanBlZw.jpeg" alt="Cover for Slang" style="max-width:120px; height:auto;" />
      </td>
      <td>1989</td>
      <td>Slang</td>
      <td>Slang</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>90</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4ea9753b677c6753a16ad020abdcc311.png" alt="Cover for Slipknot EP" style="max-width:120px; height:auto;" />
      </td>
      <td>1989</td>
      <td>Slipknot</td>
      <td>Slipknot EP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>19</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4ed1c58f551ac7d8cde9ad50c2c7cbb0.png" alt="Cover for 限界は何処だ" style="max-width:120px; height:auto;" />
      </td>
      <td>1990-10-05</td>
      <td>Gauze</td>
      <td>限界は何処だ</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/crust" | relative_url }}">crust</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/punk-rock" | relative_url }}">punk rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/crust" | relative_url }}">crust</a> # 83</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>77</td>
      <td>
          <img src="https://i.discogs.com/TReLvFiK_7mZwDUWsBDnAj6guJiCU6nj8MC8T-iv21k/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTUyMzAw/OS0xMTI3MjkxNzg1/LmpwZWc.jpeg" alt="Cover for Control" style="max-width:120px; height:auto;" />
      </td>
      <td>1994-08</td>
      <td>Hellnation</td>
      <td>Control</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>21</td>
      <td>
          <img src="https://i.discogs.com/InE0c-zKlkDhWETp3aA1s3Uw_wONTnMUHoDJx6XQfPY/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTU4MDQ4/MTctMTQwNDg5MjEy/OS01NjkyLmpwZWc.jpeg" alt="Cover for Shot By Lammi" style="max-width:120px; height:auto;" />
      </td>
      <td>1997</td>
      <td>Big D And The Kids Table</td>
      <td>Shot By Lammi</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/american" | relative_url }}">american</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dub" | relative_url }}">dub</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/modern-rock" | relative_url }}">modern rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ska" | relative_url }}">ska</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/modern-rock" | relative_url }}">modern rock</a> # 45</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>16</td>
      <td>
          <img src="https://i.discogs.com/3cUHAzb4JSxHlGJcA2UbJSE_7Panm8EAD5WIauBiv3Q/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1ODk3/NTAtMTIzODE1Njk1/MC5qcGVn.jpeg" alt="Cover for Deep Wound" style="max-width:120px; height:auto;" />
      </td>
      <td>1997</td>
      <td>Deep Wound</td>
      <td>Deep Wound</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>57</td>
      <td>
      </td>
      <td>1997</td>
      <td>Lärm</td>
      <td>Extreme Noise</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>96</td>
      <td>
          <img src="https://i.discogs.com/S_LYWfEWpuFkzqx3FjTEx3Y6NYrxsw2Xo259yZOmLsY/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI1Mzg0/OTEyLTE2NzAzMzI0/MTAtMTU1NC5qcGVn.jpeg" alt="Cover for These Wheels Roll For Victory!" style="max-width:120px; height:auto;" />
      </td>
      <td>1997</td>
      <td>Razors Edge</td>
      <td>These Wheels Roll For Victory!</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>52</td>
      <td>
      </td>
      <td>1998-01</td>
      <td>Hellnation</td>
      <td>Your Chaos Days Are Numbered</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>15</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/323c312e0e60aaf40d8f207c97a0c27f.png" alt="Cover for Chef Aid: The South Park Album" style="max-width:120px; height:auto;" />
      </td>
      <td>1998-11-24</td>
      <td>Various Artists</td>
      <td>Chef Aid: The South Park Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/comedy" | relative_url }}">comedy</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/comedy-rock" | relative_url }}">comedy rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/heavy-metal" | relative_url }}">heavy metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/nu-metal" | relative_url }}">nu metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrash-metal" | relative_url }}">thrash metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/comedy-rock" | relative_url }}">comedy rock</a> # 32</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>67</td>
      <td>
          <img src="https://i.discogs.com/9Z5GNGo0tZ4lFB7iLAk7qiGya3aF1BeSG3MT1qElbMA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI0NTU3/NDExLTE2NjM1MzA0/MjgtMTI5My5qcGVn.jpeg" alt="Cover for Dub Grind" style="max-width:120px; height:auto;" />
      </td>
      <td>1999</td>
      <td>S.O.B.</td>
      <td>Dub Grind</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>79</td>
      <td>
      </td>
      <td>1999</td>
      <td>Charles Bronson</td>
      <td>Complete Discocrappy</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>22</td>
      <td>
          <img src="https://i.discogs.com/HCWHVL3cIjAX8YI6Hf1iB78cR-v2YZc0gO2yRFRCsVM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTMxNjc1/ODktMTUyODg0MTky/Ni05NDc0LmpwZWc.jpeg" alt="Cover for Zen And The Art Of Beating Your Ass" style="max-width:120px; height:auto;" />
      </td>
      <td>1999-07</td>
      <td>9 Shocks Terror</td>
      <td>Zen And The Art Of Beating Your Ass</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>49</td>
      <td>
          <img src="https://i.discogs.com/yJglpQbKMDTQ0a8nAbzzdjpXHun8ZP3uwZGTCVE2OhU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTUyNDE4/OS0xMTI3NDIwNDE4/LmpwZWc.jpeg" alt="Cover for Fucked Up Mess" style="max-width:120px; height:auto;" />
      </td>
      <td>1999-07</td>
      <td>Hellnation</td>
      <td>Fucked Up Mess</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>63</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f48215db030148b08abafd23bb7d9c78.png" alt="Cover for Taken By Force" style="max-width:120px; height:auto;" />
      </td>
      <td>1999-10-01</td>
      <td>Proclamation</td>
      <td>Taken By Force</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>59</td>
      <td>
      </td>
      <td>2000</td>
      <td>屍</td>
      <td>自我と煩悩</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>83</td>
      <td>
      </td>
      <td>2001</td>
      <td>Spazz</td>
      <td>Sweatin&#39; Ii: Deported Live Dwarf</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>98</td>
      <td>
      </td>
      <td>2001-03-07</td>
      <td>Crucial Unit</td>
      <td>Premium Iced Tea EP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>9</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f8a4b409c044498dabdaeef2fb273e6a.png" alt="Cover for Dying For It" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-09-01</td>
      <td>Panic</td>
      <td>Dying For It</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>51</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d6afd244bfef33a8664ac9d1f628e544.png" alt="Cover for Happy Night Eletric Experience" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>Discarga</td>
      <td>Happy Night Eletric Experience</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>65</td>
      <td>
          <img src="https://i.discogs.com/uZoy4_6fwDDLlIlsyOChbCmVNU15OnNv3DNV9NcGjKU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTY1NTAx/OTUtMTYzMTU2OTIw/NC00MTc1LmpwZWc.jpeg" alt="Cover for Outsiders" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>Common Enemy</td>
      <td>Outsiders</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>80</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/662b3f4497944dbe8f0bedeb333f5890.png" alt="Cover for Moshzilla" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>Crucial Unit</td>
      <td>Moshzilla</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>32</td>
      <td>
          <img src="https://i.discogs.com/kuCgbJ1A1Vpo8HQZjBH6FG5emQFAdZqCWYeK_p6zl5c/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTMxNTU2/ODAtMTMxODMxMjgz/OS5qcGVn.jpeg" alt="Cover for Good Bye To Your Punk Rock" style="max-width:120px; height:auto;" />
      </td>
      <td>2002-10-02</td>
      <td>BBQ CHICKENS</td>
      <td>Good Bye To Your Punk Rock</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>35</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6844228736ee47fd33cae690582eaa60.png" alt="Cover for Tiranus" style="max-width:120px; height:auto;" />
      </td>
      <td>2003</td>
      <td>I Shot Cyrus</td>
      <td>Tiranus</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 7</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 7</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>29</td>
      <td>
          <img src="https://i.discogs.com/KbjyWK-RxH4GMavSQ6FjUZinrMA0QLAqxqmKXvyUgTw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI1MzM0/MDAtMTI4OTE3MzMw/Ny5qcGVn.jpeg" alt="Cover for Anti Everything EP" style="max-width:120px; height:auto;" />
      </td>
      <td>2003</td>
      <td>Surf Nazis Must Die</td>
      <td>Anti Everything EP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>61</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2138bc683e6a888a87db12b8dd3e58d5.png" alt="Cover for Waste &#39;em All" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-01-27</td>
      <td>Municipal Waste</td>
      <td>Waste &#39;em All</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a> # 88</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>11</td>
      <td>
          <img src="https://i.discogs.com/t6JXOax_VE-Hb9DuTxnTihk8fcBBza-L2JypedFPuGA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTM2OTA0/OTgtMTM0MDUyMjQ2/MS0zNDM4LmpwZWc.jpeg" alt="Cover for I Hate Myself When I&#39;m Not Skateboarding" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-06-22</td>
      <td>Bones Brigade</td>
      <td>I Hate Myself When I&#39;m Not Skateboarding</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>40</td>
      <td>
          <img src="https://i.discogs.com/pDahIZk76WomybJroM_r1-8pKZn09l2XmttBs3r5Txg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE5MTMz/NjItMTI1MjA2NjU2/NC5qcGVn.jpeg" alt="Cover for These Colors Get The Runs" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-06-23</td>
      <td>Crucial Unit</td>
      <td>These Colors Get The Runs</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>28</td>
      <td>
          <img src="https://i.discogs.com/wjWhEOAZ59Ak8PMMR6zEhvvNPysarFS-kaLUr9HGn78/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTMxNTU3/MDctMTUwOTcxNDEy/OC0yNjM3LmpwZWc.jpeg" alt="Cover for Fine Songs Playing Sucks" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-10-22</td>
      <td>BBQ CHICKENS</td>
      <td>Fine Songs Playing Sucks</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
            <li style="font-size: calc(1rem * 90 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 41</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 41</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>10</td>
      <td>
          <img src="https://i.discogs.com/Z8dsPbqh-58y1FJ3Kc-T5Jj2Q37FEpXygW1WzJAxBSQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI5NzY5/MjItMTM1OTMyNTg5/Mi0zODQxLmpwZWc.jpeg" alt="Cover for Excruciation" style="max-width:120px; height:auto;" />
      </td>
      <td>2004</td>
      <td>XFilesX</td>
      <td>Excruciation</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>45</td>
      <td>
      </td>
      <td>2004</td>
      <td>Scholastic Deth</td>
      <td>Final Examiner</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>18</td>
      <td>
          <img src="https://i.discogs.com/tYAlhdQuzzAcKKeeXNO8O4jZszrdSAz3u9YhuRPlviI/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE2NTE5/MjctMTI0MzE3NDA4/MC5qcGVn.jpeg" alt="Cover for Bring It" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>R.A.M.B.O.</td>
      <td>Bring It</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 86 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 79</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 79</li>
            <li> <a href="{{ "/tags/straight-edge" | relative_url }}">straight edge</a> # 92</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>58</td>
      <td>
          <img src="https://i.discogs.com/oNjx42YdUZU94DP8n1eDjIu48jx_r-VhPZ9jDAkGsMY/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE3MDYy/MTctMTMxODMwMTIz/Ni5qcGVn.jpeg" alt="Cover for Assassinicus" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>I Shot Cyrus</td>
      <td>Assassinicus</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 13</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 13</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>72</td>
      <td>
          <img src="https://i.discogs.com/j7DZAa326rPom4C-zlHDwSxGrW0rY-bheswOhgwLMNM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzNzMw/MDgtMTIxMzg5ODYy/Ni5qcGVn.jpeg" alt="Cover for I Shot Cyrus &amp; Discarga - Split LP" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>Discarga</td>
      <td>I Shot Cyrus &amp; Discarga - Split LP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
            <li style="font-size: calc(1rem * 88 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 36</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 36</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>14</td>
      <td>
      </td>
      <td>2005</td>
      <td>Bleach 03</td>
      <td>Bleach 03</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/japanese" | relative_url }}">japanese</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>31</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/da5b54ecdc3247248c3ae53c530f8acb.png" alt="Cover for Down Low" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-05-19</td>
      <td>Betzefer</td>
      <td>Down Low</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrash-metal" | relative_url }}">thrash metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>13</td>
      <td>
          <img src="https://i.discogs.com/WoINairICHbOjMAq58L4z0JCQZUSlQUmGxQ5TfxsCZQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI0NjUw/NzMtMTY3NTUwNzMw/MC0yOTg5LmpwZWc.jpeg" alt="Cover for Ruined" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-12-20</td>
      <td>Ceremony</td>
      <td>Ruined</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 4</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 4</li>
            <li> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a> # 49</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>20</td>
      <td>
          <img src="https://i.discogs.com/bnuDN_eLJ3ZeVMangKGuT-hHfFDFFOH0_GnF3m1QuUU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1NjM3/OTItMTM4MzI2NTk0/NC0zNzU3LmpwZWc.jpeg" alt="Cover for Almost Complete" style="max-width:120px; height:auto;" />
      </td>
      <td>2006</td>
      <td>Deep Wound</td>
      <td>Almost Complete</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>26</td>
      <td>
          <img src="https://i.discogs.com/8SCapuMH47gi_BsjXake86Srbq1lRHsru2a_fdRuDsg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTY4ODg3/NjktMTYxNjAwNzMx/MC0yMzQ0LmpwZWc.jpeg" alt="Cover for Appleshout" style="max-width:120px; height:auto;" />
      </td>
      <td>2006</td>
      <td>Appleshout</td>
      <td>Appleshout</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>34</td>
      <td>
          <img src="https://i.discogs.com/2KD1QsHRuqCObP_D9m7NHT-IaKhsuAb6hthYKdsw3TY/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1OTM1/NDItMTYxNjAxOTEy/Mi02MzQ2LmpwZWc.jpeg" alt="Cover for T.U.I." style="max-width:120px; height:auto;" />
      </td>
      <td>2006</td>
      <td>Common Enemy</td>
      <td>T.U.I.</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>100</td>
      <td>
          <img src="https://i.discogs.com/gGgLQjq4Eprl432uTXe15bp_17U1eIxIfcMHrhC5xAY/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzMDQ2/MzcwLTE1NDcwODAx/OTUtNjg2Ny5qcGVn.jpeg" alt="Cover for Estude e Morra!" style="max-width:120px; height:auto;" />
      </td>
      <td>2006</td>
      <td>Morto Pela Escola</td>
      <td>Estude e Morra!</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>24</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d88658b8f87f4e6bbc961542401879fe.png" alt="Cover for Kingdom" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-05-19</td>
      <td>Cataract</td>
      <td>Kingdom</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>25</td>
      <td>
          <img src="https://i.discogs.com/p83n12sXuHpLsO9wAmaOxVYmWrSktqGCvHBZzJXYkSw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTc3NDU0/MC0xNTQwODUyOTI4/LTczNDIuanBlZw.jpeg" alt="Cover for First Lesson" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-08-15</td>
      <td>LxExAxRxNx</td>
      <td>First Lesson</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>70</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/56e2ec84e5384ed698bdc682ef6cf22b.png" alt="Cover for Отторжение" style="max-width:120px; height:auto;" />
      </td>
      <td>2007</td>
      <td>Paranoid</td>
      <td>Отторжение</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-metal" | relative_url }}">alternative metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/russian-alternative" | relative_url }}">russian alternative</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/russian-alternative" | relative_url }}">russian alternative</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>69</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d2b816fdbd3c83da497d0f816279a80d.png" alt="Cover for 26 Beers" style="max-width:120px; height:auto;" />
      </td>
      <td>2007</td>
      <td>26 Beers</td>
      <td>26 Beers</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrash-metal" | relative_url }}">thrash metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>78</td>
      <td>
          <img src="https://i.discogs.com/EtfOvxAxAWQ5YugFZ0lX47EFJBQd3_K2bHUxkeJ_6wI/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI5NTU1/NDQtMTMwOTAwNTI5/My5qcGVn.jpeg" alt="Cover for l&#39;inferno dell&#39;uguaglianza" style="max-width:120px; height:auto;" />
      </td>
      <td>2007</td>
      <td>Milizia HC</td>
      <td>l&#39;inferno dell&#39;uguaglianza</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/italian" | relative_url }}">italian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>82</td>
      <td>
          <img src="https://i.discogs.com/l7d361n4m-uhh3foPkhnOo1A_p_1DW0lP3OTNJZMIbw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1MzYz/OTMtMTIyNjc5NjQz/OS5qcGVn.jpeg" alt="Cover for ...If There Is a Future" style="max-width:120px; height:auto;" />
      </td>
      <td>2007</td>
      <td>Korova</td>
      <td>...If There Is a Future</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>4</td>
      <td>
          <img src="https://i.discogs.com/gwJ36lXM_JRxwqjBMJdIYznvC2T5T3eXKjjnBxViYTk/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTM1ODQ2/NzgtMTM1OTQ3NDM3/NS01Mjk1LmpwZWc.jpeg" alt="Cover for Walking Disease" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-10-09</td>
      <td>Trash Talk</td>
      <td>Walking Disease</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 1</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 1</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>64</td>
      <td>
          <img src="https://i.discogs.com/8M27UyLj6PGt2zS3emvawplzOZm8XoL1e1Li_5SaLlo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTM2NDA4/OTUtMTMzODQ5MDI1/My0yODc1LmpwZWc.jpeg" alt="Cover for Deus Ex Machina" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-12</td>
      <td>The Moebius Curve</td>
      <td>Deus Ex Machina</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/barcelona" | relative_url }}">barcelona</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/barcelona" | relative_url }}">barcelona</a> # 17</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>7</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8523cd9e9f232befed07a0bf82ba6a4b.png" alt="Cover for Plagues" style="max-width:120px; height:auto;" />
      </td>
      <td>2008</td>
      <td>Trash Talk</td>
      <td>Plagues</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a> # 77</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>30</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/61a9a7c6cb5044428f60b7f646396206.png" alt="Cover for Demo" style="max-width:120px; height:auto;" />
      </td>
      <td>2008</td>
      <td>Rearranged</td>
      <td>Demo</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-hardcore" | relative_url }}">melodic hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-punk" | relative_url }}">melodic punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/moscow" | relative_url }}">moscow</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/old-school-hardcore" | relative_url }}">old school hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/russia" | relative_url }}">russia</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/russian" | relative_url }}">russian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/old-school-hardcore" | relative_url }}">old school hardcore</a> # 16</li>
            <li> <a href="{{ "/tags/moscow" | relative_url }}">moscow</a> # 18</li>
            <li> <a href="{{ "/tags/russia" | relative_url }}">russia</a> # 33</li>
            <li> <a href="{{ "/tags/melodic-punk" | relative_url }}">melodic punk</a> # 34</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>53</td>
      <td>
          <img src="https://i.discogs.com/4S1i4DN1ZjITQJ3QGJiWHBHiy6b1uE3L7ORjm9teMAo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQ4OTA0/OTAtMTM3ODYwOTgx/NS0xMzcwLmpwZWc.jpeg" alt="Cover for Thrashing Goes Lovely" style="max-width:120px; height:auto;" />
      </td>
      <td>2008</td>
      <td>Razors Edge</td>
      <td>Thrashing Goes Lovely</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/japanese" | relative_url }}">japanese</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-hardcore" | relative_url }}">melodic hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 12</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 12</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>17</td>
      <td>
      </td>
      <td>2008</td>
      <td>Vitamin X</td>
      <td>Full Scale Assault</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>33</td>
      <td>
      </td>
      <td>2008</td>
      <td>Trash Talk</td>
      <td>Plagues...Walking Disease</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>41</td>
      <td>
          <img src="https://i.discogs.com/zIGGvQcdwZlX4A5nJDYEUwNB1-z5gqVjcc35CoyNyrk/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTM0MTE5/MTYtMTMzMjc4NjMw/OC5qcGVn.jpeg" alt="Cover for 14 Rockets From W4X" style="max-width:120px; height:auto;" />
      </td>
      <td>2008</td>
      <td>Wheel4x</td>
      <td>14 Rockets From W4X</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/russian" | relative_url }}">russian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>47</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a738b648ad514a65b4950e650de4878a.png" alt="Cover for Eyeless E.P." style="max-width:120px; height:auto;" />
      </td>
      <td>2008</td>
      <td>Punch</td>
      <td>Eyeless E.P.</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>74</td>
      <td>
      </td>
      <td>2008</td>
      <td>Monsieur Po</td>
      <td>There was life...</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/deathcore" | relative_url }}">deathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/france" | relative_url }}">france</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/paris" | relative_url }}">paris</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>75</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/11e73ffc56004f7a9532014e2960bba3.png" alt="Cover for Начало Пути" style="max-width:120px; height:auto;" />
      </td>
      <td>2008</td>
      <td>Один Из Тысячи</td>
      <td>Начало Пути</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>42</td>
      <td>
          <img src="https://i.discogs.com/tEuV2V_Nn0fsdtrHU_i2avpTjiqHmZGJFUjJm0JRU0o/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI4NDk3/OTAtMTQ2NDk5MDM2/NS03MzI1LmpwZWc.jpeg" alt="Cover for Headbanger&#39;s Karaoke Club Dangerous Fire" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-02-19</td>
      <td>Hayaino Daisuki</td>
      <td>Headbanger&#39;s Karaoke Club Dangerous Fire</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/speed-metal" | relative_url }}">speed metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>87</td>
      <td>
          <img src="https://i.discogs.com/xM27ORvlQ3JrsoFQKshvQp6yaxnninmJlt9FnMUDthQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTc5Mjcx/NTEtMTQ1MTgyMDM3/Mi04NTA5LmpwZWc.jpeg" alt="Cover for Struggling with the Dark" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-03-29</td>
      <td>Nuestros Derechos</td>
      <td>Struggling with the Dark</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dutch" | relative_url }}">dutch</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/nederlandstalig" | relative_url }}">nederlandstalig</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrash-metal" | relative_url }}">thrash metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>76</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3a4be40ac3f04a5e93a859ccf7628950.png" alt="Cover for Abstract" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-10-01</td>
      <td>Cü Sujo</td>
      <td>Abstract</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/brazil" | relative_url }}">brazil</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>12</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/78dde13258de423e8677aeeaf963eae7.png" alt="Cover for Lines in the Sand" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Coke Bust</td>
      <td>Lines in the Sand</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>54</td>
      <td>
          <img src="https://i.discogs.com/KoclcUrTIQqDmrIjlQ8VcFslYgcDFWP53UK9PqnQn34/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI2NDE4/NjAtMTI5NDU5MTQ5/Ni5naWY.jpeg" alt="Cover for Living the Dream?" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Common Enemy</td>
      <td>Living the Dream?</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>86</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/eba47e59d93c40189319e91de0f055e6.png" alt="Cover for Naifa e Morto Pela Escola" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Morto Pela Escola</td>
      <td>Naifa e Morto Pela Escola</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/brazil" | relative_url }}">brazil</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>56</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/16a12e0128c2495ea3a779cb5f0aa683.png" alt="Cover for This Means War" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-01-20</td>
      <td>Blackened</td>
      <td>This Means War</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrash-metal" | relative_url }}">thrash metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>5</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b0599c403b494b8ccb0e5b84cdc76c67.png" alt="Cover for Punch" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-03-01</td>
      <td>Punch</td>
      <td>Punch</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a> # 19</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>71</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/be138acbfe8446409de7376f51b99517.png" alt="Cover for Agorapocalypse" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-04-14</td>
      <td>Agoraphobic Nosebleed</td>
      <td>Agorapocalypse</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a> # 44</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>73</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/cc65bb201fe93e9dfd494af874065e1a.png" alt="Cover for Demo" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-05-28</td>
      <td>Kali</td>
      <td>Demo</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 17</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 17</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>43</td>
      <td>
          <img src="https://i.discogs.com/M3jUoGT51_6dXHTdSsfZpPHehGBmpbk0roKdt2hjbZs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE4NTc3/NTItMTYyMzc3MDA3/MC03NTM4LmpwZWc.jpeg" alt="Cover for Show Us The Meaning Of Haste" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-07-21</td>
      <td>Hummingbird of Death</td>
      <td>Show Us The Meaning Of Haste</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 11</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 11</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>85</td>
      <td>
          <img src="https://i.discogs.com/Ct1qy6WkK7QUmijVIgQVnGjMW3N5G42kb8ENF1y0-54/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE4OTQ1/NTQtMTI1MDcwNjMy/MC5naWY.jpeg" alt="Cover for NUR FLOPS" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-08-06</td>
      <td>Napoleon Dynamite</td>
      <td>NUR FLOPS</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german-punk" | relative_url }}">german punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>93</td>
      <td>
          <img src="https://i.discogs.com/70q2Pym_WmjWctu7hNAF628maOE83AeBnTttPE14dUY/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQwOTgy/MTctMTM2MDA1Mjg2/My0yNTY1LmpwZWc.jpeg" alt="Cover for Thrash the Railroads" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-10</td>
      <td>Boiling Point</td>
      <td>Thrash the Railroads</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 29</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 29</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>88</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a7b4886b05f681bc530e78bf20f16a3b.png" alt="Cover for The Powerless Rise" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-05-11</td>
      <td>As I Lay Dying</td>
      <td>The Powerless Rise</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/christian-metalcore" | relative_url }}">christian metalcore</a> # 22</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>97</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/24816a10d1aa4bd99cf8bc352a36a0a4.png" alt="Cover for Memories of War" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-11-02</td>
      <td>Red Front</td>
      <td>Memories of War</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/death-metal" | relative_url }}">death metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/groove-metal" | relative_url }}">groove metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrash-metal" | relative_url }}">thrash metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>36</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/87d2fc945b1a85d70529b4a693653240.png" alt="Cover for Deprogrammed" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-12-25</td>
      <td>XBRAINIAX</td>
      <td>Deprogrammed</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 8</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 8</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>55</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8e904d4795d942b5900536f921311ea1.png" alt="Cover for Отъебись!" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-02-15</td>
      <td>Minefield</td>
      <td>Отъебись!</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>37</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0eca348c3846463b8e3c69e732b358b9.png" alt="Cover for The Fatal Feast" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-04-10</td>
      <td>Municipal Waste</td>
      <td>The Fatal Feast</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/thrash-metal" | relative_url }}">thrash metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a> # 95</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>62</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d34c86b6ee591c71e5a438987bab9dee.png" alt="Cover for Product Of Waste" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-08</td>
      <td>Product of Waste</td>
      <td>Product Of Waste</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>91</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f26e69bf93d4483fc8afe01ba1dd9938.png" alt="Cover for THROW UP A V THAT STANDS FOR VENICE" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-11-16</td>
      <td>Crippled Fox</td>
      <td>THROW UP A V THAT STANDS FOR VENICE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 28</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 28</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>6</td>
      <td>
          <img src="https://i.discogs.com/X7yQjaQRbcPJcnDKUMfIC7P43MvYwo_fKSG17z92OGI/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQ3NDg2/NDctMTM4NTkyOTE1/OC0xOTk3LmpwZWc.jpeg" alt="Cover for Confined" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-07-04</td>
      <td>Coke Bust</td>
      <td>Confined</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>1</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d2486ed85bb6427cc2c22f3a0bbaff53.png" alt="Cover for それは僕たちの奇跡" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-04-23</td>
      <td>μ&#39;s</td>
      <td>それは僕たちの奇跡</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/asian" | relative_url }}">asian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/asian-pop" | relative_url }}">asian pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/idol" | relative_url }}">idol</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/j-pop" | relative_url }}">j-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/japanese" | relative_url }}">japanese</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/japanese-pop" | relative_url }}">japanese pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/soundtrack" | relative_url }}">soundtrack</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrash-metal" | relative_url }}">thrash metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/idol" | relative_url }}">idol</a> # 5</li>
            <li> <a href="{{ "/tags/japanese-pop" | relative_url }}">japanese pop</a> # 6</li>
            <li> <a href="{{ "/tags/asian" | relative_url }}">asian</a> # 62</li>
            <li> <a href="{{ "/tags/asian-pop" | relative_url }}">asian pop</a> # 64</li>
            <li> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a> # 80</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>8</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9c6aa7195bafa30301f68d88a44c6b34.png" alt="Cover for They Don&#39;t Have To Believe" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-08-19</td>
      <td>Punch</td>
      <td>They Don&#39;t Have To Believe</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a> # 36</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>39</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f214117d813375ae05467dc2f81c37c5.png" alt="Cover for Insanity Alert" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-10-17</td>
      <td>Insanity Alert</td>
      <td>Insanity Alert</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrash-metal" | relative_url }}">thrash metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a> # 3</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>38</td>
      <td>
          <img src="https://i.discogs.com/AA8y6Yt63SiK2MCPQa_JE1G2b1ZQlMiaIWI5b6mbBbk/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTExMjI2/MTUzLTE1MTIyNTA4/MjMtNzU5NS5wbmc.jpeg" alt="Cover for Forbidden Techniques" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-12-01</td>
      <td>Hummingbird of Death</td>
      <td>Forbidden Techniques</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 10</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 10</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>50</td>
      <td>
          <img src="https://i.discogs.com/1TyYmbcbo1oNFvcdrdhI8LkldFbNiGrlkq0p4I1plgU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE0NTA5/MTU5LTE1NzU5OTA0/NjAtMTIwMC5qcGVn.jpeg" alt="Cover for Sociedades Secretas" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-04-11</td>
      <td>Cérebro de Galinha</td>
      <td>Sociedades Secretas</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/crossover-thrash" | relative_url }}">crossover thrash</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>89</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/df92f00b9a6717e4bfb81b714c5bc4d6.png" alt="Cover for Unrest Eternal" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-06-28</td>
      <td>Peasant</td>
      <td>Unrest Eternal</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metallic-hardcore" | relative_url }}">metallic hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>44</td>
      <td>
          <img src="https://i.discogs.com/MgFUCJhDQwi8VWEU8NEAvWNQgXglzjL7wRsHLwRRRGM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE5MzQy/MzgxLTE2MjUxNTYx/NTUtNDQ1MS5qcGVn.jpeg" alt="Cover for Bastard" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-03-09</td>
      <td>Bastard</td>
      <td>Bastard</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>46</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0533401b8fb370c0ec6b148c79689ebc.png" alt="Cover for Bad Songs Forever" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-05-06</td>
      <td>Deaf Club</td>
      <td>Bad Songs Forever</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/crust" | relative_url }}">crust</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-punk" | relative_url }}">noise punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noisecore" | relative_url }}">noisecore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/noise-punk" | relative_url }}">noise punk</a> # 26</li>
            <li> <a href="{{ "/tags/noisecore" | relative_url }}">noisecore</a> # 62</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>94</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/04b1d715c099f971b7d48305a7c40597.png" alt="Cover for Set To Fail" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-03-25</td>
      <td>Negative X Influence</td>
      <td>Set To Fail</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>95</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/66a14fd7a9ffba038e7b63b77304ce55.png" alt="Cover for Радісно й класно" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-04-17</td>
      <td>Transgender</td>
      <td>Радісно й класно</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrashcore" | relative_url }}">thrashcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ukraine" | relative_url }}">ukraine</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/ukrainian" | relative_url }}">ukrainian</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fastcore" | relative_url }}">fastcore</a> # 43</li>
            <li> <a href="{{ "/tags/billie-holiday-billie-holiday" | relative_url }}">billie holiday-billie holiday</a> # 43</li>
        </ul>
      </td>
    </tr>
  </tbody>
</table>

### Related genres

- [hardcore punk]({{ "/tags/hardcore-punk" | relative_url }})
- [hardcore]({{ "/tags/hardcore" | relative_url }})
- [fastcore]({{ "/tags/fastcore" | relative_url }})
- [powerviolence]({{ "/tags/powerviolence" | relative_url }})
- [thrash metal]({{ "/tags/thrash-metal" | relative_url }})
- [grindcore]({{ "/tags/grindcore" | relative_url }})
- [metal]({{ "/tags/metal" | relative_url }})
- [crossover thrash]({{ "/tags/crossover-thrash" | relative_url }})
- [metalcore]({{ "/tags/metalcore" | relative_url }})
- [crust]({{ "/tags/crust" | relative_url }})
