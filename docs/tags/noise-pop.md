---
category: genre
date: 2025-12-05T17:42:45.933Z
permalink: tags/noise-pop
title: noise pop
---

## noise pop

Last updated: <time datetime="2025-12-05T17:42:45.933Z">Fri Dec 05 2025 19:42:45 GMT+0200 (Eastern European Standard Time)</time>

### Description

Noise pop is a subgenre of alternative rock music that originated in the 1980s. It is characterized by the use of noisy and distorted guitar sounds, prominent use of feedback, and catchy pop melodies. The music is often described as a combination of the noise and chaos of punk rock with the melodic sensibility of pop music. Some of the key features of noise pop include the use of fuzz and distortion pedals, the incorporation of feedback and dissonant sounds, and the use of unconventional song structures. Some well-known noise pop bands include Sonic Youth, My Bloody Valentine, and Dinosaur Jr.

### 100 chronological releases

<table>
  <thead>
    <tr>
      <th>Place</th>
      <th>Cover</th>
      <th>Date</th>
      <th>Artist</th>
      <th>Album</th>
      <th>Tags</th>
      <th>Other places</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>10</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3980ed7953d380472acb2e704bae4875.png" alt="Cover for Psychocandy" style="max-width:120px; height:auto;" />
      </td>
      <td>1985-02</td>
      <td>The Jesus and Mary Chain</td>
      <td>Psychocandy</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-punk" | relative_url }}">post-punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
            <li style="font-size: calc(1rem * 73 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 21</li>
            <li> <a href="{{ "/tags/post-punk" | relative_url }}">post-punk</a> # 21</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>54</td>
      <td>
      </td>
      <td>1987-02</td>
      <td>My Bloody Valentine</td>
      <td>Sunny Sundae Smile</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>62</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bbd8d87f18bcd915b43fe386aa1cb36b.png" alt="Cover for Darklands" style="max-width:120px; height:auto;" />
      </td>
      <td>1987-10</td>
      <td>The Jesus and Mary Chain</td>
      <td>Darklands</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-punk" | relative_url }}">post-punk</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/post-punk" | relative_url }}">post-punk</a> # 55</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>95</td>
      <td>
      </td>
      <td>1988-11-21</td>
      <td>My Bloody Valentine</td>
      <td>Isn&#39;t Anything</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 17</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>98</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/912177a1df0a73f5038097f86c696433.png" alt="Cover for The Scene Between" style="max-width:120px; height:auto;" />
      </td>
      <td>1989-01</td>
      <td>The Go! Team</td>
      <td>The Scene Between</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-pop" | relative_url }}">indie pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>65</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6a414f453271424485021e7c96e9b743.png" alt="Cover for Blonder Tongue Audio Baton" style="max-width:120px; height:auto;" />
      </td>
      <td>1992</td>
      <td>Swirlies</td>
      <td>Blonder Tongue Audio Baton</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 66</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>83</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3eaba50040e1120184b12780ce96631a.png" alt="Cover for Peng!" style="max-width:120px; height:auto;" />
      </td>
      <td>1992-05-26</td>
      <td>Stereolab</td>
      <td>Peng!</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/space-rock-revival" | relative_url }}">space rock revival</a> # 55</li>
            <li> <a href="{{ "/tags/drone-rock" | relative_url }}">drone rock</a> # 57</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>68</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/085b6dbfa981aed2af885b4765b46bce.png" alt="Cover for Hit to Death in the Future Head" style="max-width:120px; height:auto;" />
      </td>
      <td>1992-08-11</td>
      <td>The Flaming Lips</td>
      <td>Hit to Death in the Future Head</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>60</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d123cccdb635418bc16dff07ed3cf4d3.png" alt="Cover for Transmissions From the Satellite Heart" style="max-width:120px; height:auto;" />
      </td>
      <td>1993-06-22</td>
      <td>The Flaming Lips</td>
      <td>Transmissions From the Satellite Heart</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>99</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f76da5e7f8895fa1de817316332a1db3.png" alt="Cover for Transient Random-Noise Bursts with Announcements" style="max-width:120px; height:auto;" />
      </td>
      <td>1993-08</td>
      <td>Stereolab</td>
      <td>Transient Random-Noise Bursts with Announcements</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/space-rock-revival" | relative_url }}">space rock revival</a> # 58</li>
            <li> <a href="{{ "/tags/drone-rock" | relative_url }}">drone rock</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>96</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/dbd157a8d994e9d5e9b5ed76c3db2efe.png" alt="Cover for Transient Random-Noise Bursts With Announcements (Expanded Edition)" style="max-width:120px; height:auto;" />
      </td>
      <td>1993-09-06</td>
      <td>Stereolab</td>
      <td>Transient Random-Noise Bursts With Announcements (Expanded Edition)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>86</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2c6ecc7298a011487d364a14abdfc55f.png" alt="Cover for Painful" style="max-width:120px; height:auto;" />
      </td>
      <td>1993-10-05</td>
      <td>Yo La Tengo</td>
      <td>Painful</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 38</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>39</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/84736238e5fa4ceecc13c7e8aca17c10.png" alt="Cover for Love Tara" style="max-width:120px; height:auto;" />
      </td>
      <td>1993-11</td>
      <td>Eric&#39;s Trip</td>
      <td>Love Tara</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/lo-fi-indie" | relative_url }}">lo-fi indie</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/lo-fi-indie" | relative_url }}">lo-fi indie</a> # 2</li>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 11</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>36</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3c834f1e78fd4a2280506efec80ee011.png" alt="Cover for Celebrate the New Dark Age" style="max-width:120px; height:auto;" />
      </td>
      <td>1994-05-26</td>
      <td>Polvo</td>
      <td>Celebrate the New Dark Age</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/real-post-hardcore" | relative_url }}">real post-hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/real-post-hardcore" | relative_url }}">real post-hardcore</a> # 2</li>
            <li> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a> # 45</li>
            <li> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a> # 52</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>48</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/901d12187dd04481c4193fca495a0c7e.png" alt="Cover for Definitely Maybe (Remastered) [Deluxe Edition]" style="max-width:120px; height:auto;" />
      </td>
      <td>1994-08-30</td>
      <td>Oasis</td>
      <td>Definitely Maybe (Remastered) [Deluxe Edition]</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/britpop" | relative_url }}">britpop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a> # 88</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>97</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/307dfc34b0c14f93ac6b5f4548ae2464.png" alt="Cover for Electr-o-pura" style="max-width:120px; height:auto;" />
      </td>
      <td>1995</td>
      <td>Yo La Tengo</td>
      <td>Electr-o-pura</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>89</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f1d9a7c2b30c3272f1218cfff855da2b.png" alt="Cover for A Certain Smile, A Certain Sadness" style="max-width:120px; height:auto;" />
      </td>
      <td>1996-01</td>
      <td>Rocketship</td>
      <td>A Certain Smile, A Certain Sadness</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>13</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a073ac85e2fb427e99cb2d154af8935b.png" alt="Cover for I Can Hear the Heart Beating as One" style="max-width:120px; height:auto;" />
      </td>
      <td>1997-04-22</td>
      <td>Yo La Tengo</td>
      <td>I Can Hear the Heart Beating as One</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a> # 83</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>46</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b0e767ee8094446ac49886333f489cef.png" alt="Cover for Complete Recordings" style="max-width:120px; height:auto;" />
      </td>
      <td>1999-04-27</td>
      <td>Black Tambourine</td>
      <td>Complete Recordings</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>49</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bbd482571c9c4743971bedbd7f9b67d1.png" alt="Cover for Reveille" style="max-width:120px; height:auto;" />
      </td>
      <td>2002-06-04</td>
      <td>Deerhoof</td>
      <td>Reveille</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>76</td>
      <td>
          <img src="https://i.discogs.com/nwq-I87RyK4BN9b-rOVB_ExtpVSd_wJrY9x6XuL3c-A/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTMxNTY2/Ny0xMTc4NDE3NDA2/LmpwZWc.jpeg" alt="Cover for Velocity: design: comfort." style="max-width:120px; height:auto;" />
      </td>
      <td>2003</td>
      <td>Sweet Trip</td>
      <td>Velocity: design: comfort.</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
            <li style="font-size: calc(1rem * 79 / 100)"> <a href="{{ "/tags/idm" | relative_url }}">idm</a></li>
            <li style="font-size: calc(1rem * 60 / 100)"> <a href="{{ "/tags/dream-pop" | relative_url }}">dream pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a> # 3</li>
            <li> <a href="{{ "/tags/idm" | relative_url }}">idm</a> # 15</li>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 19</li>
            <li> <a href="{{ "/tags/glitch" | relative_url }}">glitch</a> # 25</li>
            <li> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a> # 34</li>
            <li> <a href="{{ "/tags/ambient-pop" | relative_url }}">ambient pop</a> # 45</li>
            <li> <a href="{{ "/tags/dream-pop" | relative_url }}">dream pop</a> # 90</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>42</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/205ec595cc9241f69ddd11643823ac96.png" alt="Cover for Take Them On, On Your Own" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-08-20</td>
      <td>Black Rebel Motorcycle Club</td>
      <td>Take Them On, On Your Own</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/garage-rock-revival" | relative_url }}">garage rock revival</a> # 33</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>35</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/002f6411700f4a418a91d9b6ddca3fcc.png" alt="Cover for Spirit They&#39;re Gone, Spirit They&#39;ve Vanished" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-10-06</td>
      <td>Animal Collective</td>
      <td>Spirit They&#39;re Gone, Spirit They&#39;ve Vanished</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/progressive-pop" | relative_url }}">progressive pop</a> # 11</li>
            <li> <a href="{{ "/tags/noise" | relative_url }}">noise</a> # 12</li>
            <li> <a href="{{ "/tags/folktronica" | relative_url }}">folktronica</a> # 16</li>
            <li> <a href="{{ "/tags/freak-folk" | relative_url }}">freak folk</a> # 19</li>
            <li> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a> # 59</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>27</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5c0cbbfe317b46f8a51ef23ae92359d8.png" alt="Cover for Refried Ectoplasm [Switched On Volume 2]" style="max-width:120px; height:auto;" />
      </td>
      <td>2006</td>
      <td>Stereolab</td>
      <td>Refried Ectoplasm [Switched On Volume 2]</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/krautrock" | relative_url }}">krautrock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/psychedelic-rock" | relative_url }}">psychedelic rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/space-rock-revival" | relative_url }}">space rock revival</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/space-rock-revival" | relative_url }}">space rock revival</a> # 1</li>
            <li> <a href="{{ "/tags/krautrock" | relative_url }}">krautrock</a> # 9</li>
            <li> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a> # 14</li>
            <li> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a> # 45</li>
            <li> <a href="{{ "/tags/psychedelic-rock" | relative_url }}">psychedelic rock</a> # 94</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>82</td>
      <td>
      </td>
      <td>2006-04-02</td>
      <td>Elle Milano</td>
      <td>Swearing&#39;s For Art Students</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>79</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/aa964fbf26e84a25b1b544d08b094cfe.png" alt="Cover for Friend Opportunity" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-01-22</td>
      <td>Deerhoof</td>
      <td>Friend Opportunity</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>66</td>
      <td>
          <img src="https://i.discogs.com/L6MdFEeVbKz2KnxzEWvkq2-_CO4RtrNb9m_bXK7Xdrs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIxODQ3/MjQtMTI2ODU3Nzk2/OS5qcGVn.jpeg" alt="Cover for Triángulo de Amor Bizarro" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-02-19</td>
      <td>Triángulo de amor bizarro</td>
      <td>Triángulo de Amor Bizarro</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>100</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f7c7f0ed9537424e9ea656f3c55b210b.png" alt="Cover for You&#39;re Living All Over Me (Remastered)" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-05</td>
      <td>Dinosaur Jr.</td>
      <td>You&#39;re Living All Over Me (Remastered)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>75</td>
      <td>
      </td>
      <td>2007-08-20</td>
      <td>Liars</td>
      <td>Liars</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-punk" | relative_url }}">post-punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/art-punk" | relative_url }}">art punk</a> # 19</li>
            <li> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a> # 90</li>
            <li> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a> # 93</li>
            <li> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a> # 95</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>40</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/380139cf8d2046e4ba63c4845e289ae0.png" alt="Cover for Distortion" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-01-15</td>
      <td>The Magnetic Fields</td>
      <td>Distortion</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>85</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b96f0801ad5f4cd390803c264ba1267c.png" alt="Cover for Two Airships &#x2F; Exploder Falls" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-08-25</td>
      <td>Candy Claws</td>
      <td>Two Airships &#x2F; Exploder Falls</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>72</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/14c70266832c44c2980495b4866ab4fd.png" alt="Cover for We Are Beautiful, We Are Doomed" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-10-06</td>
      <td>Los Campesinos!</td>
      <td>We Are Beautiful, We Are Doomed</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/twee-pop" | relative_url }}">twee pop</a> # 56</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>28</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/eb942d68dd804d99949cc46e6e90ef5c.png" alt="Cover for Jewellery" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-02</td>
      <td>Micachu</td>
      <td>Jewellery</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop-rock" | relative_url }}">pop rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>43</td>
      <td>
          <img src="https://i.discogs.com/OKCwUSTdXIezemwexczZE-GZLz-JkkLBeygxFFP9rQU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE4MzE2/NTYtMTI0NjM4MjI0/Ni5naWY.jpeg" alt="Cover for An When" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-06-10</td>
      <td>Tonstartssbandht</td>
      <td>An When</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/lo-fi" | relative_url }}">lo-fi</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>94</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4eec90eaa8e6f3189924bef205f6ee5a.png" alt="Cover for Sleigh Bells" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-12-06</td>
      <td>Sleigh Bells</td>
      <td>Sleigh Bells</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>84</td>
      <td>
          <img src="https://i.discogs.com/ja4Yy99xodXjXnnkJ0VYWVwpdxIiRu3PeLdb7jT9nDk/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIzNjI2/MTEtMTI4ODIxMzI0/Ni5qcGVn.jpeg" alt="Cover for Tell &#39;Em" style="max-width:120px; height:auto;" />
      </td>
      <td>2010</td>
      <td>Sleigh Bells</td>
      <td>Tell &#39;Em</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>59</td>
      <td>
          <img src="https://i.discogs.com/Vw5LIDqLaie2LlGLDXMszQLI8_liIMsXCT16omPCLog/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTU4MTAw/OTEtMTYwMzIwMzYy/Ni04MzM4LmpwZWc.jpeg" alt="Cover for Romance is Boring" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-02-15</td>
      <td>Los Campesinos!</td>
      <td>Romance is Boring</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 51 / 100)"> <a href="{{ "/tags/indie-pop" | relative_url }}">indie pop</a></li>
            <li style="font-size: calc(1rem * 51 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>8</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ce20760300aa191a41b71d5e33e9d014.png" alt="Cover for Treats" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-05-11</td>
      <td>Sleigh Bells</td>
      <td>Treats</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>44</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ea90bcc6fd4e34aa9607632c36965e6f.png" alt="Cover for King Of The Beach" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-08-02</td>
      <td>Wavves</td>
      <td>King Of The Beach</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/lo-fi" | relative_url }}">lo-fi</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/lo-fi" | relative_url }}">lo-fi</a> # 35</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>73</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/dc04365b484c4f769bd8d9c6d3bdc7f2.png" alt="Cover for The Big Roar" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-01-24</td>
      <td>The Joy Formidable</td>
      <td>The Big Roar</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>15</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ad9616248f514bf3a8fafc228adb5957.png" alt="Cover for He Gets Me High" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-03-04</td>
      <td>Dum Dum Girls</td>
      <td>He Gets Me High</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>52</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/27d56baeb9ed47fc8d8df920412f691b.png" alt="Cover for My Back Is Killing Me Baby" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-03-26</td>
      <td>Car Seat Headrest</td>
      <td>My Back Is Killing Me Baby</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 69</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>24</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a7fa351087bb4d849eb16938fc93f0db.png" alt="Cover for Raven in the Grave" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-04-04</td>
      <td>The Raveonettes</td>
      <td>Raven in the Grave</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>88</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5796808e74451406ad478d011c74c395.png" alt="Cover for Bodysongs" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-09-20</td>
      <td>Born Gold</td>
      <td>Bodysongs</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electropop" | relative_url }}">electropop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a> # 29</li>
            <li> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a> # 91</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>12</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f1cce48f13aa4839c8bfbcb1c6ec5f7b.png" alt="Cover for Reign of Terror" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-02-20</td>
      <td>Sleigh Bells</td>
      <td>Reign of Terror</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 71 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>17</td>
      <td>
      </td>
      <td>2012-06-15</td>
      <td>Teen Suicide</td>
      <td>Goblin Problems</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>78</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e78d64f06dc143efc300d181a64895b0.png" alt="Cover for The Last Weekend of Summer" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-09-02</td>
      <td>Heccra</td>
      <td>The Last Weekend of Summer</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/easycore" | relative_url }}">easycore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/midwest-emo" | relative_url }}">midwest emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/easycore" | relative_url }}">easycore</a> # 14</li>
            <li> <a href="{{ "/tags/midwest-emo" | relative_url }}">midwest emo</a> # 46</li>
            <li> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>29</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7843cead7756408aa2e0c60c786b3440.png" alt="Cover for Breakup Song" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-09-04</td>
      <td>Deerhoof</td>
      <td>Breakup Song</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>32</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/cfe3b3d52b194a048b6c9bcbd958f89a.png" alt="Cover for Observator" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-09-10</td>
      <td>The Raveonettes</td>
      <td>Observator</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>53</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ddbabf15d7a823d7e8c12d937d50634b.png" alt="Cover for still bummed" style="max-width:120px; height:auto;" />
      </td>
      <td>2013</td>
      <td>Nouns</td>
      <td>still bummed</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 74</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>93</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4feafa36e4c04594bc8e24d59ffc44d9.png" alt="Cover for Damage and Joy" style="max-width:120px; height:auto;" />
      </td>
      <td>2013</td>
      <td>The Jesus and Mary Chain</td>
      <td>Damage and Joy</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>69</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1153b95ac1fc420493a7f139f7c08fa4.png" alt="Cover for Dissed And Dismissed" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-02-26</td>
      <td>Tony Molina</td>
      <td>Dissed And Dismissed</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-pop" | relative_url }}">indie pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>64</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/98ee8f307004ac47f938929a37755492.png" alt="Cover for Whirr &amp; Nothing (Split Version)" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-04-20</td>
      <td>Whirr</td>
      <td>Whirr &amp; Nothing (Split Version)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a> # 6</li>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 8</li>
            <li> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a> # 38</li>
            <li> <a href="{{ "/tags/dream-pop" | relative_url }}">dream pop</a> # 72</li>
            <li> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a> # 72</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>57</td>
      <td>
          <img src="https://i.discogs.com/U7vzuOnW95aMyedDA333V5_fms9oTOxQNrnmXi3_iZ8/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQ5ODA3/MDUtMTM4MjI1ODY0/Ni03OTA0LmpwZWc.jpeg" alt="Cover for Egyptian Shumba" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-06</td>
      <td>The Tammys</td>
      <td>Egyptian Shumba</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>31</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/32375ed29f6649dbc96a6bb660e76d08.png" alt="Cover for Bitter Rivals" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-10-08</td>
      <td>Sleigh Bells</td>
      <td>Bitter Rivals</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>1</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/537a68a2dc6a418fc1acf47f894f8146.png" alt="Cover for Living While Starving" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-10-27</td>
      <td>Car Seat Headrest</td>
      <td>Living While Starving</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a> # 4</li>
            <li> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a> # 63</li>
            <li> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>3</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a5cafce5e257246769aa6f05c6f77724.png" alt="Cover for Night Time, My Time" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-10-29</td>
      <td>Sky Ferreira</td>
      <td>Night Time, My Time</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/synthpop" | relative_url }}">synthpop</a> # 41</li>
            <li> <a href="{{ "/tags/dark-pop" | relative_url }}">dark pop</a> # 52</li>
            <li> <a href="{{ "/tags/alternative-dance" | relative_url }}">alternative dance</a> # 58</li>
            <li> <a href="{{ "/tags/gothic-pop" | relative_url }}">gothic pop</a> # 68</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>91</td>
      <td>
      </td>
      <td>2014-02-04</td>
      <td>Xiu Xiu</td>
      <td>Angel Guts: Red Classroom</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/synth-punk" | relative_url }}">synth punk</a> # 30</li>
            <li> <a href="{{ "/tags/post-industrial" | relative_url }}">post-industrial</a> # 41</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>77</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ee337bb61ccb47a0c749fe759c88beff.png" alt="Cover for St. Vincent" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-02-24</td>
      <td>St. Vincent</td>
      <td>St. Vincent</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-pop" | relative_url }}">art pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/art-pop" | relative_url }}">art pop</a> # 66</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>4</td>
      <td>
      </td>
      <td>2014-11-11</td>
      <td>Mitski</td>
      <td>Bury Me at Makeout Creek</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 4</li>
            <li> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a> # 8</li>
            <li> <a href="{{ "/tags/singer-songwriter" | relative_url }}">singer-songwriter</a> # 35</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>50</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/534f9352d5b99c9a397ead93270c3614.png" alt="Cover for Blessing" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-02-11</td>
      <td>Alex G</td>
      <td>Blessing</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>19</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/703616fde4bec426d7819777f11e2e98.png" alt="Cover for Puberty 2" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-06-17</td>
      <td>Mitski</td>
      <td>Puberty 2</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a> # 17</li>
            <li> <a href="{{ "/tags/art-pop" | relative_url }}">art pop</a> # 23</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>6</td>
      <td>
          <img src="https://i.discogs.com/rGOoOW84RpOWCmKOg5VKKepnt26MDXmSdhRANVHnEOc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTc2MTA4/MjMtMTUzNTc3NDcw/MC00NDEzLmpwZWc.jpeg" alt="Cover for Teens Of Style" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-07-08</td>
      <td>Car Seat Headrest</td>
      <td>Teens Of Style</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/power-pop" | relative_url }}">power pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/power-pop" | relative_url }}">power pop</a> # 12</li>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 27</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>7</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/73fcb06011ca424b57705f64e6a0535a.png" alt="Cover for return" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-07-11</td>
      <td>blue smiley</td>
      <td>return</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 3</li>
            <li> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a> # 9</li>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 25</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>11</td>
      <td>
          <img src="https://i.discogs.com/FDD6DX23wW47PduwipKPXGYQKDkvkPRslVl4Wm3GYrQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTk2ODky/MjMtMTQ4NDgwODEw/My04OTAyLmpwZWc.jpeg" alt="Cover for Radiolove" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-08-19</td>
      <td>tanukichan</td>
      <td>Radiolove</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 39</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>41</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8e860a0c239340dfc0df673dd483934a.png" alt="Cover for Pe&#39;ahi" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-12-30</td>
      <td>The Raveonettes</td>
      <td>Pe&#39;ahi</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>16</td>
      <td>
          <img src="https://i.discogs.com/lb9IJiEsoiu9lN3fMZWUgNjgpUIC_EXtBpiKVzQkF4w/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwODk0/NTcyLTE1MDYwOTkx/OTEtMzkzMy5qcGVn.jpeg" alt="Cover for Visions Of A Life" style="max-width:120px; height:auto;" />
      </td>
      <td>2017</td>
      <td>Wolf Alice</td>
      <td>Visions Of A Life</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
            <li style="font-size: calc(1rem * 60 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 14</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>70</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/81ad6437f703ac6cc9a9ea4244729cb1.png" alt="Cover for Plastic Cough" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-07-07</td>
      <td>Great Grandpa</td>
      <td>Plastic Cough</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>2</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3b7d699929961ff0ae19159d313b9194.png" alt="Cover for TOTEP" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-02-20</td>
      <td>Kero Kero Bonito</td>
      <td>TOTEP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/twee-pop" | relative_url }}">twee pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/twee-pop" | relative_url }}">twee pop</a> # 2</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>63</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/dab838e3181321e7923b25271389180d.png" alt="Cover for No Flashlight" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-03-16</td>
      <td>Mount Eerie</td>
      <td>No Flashlight</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-folk" | relative_url }}">indie folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>20</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/879a4513ba84fdb19f5e19affa55c5ca.png" alt="Cover for WASTEISOLATION" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-04-13</td>
      <td>Black Dresses</td>
      <td>WASTEISOLATION</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electro-industrial" | relative_url }}">electro-industrial</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/electro-industrial" | relative_url }}">electro-industrial</a> # 4</li>
            <li> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a> # 7</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>23</td>
      <td>
          <img src="https://i.discogs.com/wTZ1NyVIm2cJf5AB2BIMKMiYY2v-7D5cotC4scSI2YI/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEyNzM3/NTYwLTE1NDEwMDEy/NTAtNjM5Ny5qcGVn.jpeg" alt="Cover for Baby You&#39;re a Haunted House" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-10-26</td>
      <td>Gerard Way</td>
      <td>Baby You&#39;re a Haunted House</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>18</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e873ef3caa501f6270b17c952e5d547f.png" alt="Cover for Time &#39;n&#39; Place" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-12-14</td>
      <td>Kero Kero Bonito</td>
      <td>Time &#39;n&#39; Place</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/twee-pop" | relative_url }}">twee pop</a> # 18</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>71</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/36adb5886e230e682c0f8368dd1c040c.png" alt="Cover for The Devil-Faces of My Old Friends, Beneath Me" style="max-width:120px; height:auto;" />
      </td>
      <td>2019</td>
      <td>Heccra</td>
      <td>The Devil-Faces of My Old Friends, Beneath Me</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/nintendocore" | relative_url }}">nintendocore</a> # 5</li>
            <li> <a href="{{ "/tags/easycore" | relative_url }}">easycore</a> # 11</li>
            <li> <a href="{{ "/tags/trancecore" | relative_url }}">trancecore</a> # 16</li>
            <li> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a> # 49</li>
            <li> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a> # 58</li>
            <li> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a> # 86</li>
            <li> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a> # 97</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>38</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/04320832727734600b1bdd210630b484.png" alt="Cover for Come In" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-04-29</td>
      <td>Weatherday</td>
      <td>Come In</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/lgbt" | relative_url }}">lgbt</a> # 62</li>
            <li> <a href="{{ "/tags/lo-fi-indie" | relative_url }}">lo-fi indie</a> # 65</li>
            <li> <a href="{{ "/tags/midwest-emo" | relative_url }}">midwest emo</a> # 67</li>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 80</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>87</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c8eeea6b798dbed8249fd273c422ece9.png" alt="Cover for Only us and linger on" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-04-29</td>
      <td>Weatherday</td>
      <td>Only us and linger on</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/midwest-emo" | relative_url }}">midwest emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 24</li>
            <li> <a href="{{ "/tags/midwest-emo" | relative_url }}">midwest emo</a> # 52</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>47</td>
      <td>
          <img src="https://i.discogs.com/_nFMInMOKAs3-UAB5Neav-1VZqGuwWJrx8zdHCEiB3M/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE5MTg1/MjA1LTE2MjQwMTc5/ODAtMzg2Ny5qcGVn.jpeg" alt="Cover for Various Types Of Ads" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-08-26</td>
      <td>Loli in early 20s</td>
      <td>Various Types Of Ads</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/glitch" | relative_url }}">glitch</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/glitch" | relative_url }}">glitch</a> # 38</li>
            <li> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a> # 59</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>22</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b1548e7729ccfc2f7cbea83f5552c500.png" alt="Cover for What&#39;s Tonight To Eternity" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-02-14</td>
      <td>Cindy Lee</td>
      <td>What&#39;s Tonight To Eternity</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hypnagogic-pop" | relative_url }}">hypnagogic pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise" | relative_url }}">noise</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/noise" | relative_url }}">noise</a> # 7</li>
            <li> <a href="{{ "/tags/hypnagogic-pop" | relative_url }}">hypnagogic pop</a> # 16</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>81</td>
      <td>
          <img src="https://i.discogs.com/VWgcQU6ZvbJhJBCZn7OyysG7Po9WZg50c5RDrJgp2Ps/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1MzA3/NjgyLTE1ODk1MDM3/NTEtMTI5Ni5qcGVn.jpeg" alt="Cover for How I&#39;m Feeling Now" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-05-15</td>
      <td>Charli XCX</td>
      <td>How I&#39;m Feeling Now</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/bubblegum-bass" | relative_url }}">bubblegum bass</a></li>
            <li style="font-size: calc(1rem * 84 / 100)"> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a></li>
            <li style="font-size: calc(1rem * 62 / 100)"> <a href="{{ "/tags/electropop" | relative_url }}">electropop</a></li>
            <li style="font-size: calc(1rem * 62 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/bubblegum-bass" | relative_url }}">bubblegum bass</a> # 2</li>
            <li> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a> # 3</li>
            <li> <a href="{{ "/tags/deconstructed-club" | relative_url }}">deconstructed club</a> # 22</li>
            <li> <a href="{{ "/tags/electropop" | relative_url }}">electropop</a> # 28</li>
            <li> <a href="{{ "/tags/experimental-pop" | relative_url }}">experimental pop</a> # 30</li>
            <li> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a> # 36</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>58</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ad8db59a373d755ad3a8ce46c2ff5b4c.png" alt="Cover for bedhead" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-03-24</td>
      <td>Bar Italia</td>
      <td>bedhead</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hypnagogic-pop" | relative_url }}">hypnagogic pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-punk" | relative_url }}">post-punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/psychedelic-folk" | relative_url }}">psychedelic folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 17</li>
            <li> <a href="{{ "/tags/psychedelic-folk" | relative_url }}">psychedelic folk</a> # 24</li>
            <li> <a href="{{ "/tags/hypnagogic-pop" | relative_url }}">hypnagogic pop</a> # 35</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>37</td>
      <td>
          <img src="https://i.discogs.com/7pq3KTG2C0XFb7ZrU-C3i_Y6ytqAHjr8U3KxeyF7M4c/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE4Mjk5/NTUxLTE2MTg0Mzg0/MDAtNzY0NC5qcGVn.jpeg" alt="Cover for PXE" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-03-31</td>
      <td>ECCO2k</td>
      <td>PXE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-pop" | relative_url }}">art pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a> # 2</li>
            <li> <a href="{{ "/tags/art-pop" | relative_url }}">art pop</a> # 21</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>34</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/04464206a1908e3f02061d6806b3f171.png" alt="Cover for Sophia Chablau e Uma Enorme Perda de Tempo" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-04</td>
      <td>Sophia Chablau e Uma Enorme Perda de Tempo</td>
      <td>Sophia Chablau e Uma Enorme Perda de Tempo</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-pop" | relative_url }}">indie pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>5</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fea086e4992bf799f708ffd9803310ae.png" alt="Cover for pushing daisies" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-08-12</td>
      <td>Julie</td>
      <td>pushing daisies</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 5</li>
            <li> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a> # 23</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>51</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f4efcb4378cdec13aea117c6bb559415.png" alt="Cover for frailty" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-11-12</td>
      <td>dltzk</td>
      <td>frailty</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a> # 72</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>55</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7b0a7000d5f53779cd208d4702035c83.png" alt="Cover for Weatherglow" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-11-26</td>
      <td>Asian Glow</td>
      <td>Weatherglow</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 15</li>
            <li> <a href="{{ "/tags/emo" | relative_url }}">emo</a> # 70</li>
            <li> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a> # 77</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>30</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b1210a05d7b4b4f131b17f2009420b33.png" alt="Cover for Between II Gardens" style="max-width:120px; height:auto;" />
      </td>
      <td>2022</td>
      <td>Sword II</td>
      <td>Between II Gardens</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dream-pop" | relative_url }}">dream pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-punk" | relative_url }}">post-punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 10</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>45</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e41b308ca8a94f72e26a79320a3bf313.png" alt="Cover for Blue Rev" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-10-07</td>
      <td>Alvvays</td>
      <td>Blue Rev</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>25</td>
      <td>
      </td>
      <td>2022-11-16</td>
      <td>Jane Remover</td>
      <td>frailty</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a> # 10</li>
            <li> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a> # 24</li>
            <li> <a href="{{ "/tags/bitpop" | relative_url }}">bitpop</a> # 47</li>
            <li> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a> # 61</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>56</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/852f011617b125728935de19620b9f05.png" alt="Cover for pg.4 a picture of three hedges&#x2F;through your window" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-02-17</td>
      <td>Julie</td>
      <td>pg.4 a picture of three hedges&#x2F;through your window</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/grunge" | relative_url }}">grunge</a> # 75</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>21</td>
      <td>
          <img src="https://i.discogs.com/ZIiXV-DhNciC64Dl675Q-rdLVH7JWUpoykFAHsKQDAE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI2NTY1/NTg0LTE2Nzk5NDcx/MjItNzgzMy5wbmc.jpeg" alt="Cover for Praise A Lord Who Chews But Which Does Not Consume; (Or Simply, Hot Between Worlds)" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-03-17</td>
      <td>Yves Tumor</td>
      <td>Praise A Lord Who Chews But Which Does Not Consume; (Or Simply, Hot Between Worlds)</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/dance-punk" | relative_url }}">dance-punk</a> # 5</li>
            <li> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a> # 13</li>
            <li> <a href="{{ "/tags/post-punk-revival" | relative_url }}">post-punk revival</a> # 18</li>
            <li> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a> # 36</li>
            <li> <a href="{{ "/tags/post-punk" | relative_url }}">post-punk</a> # 49</li>
            <li> <a href="{{ "/tags/glam-rock" | relative_url }}">glam rock</a> # 64</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>61</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b2adaaa89951430f98fea59ea800c2d8.png" alt="Cover for Spirit They&#39;re Gone Spirit They&#39;ve Vanished" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-05-12</td>
      <td>Animal Collective</td>
      <td>Spirit They&#39;re Gone Spirit They&#39;ve Vanished</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/freak-folk" | relative_url }}">freak folk</a> # 77</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>9</td>
      <td>
          <img src="https://i.discogs.com/aB5LD8JpL343ykW5sHgbekH4WXTRFYT3VthpJMe-NQM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI4NTYy/NTYwLTE2OTcwMzYw/MjctODk4NS5qcGVn.jpeg" alt="Cover for S33K H3LP" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-05-28</td>
      <td>femtanyl</td>
      <td>S33K H3LP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/breakbeat" | relative_url }}">breakbeat</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/breakbeat-hardcore" | relative_url }}">breakbeat hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/digital-hardcore" | relative_url }}">digital hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-punk" | relative_url }}">noise punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/synth-punk" | relative_url }}">synth punk</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/noise-punk" | relative_url }}">noise punk</a> # 1</li>
            <li> <a href="{{ "/tags/breakbeat-hardcore" | relative_url }}">breakbeat hardcore</a> # 1</li>
            <li> <a href="{{ "/tags/synth-punk" | relative_url }}">synth punk</a> # 1</li>
            <li> <a href="{{ "/tags/digital-hardcore" | relative_url }}">digital hardcore</a> # 2</li>
            <li> <a href="{{ "/tags/breakbeat" | relative_url }}">breakbeat</a> # 5</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>33</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f092d27fd89f140a0ca84aaa6b00dc4c.png" alt="Cover for Girl With Fish" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-06-09</td>
      <td>feeble little horse</td>
      <td>Girl With Fish</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/lo-fi-indie" | relative_url }}">lo-fi indie</a> # 23</li>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 85</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>74</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/37a741f92935dac4203daa1e81af2fbe.png" alt="Cover for i&#39;m so lucky" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-09-01</td>
      <td>Spirit of the Beehive</td>
      <td>i&#39;m so lucky</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-punk" | relative_url }}">post-punk</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>14</td>
      <td>
          <img src="https://i.discogs.com/OPrGBAcb04cqMS53ePPAbX_T_a0S_KpLGkL-dAN2Z4A/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI4MzU3/NDQ3LTE2OTUzNTkx/NjAtODU1OC5qcGVn.jpeg" alt="Cover for Wallsocket" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-09-22</td>
      <td>Underscores</td>
      <td>Wallsocket</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a> # 26</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>92</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ff6b0a7ac633a7ae26c8e4da9a663bef.png" alt="Cover for Cartwheel" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-11-03</td>
      <td>Hotline TNT</td>
      <td>Cartwheel</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/power-pop" | relative_url }}">power pop</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/power-pop" | relative_url }}">power pop</a> # 57</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>67</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/501f9d803c4d379e229775b39e0d347f.png" alt="Cover for Stepdream" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-11-10</td>
      <td>quannnic</td>
      <td>Stepdream</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grunge" | relative_url }}">grunge</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a> # 77</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>26</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/00e00de8eeac2004839c87384be90f20.png" alt="Cover for Nineteen In Love" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Hotline TNT</td>
      <td>Nineteen In Love</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dream-pop" | relative_url }}">dream pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/lo-fi" | relative_url }}">lo-fi</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/slacker-rock" | relative_url }}">slacker rock</a> # 23</li>
            <li> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a> # 59</li>
            <li> <a href="{{ "/tags/lo-fi" | relative_url }}">lo-fi</a> # 66</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>80</td>
      <td>
      </td>
      <td></td>
      <td>Estelle Allen</td>
      <td>music 2</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/digicore" | relative_url }}">digicore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-pop" | relative_url }}">experimental pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-pop" | relative_url }}">experimental pop</a> # 7</li>
            <li> <a href="{{ "/tags/glitch-pop" | relative_url }}">glitch pop</a> # 26</li>
            <li> <a href="{{ "/tags/digicore" | relative_url }}">digicore</a> # 28</li>
            <li> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a> # 87</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>90</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/739a3d93d8b994af60a6b3dafa6be6d0.png" alt="Cover for White Ceiling &#x2F; Black Dots Wandering Around" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Parannoul</td>
      <td>White Ceiling &#x2F; Black Dots Wandering Around</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-pop" | relative_url }}">noise pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/shoegaze" | relative_url }}">shoegaze</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
  </tbody>
</table>

### Related genres

- [indie rock]({{ "/tags/indie-rock" | relative_url }})
- [shoegaze]({{ "/tags/shoegaze" | relative_url }})
- [noise rock]({{ "/tags/noise-rock" | relative_url }})
- [dream pop]({{ "/tags/dream-pop" | relative_url }})
- [alternative]({{ "/tags/alternative" | relative_url }})
- [alternative rock]({{ "/tags/alternative-rock" | relative_url }})
- [indie pop]({{ "/tags/indie-pop" | relative_url }})
- [lo-fi]({{ "/tags/lo-fi" | relative_url }})
- [rock]({{ "/tags/rock" | relative_url }})
- [noise]({{ "/tags/noise" | relative_url }})
