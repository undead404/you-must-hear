---
category: genre
date: 2025-11-19T20:20:05.576Z
permalink: tags/new-age
title: new age
---

## new age

Last updated: <time datetime="2025-11-19T20:20:05.576Z">Wed Nov 19 2025 22:20:05 GMT+0200 (Eastern European Standard Time)</time>

### Description

New age music is a genre of music that emerged in the 1970s and is characterized by its relaxing and meditative qualities. It often features instrumental pieces that combine acoustic and electronic elements, with an emphasis on creating a calming and peaceful atmosphere. New age music has been used for a variety of purposes, including meditation, relaxation, yoga, and as background music for spa treatments and other wellness activities. Some of the most popular new age artists include Enya, Yanni, and Kitaro.

### 100 chronological releases

<table>
  <thead>
    <tr>
      <th>Place</th>
      <th>Cover</th>
      <th>Date</th>
      <th>Artist</th>
      <th>Album</th>
      <th>Tags</th>
      <th>Other places</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>87</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/264a3259e67773743a3e982b1742ba2b.png" alt="Cover for The Division Bell (2011 Remastered Version)" style="max-width:120px; height:auto;" />
      </td>
      <td>1971-01</td>
      <td>Pink Floyd</td>
      <td>The Division Bell (2011 Remastered Version)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/space-rock" | relative_url }}">space rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a> # 23</li>
            <li> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a> # 73</li>
            <li> <a href="{{ "/tags/space-rock" | relative_url }}">space rock</a> # 94</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>67</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/646de2297f18495eb7e20c5da4675daa.png" alt="Cover for Hosianna Mantra" style="max-width:120px; height:auto;" />
      </td>
      <td>1972</td>
      <td>Popol Vuh</td>
      <td>Hosianna Mantra</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 82 / 100)"> <a href="{{ "/tags/krautrock" | relative_url }}">krautrock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/krautrock" | relative_url }}">krautrock</a> # 86</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>12</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/93d6842955b0473abb8a86ce05041134.png" alt="Cover for Mythos" style="max-width:120px; height:auto;" />
      </td>
      <td>1972</td>
      <td>Mythos</td>
      <td>Mythos</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 54 / 100)"> <a href="{{ "/tags/chillout" | relative_url }}">chillout</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>56</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d9923b228193054110d5ea3df0e9dea9.png" alt="Cover for Ommadawn" style="max-width:120px; height:auto;" />
      </td>
      <td>1975-10-21</td>
      <td>Mike Oldfield</td>
      <td>Ommadawn</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a> # 46</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>60</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c1b3ad9f823b9ce2b880ae700c1f4677.png" alt="Cover for The Friends Of Mr Cairo" style="max-width:120px; height:auto;" />
      </td>
      <td>1981</td>
      <td>Jon &amp; Vangelis</td>
      <td>The Friends Of Mr Cairo</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 77 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>11</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/547a81d3bf55072b5fffa2e5976bb28e.png" alt="Cover for Asia" style="max-width:120px; height:auto;" />
      </td>
      <td>1982</td>
      <td>Kitaro</td>
      <td>Asia</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/asia" | relative_url }}">asia</a> # 39</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>39</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1608839da50c494e9f145301431d8e62.png" alt="Cover for Crises" style="max-width:120px; height:auto;" />
      </td>
      <td>1983-05-27</td>
      <td>Mike Oldfield</td>
      <td>Crises</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a> # 42</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>7</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d8301fa134f64f3f8e1eaa28908ed9eb.png" alt="Cover for Zoolook" style="max-width:120px; height:auto;" />
      </td>
      <td>1984</td>
      <td>Jean Michel Jarre</td>
      <td>Zoolook</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/synthesizer" | relative_url }}">synthesizer</a> # 38</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>61</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e36bee3f48c36375696bc16bbeb48344.png" alt="Cover for The Dreaming (2018 Remaster)" style="max-width:120px; height:auto;" />
      </td>
      <td>1984</td>
      <td>Kate Bush</td>
      <td>The Dreaming (2018 Remaster)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>73</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b05f72ac6af78bfe921a973dcaac02ac.png" alt="Cover for Wind" style="max-width:120px; height:auto;" />
      </td>
      <td>1986</td>
      <td>Gigi Masin</td>
      <td>Wind</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient-pop" | relative_url }}">ambient pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/jazz" | relative_url }}">jazz</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ambient-pop" | relative_url }}">ambient pop</a> # 84</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>35</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a8d685762927898475780b2928a90378.png" alt="Cover for Enya" style="max-width:120px; height:auto;" />
      </td>
      <td>1986</td>
      <td>Enya</td>
      <td>Enya</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>1</td>
      <td>
      </td>
      <td>1988</td>
      <td>David Lanz</td>
      <td>Cristofori&#39;s Dream</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/piano" | relative_url }}">piano</a></li>
            <li style="font-size: calc(1rem * 84 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>78</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/636d4d7e29efb7966c7035a866c51a75.png" alt="Cover for Watermark" style="max-width:120px; height:auto;" />
      </td>
      <td>1988-09-19</td>
      <td>Enya</td>
      <td>Watermark</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 81</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>10</td>
      <td>
          <img src="https://i.discogs.com/pvW3ynP607PZmMbCCwNo_uT7_8u4Lkd42X9dDf3G3Yw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTc1MzU3/MC0xMTU1MzM1NTE5/LmpwZWc.jpeg" alt="Cover for Ancient Journeys" style="max-width:120px; height:auto;" />
      </td>
      <td>1990</td>
      <td>Cusco</td>
      <td>Ancient Journeys</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>92</td>
      <td>
          <img src="https://i.discogs.com/HIGoMWUUjK3H20PjFEBF6axofnZvIhTx10jKEHLB-UQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTk3OTEz/NTQtMTUxNTM4MTA4/NS04NTE5LmpwZWc.jpeg" alt="Cover for Mcmxc Ad" style="max-width:120px; height:auto;" />
      </td>
      <td>1990</td>
      <td>Enigma</td>
      <td>Mcmxc Ad</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>97</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9a07cd9e67d38801b0f027acc2db6e7c.png" alt="Cover for MCMXC a.D." style="max-width:120px; height:auto;" />
      </td>
      <td>1990-12-03</td>
      <td>Enigma</td>
      <td>MCMXC a.D.</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/chillout" | relative_url }}">chillout</a> # 13</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>98</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/331b49efcc554fc181a393923f1afbd0.png" alt="Cover for MCMXC A.D" style="max-width:120px; height:auto;" />
      </td>
      <td>1991</td>
      <td>Enigma</td>
      <td>MCMXC A.D</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/chillout" | relative_url }}">chillout</a> # 18</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>15</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2d783c80c740484ac836a5596954807f.png" alt="Cover for Dream" style="max-width:120px; height:auto;" />
      </td>
      <td>1992</td>
      <td>Kitaro</td>
      <td>Dream</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>95</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9a10ff9fd20a4a25caf2f146e56f4ab2.png" alt="Cover for Enigma 2 - The Cross Of Changes" style="max-width:120px; height:auto;" />
      </td>
      <td>1993</td>
      <td>Enigma</td>
      <td>Enigma 2 - The Cross Of Changes</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>86</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/169704fbcca84b1ba474c2cf42b5420c.png" alt="Cover for The Cross of Changes" style="max-width:120px; height:auto;" />
      </td>
      <td>1993-12-06</td>
      <td>Enigma</td>
      <td>The Cross of Changes</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>94</td>
      <td>
          <img src="https://i.discogs.com/BInqHexXPU6x9pk-JlZKvz_IXUH14fnOYVyPxpnh4UE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTExMjMw/LTE2MDgyNDA3ODct/NTc2Ny5wbmc.jpeg" alt="Cover for Cross Of Changes" style="max-width:120px; height:auto;" />
      </td>
      <td>1993-12-06</td>
      <td>Enigma</td>
      <td>Cross Of Changes</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>77</td>
      <td>
          <img src="https://i.discogs.com/Qwuw6eCnZ8cjZSl1cB2yFxjdZTr6hSkOIYX5imAR6Ic/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTM1MTk1/LTEzOTc1ODE5MDIt/MzExNC5qcGVn.jpeg" alt="Cover for Songs Of Distant Earth" style="max-width:120px; height:auto;" />
      </td>
      <td>1994</td>
      <td>Mike Oldfield</td>
      <td>Songs Of Distant Earth</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>37</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/150e6f94975344adf3d2199b73318b7b.png" alt="Cover for Alegria" style="max-width:120px; height:auto;" />
      </td>
      <td>1994-10-11</td>
      <td>Cirque Du Soleil</td>
      <td>Alegria</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 73 / 100)"> <a href="{{ "/tags/soundtrack" | relative_url }}">soundtrack</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>65</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2bdaef4ac600cb697304369d62803d26.png" alt="Cover for The Songs of Distant Earth" style="max-width:120px; height:auto;" />
      </td>
      <td>1994-11-21</td>
      <td>Mike Oldfield</td>
      <td>The Songs of Distant Earth</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>42</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0ff788a63c5d4c74ae0830086d8f071f.png" alt="Cover for Songs From a Secret Garden" style="max-width:120px; height:auto;" />
      </td>
      <td>1995</td>
      <td>Secret Garden</td>
      <td>Songs From a Secret Garden</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 85</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>76</td>
      <td>
      </td>
      <td>1995-05-04</td>
      <td>Adiemus</td>
      <td>Songs of Sanctuary</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/choral" | relative_url }}">choral</a> # 90</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>74</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/576e78c8375a4e17bc8fbf4fa19f2966.png" alt="Cover for Boheme" style="max-width:120px; height:auto;" />
      </td>
      <td>1995-06-08</td>
      <td>Deep Forest</td>
      <td>Boheme</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 65 / 100)"> <a href="{{ "/tags/world" | relative_url }}">world</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/world" | relative_url }}">world</a> # 84</li>
            <li> <a href="{{ "/tags/ethnic" | relative_url }}">ethnic</a> # 96</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>89</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e793951f5d8fc2de696ea880f9bd8437.png" alt="Cover for The Memory of Trees" style="max-width:120px; height:auto;" />
      </td>
      <td>1995-11-20</td>
      <td>Enya</td>
      <td>The Memory of Trees</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 79 / 100)"> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 94</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>85</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/334bd432070d4894acd87f596d902724.png" alt="Cover for Le Roi Est Mort Vive Le Roi" style="max-width:120px; height:auto;" />
      </td>
      <td>1996</td>
      <td>Enigma</td>
      <td>Le Roi Est Mort Vive Le Roi</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>22</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6748a7fa94f54b23bf6a681fc309930c.png" alt="Cover for Era" style="max-width:120px; height:auto;" />
      </td>
      <td>1996</td>
      <td>Era</td>
      <td>Era</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>72</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b4d1c66ea55e474cad71a8ebe62f9294.png" alt="Cover for Le Roi Est Mort, Vive Le Roi!" style="max-width:120px; height:auto;" />
      </td>
      <td>1996-11-25</td>
      <td>Enigma</td>
      <td>Le Roi Est Mort, Vive Le Roi!</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>23</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fee1de72c64a25844f8001ab99e91bca.png" alt="Cover for Ancient Nazca - Inca Mysteries" style="max-width:120px; height:auto;" />
      </td>
      <td>1997</td>
      <td>Medwyn Goodall</td>
      <td>Ancient Nazca - Inca Mysteries</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/flute" | relative_url }}">flute</a> # 41</li>
            <li> <a href="{{ "/tags/native-american" | relative_url }}">native american</a> # 88</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>20</td>
      <td>
          <img src="https://i.discogs.com/LRHz3MMcq32UgrJo42jwAoZSXMv-7zlWM4iudhQ9lyA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTU2MTA1/OS0xMTU3NzQ4MzIx/LmpwZWc.jpeg" alt="Cover for The Book Of Secrets" style="max-width:120px; height:auto;" />
      </td>
      <td>1997-09-30</td>
      <td>Loreena McKennitt</td>
      <td>The Book Of Secrets</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ethereal" | relative_url }}">ethereal</a> # 20</li>
            <li> <a href="{{ "/tags/poptron" | relative_url }}">poptron</a> # 64</li>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 99</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>80</td>
      <td>
      </td>
      <td>1997-11-10</td>
      <td>Enya</td>
      <td>Paint the Sky With Stars: The Best of Enya</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 83</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>53</td>
      <td>
      </td>
      <td>1997-11-10</td>
      <td>Enya</td>
      <td>Paint The Sky With Stars</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>28</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/849a96a3f3436beeef36df34802fe967.png" alt="Cover for Eden" style="max-width:120px; height:auto;" />
      </td>
      <td>1998</td>
      <td>Sarah Brightman</td>
      <td>Eden</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/classical-crossover" | relative_url }}">classical crossover</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/classical-crossover" | relative_url }}">classical crossover</a> # 84</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>5</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c30c3d8a2c8f45e2bfc5a21a2639b2c2.png" alt="Cover for Millenium Hits" style="max-width:120px; height:auto;" />
      </td>
      <td>1998</td>
      <td>Era</td>
      <td>Millenium Hits</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>47</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/89d7c14e743065ef85ee21a9426e230e.png" alt="Cover for Myth" style="max-width:120px; height:auto;" />
      </td>
      <td>1998</td>
      <td>Chorus of Tribes</td>
      <td>Myth</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 72 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 72 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>16</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ee9d91bd8eaa429db39249ad8fd69f19.png" alt="Cover for Comparsa" style="max-width:120px; height:auto;" />
      </td>
      <td>1998-01-14</td>
      <td>Deep Forest</td>
      <td>Comparsa</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/world" | relative_url }}">world</a></li>
            <li style="font-size: calc(1rem * 59 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/world" | relative_url }}">world</a> # 76</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>3</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/82a9baff5e056b1da3429a39dad68cd2.png" alt="Cover for Land Of Forever" style="max-width:120px; height:auto;" />
      </td>
      <td>1998-05-19</td>
      <td>2002</td>
      <td>Land Of Forever</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 72 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>30</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f83799cf450cb82f291fd14fe92514ef.png" alt="Cover for Masters of Chant" style="max-width:120px; height:auto;" />
      </td>
      <td>1999</td>
      <td>Gregorian</td>
      <td>Masters of Chant</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/choir" | relative_url }}">choir</a> # 68</li>
            <li> <a href="{{ "/tags/chant" | relative_url }}">chant</a> # 79</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>55</td>
      <td>
      </td>
      <td>1999-03-02</td>
      <td>Secret Garden</td>
      <td>Dawn of a New Century</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>21</td>
      <td>
      </td>
      <td>2000</td>
      <td>Anugama</td>
      <td>Shamanic Dream II</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/meditation" | relative_url }}">meditation</a> # 29</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>48</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/991b35c0ea3c561431be79d84c8455f0.png" alt="Cover for Global Spirit" style="max-width:120px; height:auto;" />
      </td>
      <td>2000</td>
      <td>Karunesh</td>
      <td>Global Spirit</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ethnic" | relative_url }}">ethnic</a> # 95</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>88</td>
      <td>
          <img src="https://i.discogs.com/BblRLaPbh7YdP-_luyol46Lddc7u0R-0WRp697dHvFE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTM4NTcz/MC0xMTg4NjEzNzgy/LmpwZWc.jpeg" alt="Cover for Era 1" style="max-width:120px; height:auto;" />
      </td>
      <td>2000</td>
      <td>Era</td>
      <td>Era 1</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 61</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>63</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fe7f4c29fe1bbbd46455c41cc57af875.png" alt="Cover for Era I 2002" style="max-width:120px; height:auto;" />
      </td>
      <td>2000</td>
      <td>Era</td>
      <td>Era I 2002</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>69</td>
      <td>
          <img src="https://i.discogs.com/QBVxiqKQUSmG5AYymnyfNV_HGeNTKBRbSkU5cHTpuQs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIwMjE5/MzA4LTE2MzE2NzU3/MTctNTQ1Ni5qcGVn.jpeg" alt="Cover for Paint The Sky With Stars: The" style="max-width:120px; height:auto;" />
      </td>
      <td>2000</td>
      <td>Enya</td>
      <td>Paint The Sky With Stars: The</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>71</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/39c41b2a01cdf334b36c9c409037c082.png" alt="Cover for The Screen Behind the Mirror" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-01-13</td>
      <td>Enigma</td>
      <td>The Screen Behind the Mirror</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>27</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6bf5ee9918fa425aa8dd38829d7cc6f6.png" alt="Cover for Era 2" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-05-15</td>
      <td>Era</td>
      <td>Era 2</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>81</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4d6a333808f4a1e7cd69d30ed457248e.png" alt="Cover for A Day Without Rain" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-11-11</td>
      <td>Enya</td>
      <td>A Day Without Rain</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 98</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>79</td>
      <td>
          <img src="https://i.discogs.com/UxRax5WpNcEg7vqKIFhCehaeLgt52OSMJZxPu9to90E/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEyNTA5/NTI4LTE1MzY2ODQw/MDgtODc4MC5qcGVn.jpeg" alt="Cover for Paint the Sky With Stars - The Best of Enya" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Enya</td>
      <td>Paint the Sky With Stars - The Best of Enya</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 82</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>18</td>
      <td>
          <img src="https://i.discogs.com/PSc-wJnsqMyQklFfugX4zS1BInUfWgj10Z8cNGj_MxE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTc2MDIw/ODAtMTQ0NDkyMzgw/MC00MjIyLmpwZWc.jpeg" alt="Cover for Aquaria - A Liquid Blue Transecape" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Diane Arkenstone</td>
      <td>Aquaria - A Liquid Blue Transecape</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>82</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e0c6116ebb37474f8c1bb4e05bd5d760.png" alt="Cover for Unearthed" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-01-01</td>
      <td>E.S. Posthumus</td>
      <td>Unearthed</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/orchestral" | relative_url }}">orchestral</a> # 61</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>58</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/53d52f511ff1de9a575b4a8426052380.png" alt="Cover for Zen Breakfast" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-07-10</td>
      <td>Karunesh</td>
      <td>Zen Breakfast</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/zen" | relative_url }}">zen</a> # 100</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>44</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b4c1ea90f0c643c1bab6d3796ae1d14e.png" alt="Cover for Aquaria: A Liquid Blue Trancescape" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-10-02</td>
      <td>Diane Arkenstone</td>
      <td>Aquaria: A Liquid Blue Trancescape</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>64</td>
      <td>
          <img src="https://i.discogs.com/sg82ymSUO9Q18c8LHRAjwQrkvvJzQgjj0uR-gg3HOmo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTMyOTEw/MTktMTMyNDIyMjc4/Ny5qcGVn.jpeg" alt="Cover for Themes From &quot;Calmi Cuori Appassionati&quot;" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-10-24</td>
      <td>Enya</td>
      <td>Themes From &quot;Calmi Cuori Appassionati&quot;</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>17</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/02d40218b1714225a83892bdef066295.png" alt="Cover for Jewel in the Sun" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>Diane Arkenstone</td>
      <td>Jewel in the Sun</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>45</td>
      <td>
      </td>
      <td>2002-01-02</td>
      <td>2002</td>
      <td>Across an Ocean of Dreams</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>83</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/829f4a5ca70bd3818331f627519ac974.png" alt="Cover for Aphelion" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-01-28</td>
      <td>Amethystium</td>
      <td>Aphelion</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 51 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>70</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bb88a22fc364c5454c13ddbbdfb77271.png" alt="Cover for The Mass" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-03-18</td>
      <td>Era</td>
      <td>The Mass</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 23</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>75</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0e58521ce8564f7fa8ae65e0b2567d87.png" alt="Cover for Voyageur" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-09-08</td>
      <td>Enigma</td>
      <td>Voyageur</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>46</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6c4ed450b55e4876ae32630d9ee5ce72.png" alt="Cover for The Very Best Of" style="max-width:120px; height:auto;" />
      </td>
      <td>2004</td>
      <td>Era</td>
      <td>The Very Best Of</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/chant" | relative_url }}">chant</a> # 24</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>49</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/19327a601eea461fb65ec98ca314d6e3.png" alt="Cover for Earthsongs" style="max-width:120px; height:auto;" />
      </td>
      <td>2004</td>
      <td>Secret Garden</td>
      <td>Earthsongs</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>93</td>
      <td>
          <img src="https://i.discogs.com/puEwOPZMO94dFWs8vn0BiG9MEPBkN7SdEGPHhpxEMNA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTY5NjY0/NDMtMTQzMDU4NjUz/My02NzkwLmpwZWc.jpeg" alt="Cover for May It Be" style="max-width:120px; height:auto;" />
      </td>
      <td>2004</td>
      <td>Enya</td>
      <td>May It Be</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>50</td>
      <td>
      </td>
      <td>2004-05-11</td>
      <td>Karunesh</td>
      <td>Call of the Mystic</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 62 / 100)"> <a href="{{ "/tags/meditation" | relative_url }}">meditation</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/meditation" | relative_url }}">meditation</a> # 93</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>32</td>
      <td>
      </td>
      <td>2004-09-21</td>
      <td>Lesiëm</td>
      <td>Auracle</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fantasy-new-age" | relative_url }}">fantasy new age</a> # 27</li>
            <li> <a href="{{ "/tags/ethnic-fusion" | relative_url }}">ethnic fusion</a> # 31</li>
            <li> <a href="{{ "/tags/chant" | relative_url }}">chant</a> # 43</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>13</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a00e3c4075d4427699299f718824a0ff.png" alt="Cover for Evermind" style="max-width:120px; height:auto;" />
      </td>
      <td>2004-10-05</td>
      <td>Amethystium</td>
      <td>Evermind</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>68</td>
      <td>
          <img src="https://i.discogs.com/cxBHEk3nW-wsSFVB5CtZmEsG_vBdOJDOKF25yBPk3ms/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTc0NjU4/MzAtMTU3Mzc1NjE0/Ny02NTE0LmpwZWc.jpeg" alt="Cover for Very Best Of" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>Era</td>
      <td>Very Best Of</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/chant" | relative_url }}">chant</a> # 29</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>33</td>
      <td>
      </td>
      <td>2005-07-18</td>
      <td>Above &amp; Beyond vs. Andy Moor</td>
      <td>Air For Life</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/trance" | relative_url }}">trance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/vocal-trance" | relative_url }}">vocal trance</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/vocal-trance" | relative_url }}">vocal trance</a> # 89</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>25</td>
      <td>
          <img src="https://i.discogs.com/iafusDcwYIACbU8cjgn2CxxPz9p0Pwkqk3ftfUC0fEM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwMjY2/OTAtMTYyNjQ4MDU1/NC00OTY1LmpwZWc.jpeg" alt="Cover for Nine Worlds" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-08-02</td>
      <td>Achillea</td>
      <td>Nine Worlds</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>91</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/68cb7bd9a5634902a65bcd1c947464db.png" alt="Cover for Amarantine" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-11-18</td>
      <td>Enya</td>
      <td>Amarantine</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 94 / 100)"> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 97</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>4</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a1f939138aa9c542c185b6f99c005fd6.png" alt="Cover for FaerieLore: Journey to the Faerie Ring" style="max-width:120px; height:auto;" />
      </td>
      <td>2006</td>
      <td>Llewellyn</td>
      <td>FaerieLore: Journey to the Faerie Ring</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chillout" | relative_url }}">chillout</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>66</td>
      <td>
          <img src="https://i.discogs.com/pwy7OQvJc_TfZHJS_I4CxnvrC4GrXg28GGfyvLz9wio/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTMyMTMx/MTUtMTMyMTA1MTk2/OS5qcGVn.jpeg" alt="Cover for The Water Is Wide" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-01-10</td>
      <td>Órla Fallon</td>
      <td>The Water Is Wide</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>8</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2d8f317e332e5d511782c695b225d3fe.png" alt="Cover for Breathe" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-01-31</td>
      <td>Blue Stone</td>
      <td>Breathe</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 51 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/chill-new-age" | relative_url }}">chill new age</a> # 6</li>
            <li> <a href="{{ "/tags/fantasy-new-age" | relative_url }}">fantasy new age</a> # 52</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>19</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/54c3d330455d8ee16d49f331b96e2a2d.png" alt="Cover for A Posteriori" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-09-16</td>
      <td>Enigma</td>
      <td>A Posteriori</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>24</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d491460d74dc489ebb0dd02a8509847e.png" alt="Cover for Emblem (Selected Pieces)" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-10-10</td>
      <td>Amethystium</td>
      <td>Emblem (Selected Pieces)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 77 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>6</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d6cd4a9f13f643f190c35f4c7e45b2e9.png" alt="Cover for Heart of Winter" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-12-01</td>
      <td>Frozen Silence</td>
      <td>Heart of Winter</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/piano" | relative_url }}">piano</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>62</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/95880b72ba54466aa96069bcd8f9b134.png" alt="Cover for Amadas Estrellas" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-02-13</td>
      <td>Achillea</td>
      <td>Amadas Estrellas</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/new-age-instrumental" | relative_url }}">new age instrumental</a> # 1</li>
            <li> <a href="{{ "/tags/ethnic-fusion" | relative_url }}">ethnic fusion</a> # 36</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>34</td>
      <td>
          <img src="https://i.discogs.com/voJekx7Jwd-dBLtz6UXRf4HBbHeIQ8pJE46isKpd0Ns/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI0ODg5/Mi0xMDgzNDgxMzI4/LmpwZw.jpeg" alt="Cover for ICO〜霧の中の旋律〜" style="max-width:120px; height:auto;" />
      </td>
      <td>2008</td>
      <td>ICO</td>
      <td>ICO〜霧の中の旋律〜</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/soundtrack" | relative_url }}">soundtrack</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>38</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d8efbf31678522e09a586376254e80d7.png" alt="Cover for Isabliss" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-06-17</td>
      <td>Amethystium</td>
      <td>Isabliss</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>41</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2271553ec494e77603a5b87740841f12.png" alt="Cover for Seven Lives Many Faces" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-08-30</td>
      <td>Enigma</td>
      <td>Seven Lives Many Faces</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>57</td>
      <td>
      </td>
      <td>2008-11-07</td>
      <td>Enya</td>
      <td>And Winter Came</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/celtic" | relative_url }}">celtic</a> # 79</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>14</td>
      <td>
          <img src="https://i.discogs.com/HhxHWRfH537lmUcTkEOsyJvEN8Ev1XZtp_LMP2h7vHE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI3MzMx/NjUtMTMwMDMzMDMz/MC5qcGVn.jpeg" alt="Cover for Liquid Mind IX: Lullaby" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-03-02</td>
      <td>Liquid Mind</td>
      <td>Liquid Mind IX: Lullaby</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>26</td>
      <td>
      </td>
      <td>2010-02-02</td>
      <td>E.S. Posthumus</td>
      <td>Makara</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/choral" | relative_url }}">choral</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/orchestral" | relative_url }}">orchestral</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/orchestral" | relative_url }}">orchestral</a> # 81</li>
            <li> <a href="{{ "/tags/choral" | relative_url }}">choral</a> # 99</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>99</td>
      <td>
          <img src="https://i.discogs.com/gkfPrRmhsdIKhGbDSnjGu60ODzk2ec6hPJy_AlrBiRg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTU5MjU3/ODktMTQwNjQ5MDM5/My03MDk2LmpwZWc.jpeg" alt="Cover for Beggin For Thread" style="max-width:120px; height:auto;" />
      </td>
      <td>2014</td>
      <td>Banks</td>
      <td>Beggin For Thread</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/russian" | relative_url }}">russian</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a> # 66</li>
            <li> <a href="{{ "/tags/russian" | relative_url }}">russian</a> # 100</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>40</td>
      <td>
      </td>
      <td>2014-12-01</td>
      <td>Marissa Nadler</td>
      <td>Before July: Demos &amp; Unreleased Songs</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/brutal-death-metal" | relative_url }}">brutal death metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chillout" | relative_url }}">chillout</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/comedy" | relative_url }}">comedy</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/easy-listening" | relative_url }}">easy listening</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/humour" | relative_url }}">humour</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/j-pop" | relative_url }}">j-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/j-rock" | relative_url }}">j-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/parody" | relative_url }}">parody</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rechtsrock" | relative_url }}">rechtsrock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/true-metal" | relative_url }}">true metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/visual-kei" | relative_url }}">visual kei</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/j-rock" | relative_url }}">j-rock</a> # 45</li>
            <li> <a href="{{ "/tags/comedy" | relative_url }}">comedy</a> # 58</li>
            <li> <a href="{{ "/tags/easy-listening" | relative_url }}">easy listening</a> # 64</li>
            <li> <a href="{{ "/tags/brutal-death-metal" | relative_url }}">brutal death metal</a> # 74</li>
            <li> <a href="{{ "/tags/visual-kei" | relative_url }}">visual kei</a> # 85</li>
            <li> <a href="{{ "/tags/humour" | relative_url }}">humour</a> # 95</li>
            <li> <a href="{{ "/tags/true-metal" | relative_url }}">true metal</a> # 98</li>
            <li> <a href="{{ "/tags/parody" | relative_url }}">parody</a> # 99</li>
            <li> <a href="{{ "/tags/rechtsrock" | relative_url }}">rechtsrock</a> # 100</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>9</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/137c1c23e40307349d85afc4ed61ff7b.png" alt="Cover for Keyboard Fantasies" style="max-width:120px; height:auto;" />
      </td>
      <td>2016</td>
      <td>Beverly Glenn-Copeland</td>
      <td>Keyboard Fantasies</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folk" | relative_url }}">folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>59</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a05726091663e1457cdba9ec4b6d07cc.png" alt="Cover for EARS" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-04-01</td>
      <td>Kaitlyn Aurelia Smith</td>
      <td>EARS</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-electronic" | relative_url }}">progressive electronic</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/progressive-electronic" | relative_url }}">progressive electronic</a> # 96</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>43</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2ced52d83eb0a172a5b61d7a4b2ffa4a.png" alt="Cover for カミイロアワセ" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-08-03</td>
      <td>binaria</td>
      <td>カミイロアワセ</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient-pop" | relative_url }}">ambient pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chillout" | relative_url }}">chillout</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dream-pop" | relative_url }}">dream pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/singer-songwriter" | relative_url }}">singer-songwriter</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/trip-hop" | relative_url }}">trip-hop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ambient-pop" | relative_url }}">ambient pop</a> # 70</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>29</td>
      <td>
          <img src="https://i.discogs.com/ozvVeKR28DoVsvf3vaHspLEEhB-6eIaqA5w3F2DrCdM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1MDUx/OTE0LTE1ODYwMjk2/MTEtNzg5MC5qcGVn.jpeg" alt="Cover for ~~~" style="max-width:120px; height:auto;" />
      </td>
      <td>2019</td>
      <td>ana roxanne</td>
      <td>~~~</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a> # 44</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>2</td>
      <td>
      </td>
      <td>2020</td>
      <td>Yann Tiersen</td>
      <td>The Lighthouse</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>100</td>
      <td>
          <img src="https://i.discogs.com/aHxYpn2uti2Cl76eODK9OM7g6cuVVkLi-bP_w3YwHqk/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1OTM4/MDI3LTE2MDA1NDM0/NTUtMTgzNC5qcGVn.jpeg" alt="Cover for Pollen" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-09-17</td>
      <td>ECCO2k</td>
      <td>Pollen</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient-pop" | relative_url }}">ambient pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-pop" | relative_url }}">art pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ballad" | relative_url }}">ballad</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/baroque-pop" | relative_url }}">baroque pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ethereal" | relative_url }}">ethereal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/art-pop" | relative_url }}">art pop</a> # 85</li>
            <li> <a href="{{ "/tags/ethereal" | relative_url }}">ethereal</a> # 92</li>
            <li> <a href="{{ "/tags/baroque-pop" | relative_url }}">baroque pop</a> # 97</li>
            <li> <a href="{{ "/tags/ballad" | relative_url }}">ballad</a> # 98</li>
            <li> <a href="{{ "/tags/ambient-pop" | relative_url }}">ambient pop</a> # 99</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>54</td>
      <td>
          <img src="https://i.discogs.com/7mjjGuyIsoEJN5H8CC-gWdf-WWoTEGzKnPNENIWEVcA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE2MjQ3/NzczLTE2MDU5NTM2/OTAtMTQ3OC5qcGVn.jpeg" alt="Cover for Vampire Fitness" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-11-13</td>
      <td>KATYA</td>
      <td>Vampire Fitness</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/bubblegum-bass" | relative_url }}">bubblegum bass</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/deconstructed-club" | relative_url }}">deconstructed club</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ebm" | relative_url }}">ebm</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/future-house" | relative_url }}">future house</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/industrial" | relative_url }}">industrial</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise" | relative_url }}">noise</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/spoken-word" | relative_url }}">spoken word</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/industrial" | relative_url }}">industrial</a> # 25</li>
            <li> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a> # 35</li>
            <li> <a href="{{ "/tags/bubblegum-bass" | relative_url }}">bubblegum bass</a> # 50</li>
            <li> <a href="{{ "/tags/ebm" | relative_url }}">ebm</a> # 87</li>
            <li> <a href="{{ "/tags/noise" | relative_url }}">noise</a> # 88</li>
            <li> <a href="{{ "/tags/deconstructed-club" | relative_url }}">deconstructed club</a> # 90</li>
            <li> <a href="{{ "/tags/spoken-word" | relative_url }}">spoken word</a> # 91</li>
            <li> <a href="{{ "/tags/future-house" | relative_url }}">future house</a> # 99</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>84</td>
      <td>
          <img src="https://i.discogs.com/qO-3lyfJNsw076-kYJbQikLKycXEez9rVTaV_uupwo8/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIwODI1/NzcwLTE2NTMwNTE0/NjgtODYyNi5qcGVn.jpeg" alt="Cover for trees etc." style="max-width:120px; height:auto;" />
      </td>
      <td>2021</td>
      <td>Naran Ratan</td>
      <td>trees etc.</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>36</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c4068082d13aef2f86ae1a4572005b8f.png" alt="Cover for Längtan Får Vingar" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-07-02</td>
      <td>VilloVilduVeta</td>
      <td>Längtan Får Vingar</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folktronica" | relative_url }}">folktronica</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/folktronica" | relative_url }}">folktronica</a> # 76</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>52</td>
      <td>
          <img src="https://i.discogs.com/fmork5iqrE8J5qOvfvojru6-lozl6PArgoU2gHAOPic/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIxMDc3/ODUxLTE2Mzc1NzYz/MDMtNDc0Ny5qcGVn.jpeg" alt="Cover for The Bridge Of Khazad-Dum (from &quot;The Lord Of The Rings&quot;)" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-10-15</td>
      <td>Eurielle</td>
      <td>The Bridge Of Khazad-Dum (from &quot;The Lord Of The Rings&quot;)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/asian" | relative_url }}">asian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/breakcore" | relative_url }}">breakcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/british" | relative_url }}">british</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/classic-rock" | relative_url }}">classic rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/classical" | relative_url }}">classical</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/comedy" | relative_url }}">comedy</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/death-metal" | relative_url }}">death metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/easy-listening" | relative_url }}">easy listening</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronica" | relative_url }}">electronica</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emocore" | relative_url }}">emocore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/funk" | relative_url }}">funk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/gothic-metal" | relative_url }}">gothic metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/gothic-rock" | relative_url }}">gothic rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/heavy-metal" | relative_url }}">heavy metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/house" | relative_url }}">house</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/humour" | relative_url }}">humour</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/j-pop" | relative_url }}">j-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/j-rock" | relative_url }}">j-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/japanese" | relative_url }}">japanese</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise" | relative_url }}">noise</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/parody" | relative_url }}">parody</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/polish" | relative_url }}">polish</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/power-metal" | relative_url }}">power metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rnb" | relative_url }}">rnb</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/soul" | relative_url }}">soul</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/spanish" | relative_url }}">spanish</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/symphonic-metal" | relative_url }}">symphonic metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/techno" | relative_url }}">techno</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/trap" | relative_url }}">trap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/japanese" | relative_url }}">japanese</a> # 1</li>
            <li> <a href="{{ "/tags/symphonic-metal" | relative_url }}">symphonic metal</a> # 3</li>
            <li> <a href="{{ "/tags/j-pop" | relative_url }}">j-pop</a> # 11</li>
            <li> <a href="{{ "/tags/techno" | relative_url }}">techno</a> # 39</li>
            <li> <a href="{{ "/tags/classical" | relative_url }}">classical</a> # 40</li>
            <li> <a href="{{ "/tags/gothic-rock" | relative_url }}">gothic rock</a> # 51</li>
            <li> <a href="{{ "/tags/asian" | relative_url }}">asian</a> # 62</li>
            <li> <a href="{{ "/tags/j-rock" | relative_url }}">j-rock</a> # 66</li>
            <li> <a href="{{ "/tags/comedy" | relative_url }}">comedy</a> # 69</li>
            <li> <a href="{{ "/tags/breakcore" | relative_url }}">breakcore</a> # 70</li>
            <li> <a href="{{ "/tags/emocore" | relative_url }}">emocore</a> # 71</li>
            <li> <a href="{{ "/tags/polish" | relative_url }}">polish</a> # 72</li>
            <li> <a href="{{ "/tags/easy-listening" | relative_url }}">easy listening</a> # 75</li>
            <li> <a href="{{ "/tags/spanish" | relative_url }}">spanish</a> # 78</li>
            <li> <a href="{{ "/tags/noise" | relative_url }}">noise</a> # 85</li>
            <li> <a href="{{ "/tags/humour" | relative_url }}">humour</a> # 98</li>
            <li> <a href="{{ "/tags/parody" | relative_url }}">parody</a> # 100</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>96</td>
      <td>
          <img src="https://i.discogs.com/gDMid6TPlvTPsYe8rcJ8fjvLxRwLc9XBvW6D52MFIbg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIyMDgx/MjgyLTE2NDQzNTQw/NjUtMTMzOC5qcGVn.jpeg" alt="Cover for Billions" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-02-15</td>
      <td>Caroline Polachek</td>
      <td>Billions</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>90</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/95ce547b998eaaabab6523b8382cfaff.png" alt="Cover for Cansar de Dançar" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-05-06</td>
      <td>Juliette</td>
      <td>Cansar de Dançar</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/baroque-pop" | relative_url }}">baroque pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folk" | relative_url }}">folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folktronica" | relative_url }}">folktronica</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/baroque-pop" | relative_url }}">baroque pop</a> # 88</li>
            <li> <a href="{{ "/tags/folktronica" | relative_url }}">folktronica</a> # 96</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>51</td>
      <td>
          <img src="https://i.discogs.com/hlya1me140K8dfeQ3RIPQIzqMagfsICpLWZFWsJI3kE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIzNDQ3/ODg4LTE2NTQyMTUx/NjctNTE0Ny5qcGVn.jpeg" alt="Cover for In the Abyss: Music for Weddings" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-05-20</td>
      <td>whitearmor</td>
      <td>In the Abyss: Music for Weddings</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-age" | relative_url }}">new age</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>31</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8a062a16a19944f402c2884dd2ae7247.png" alt="Cover for New Blue Sun" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-11-17</td>
      <td>André 3000</td>
      <td>New Blue Sun</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/spiritual-jazz" | relative_url }}">spiritual jazz</a> # 99</li>
        </ul>
      </td>
    </tr>
  </tbody>
</table>

### Related genres

- [ambient]({{ "/tags/ambient" | relative_url }})
- [electronic]({{ "/tags/electronic" | relative_url }})
- [instrumental]({{ "/tags/instrumental" | relative_url }})
- [chillout]({{ "/tags/chillout" | relative_url }})
- [meditation]({{ "/tags/meditation" | relative_url }})
- [electronica]({{ "/tags/electronica" | relative_url }})
- [piano]({{ "/tags/piano" | relative_url }})
- [world]({{ "/tags/world" | relative_url }})
- [downtempo]({{ "/tags/downtempo" | relative_url }})
- [easy listening]({{ "/tags/easy-listening" | relative_url }})
