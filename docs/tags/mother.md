---
category: genre
date: 2025-12-07T10:32:28.170Z
permalink: tags/mother
title: mother
---

## mother

Last updated: <time datetime="2025-12-07T10:32:28.170Z">Sun Dec 07 2025 12:32:28 GMT+0200 (Eastern European Standard Time)</time>

### Description

&quot;Mother&quot; is a modern pop culture term describing artists who embody confidence, fierce independence, and empowering femininity through their music and persona. The sound spans mainstream pop, hip-hop, and dance genres, characterized by bold production, unapologetic lyrics, and commanding vocal delivery. It celebrates performers who inspire devotion through their charisma, style, and ability to serve iconic, trend-setting moments.

### 100 chronological releases

<table>
  <thead>
    <tr>
      <th>Place</th>
      <th>Cover</th>
      <th>Date</th>
      <th>Artist</th>
      <th>Album</th>
      <th>Tags</th>
      <th>Other places</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>32</td>
      <td>
          <img src="https://i.discogs.com/VF-SIusOeEj4VeX00wQJjRJa8mtyRZ1UGECSSky7EWA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTg2MzM1/NDUtMTQ2NTU3NDk4/NC0yMjk2LmpwZWc.jpeg" alt="Cover for I Don’t Search I Find (Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>1984-05-25</td>
      <td>Madonna</td>
      <td>I Don’t Search I Find (Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 27</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 41</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>23</td>
      <td>
          <img src="https://i.discogs.com/523MHUKgDEXhUPmmx9VXk76CAmeXSkDuo6JOstsMIug/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTM1MTc1/OTQtMTMzMzgxODk4/MS5qcGVn.jpeg" alt="Cover for Lucky Star (Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>1984-08-25</td>
      <td>Madonna</td>
      <td>Lucky Star (Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 20</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 31</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>5</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/68ff9924c5f15930f1ccf303defba622.png" alt="Cover for Material Girl (2024 Remaster)" style="max-width:120px; height:auto;" />
      </td>
      <td>1985-03-10</td>
      <td>Madonna</td>
      <td>Material Girl (2024 Remaster)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 2</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>96</td>
      <td>
          <img src="https://i.discogs.com/z2PpDoEeP16Z1Mzvmklwp3iJG81hHp0bRCYIDyWIMDI/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEyNzA5/NTgtMTYwMDc3MjI2/NS04NzQwLmpwZWc.jpeg" alt="Cover for Material Girl (2024 Remaster) - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>1985-03-10</td>
      <td>Madonna</td>
      <td>Material Girl (2024 Remaster) - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 87</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>77</td>
      <td>
          <img src="https://i.discogs.com/CdooLZ3e4ccymwyIX4dD4Ivz0uIxelA6f8JzSopPWzQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzOTA1/NzI3LTE1NjM3NTY1/MDgtNTc4OC5qcGVn.jpeg" alt="Cover for Nothing Fails (The Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>1985-10-21</td>
      <td>Madonna</td>
      <td>Nothing Fails (The Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/choir" | relative_url }}">choir</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/choir" | relative_url }}">choir</a> # 65</li>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 68</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>43</td>
      <td>
          <img src="https://i.discogs.com/lzZkyerqTbirtHXyKnB35CZpqfdhnw8wvammKUhVUWQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI1Nzg2/NTUtMTUxODM0MTgz/MS01NDEwLmpwZWc.jpeg" alt="Cover for Love Don&#39;t Live Here Anymore (Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>1986-03-10</td>
      <td>Madonna</td>
      <td>Love Don&#39;t Live Here Anymore (Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ballad" | relative_url }}">ballad</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 36</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>81</td>
      <td>
          <img src="https://i.discogs.com/lzZkyerqTbirtHXyKnB35CZpqfdhnw8wvammKUhVUWQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI1Nzg2/NTUtMTUxODM0MTgz/MS01NDEwLmpwZWc.jpeg" alt="Cover for Deadpool &amp; Wolverine: Madonna&#39;s &quot;Like a Prayer&quot; EP - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>1986-03-10</td>
      <td>Madonna</td>
      <td>Deadpool &amp; Wolverine: Madonna&#39;s &quot;Like a Prayer&quot; EP - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>30</td>
      <td>
          <img src="https://i.discogs.com/S_5rpx-uWFfL80zW3-czCSzurXNZx4uzuN2GsE9uCag/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTc2NTE3/MC0xNTg2NzcyMzM5/LTQwMzMuanBlZw.jpeg" alt="Cover for Evita (Music from the Motion Picture)" style="max-width:120px; height:auto;" />
      </td>
      <td>1987</td>
      <td>Madonna</td>
      <td>Evita (Music from the Motion Picture)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/musical" | relative_url }}">musical</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 25</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>76</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/eba3c729c139dd5341193a3a4eed4e50.png" alt="Cover for Finally Enough Love: 50 Number Ones (2022 Remasters)" style="max-width:120px; height:auto;" />
      </td>
      <td>1987</td>
      <td>Madonna</td>
      <td>Finally Enough Love: 50 Number Ones (2022 Remasters)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/american" | relative_url }}">american</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 67</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>88</td>
      <td>
          <img src="https://i.discogs.com/tqQTnnlQTrExDQmKMmfr4iXwEtg0EUlEQgKZlOtLNdo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTYwNjE2/NzgtMTUyNDM5ODg5/MS0xOTY5LmpwZWc.jpeg" alt="Cover for What It Feels Like For A Girl (Remixes) - EP" style="max-width:120px; height:auto;" />
      </td>
      <td>1987</td>
      <td>Madonna</td>
      <td>What It Feels Like For A Girl (Remixes) - EP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 78</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>9</td>
      <td>
          <img src="https://i.discogs.com/DtbrdE7zio4hA6YssQUjVCPxwYavcu7_wh3J54LpUmo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE4MzY5/NDctMTI0NjgwMjMy/MS5qcGVn.jpeg" alt="Cover for Express Yourself" style="max-width:120px; height:auto;" />
      </td>
      <td>1989-05-09</td>
      <td>Madonna</td>
      <td>Express Yourself</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 6</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>21</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/106fbc0b83ea3aea2fa9fc6898d3e31c.png" alt="Cover for Cherish" style="max-width:120px; height:auto;" />
      </td>
      <td>1989-09-04</td>
      <td>Madonna</td>
      <td>Cherish</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 18</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>40</td>
      <td>
      </td>
      <td>1990-01-30</td>
      <td>Madonna</td>
      <td>Keep It Together</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 33</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>87</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/44dbcb0108005c6391cc464c7ff14b76.png" alt="Cover for Vogue" style="max-width:120px; height:auto;" />
      </td>
      <td>1990-03-20</td>
      <td>Madonna</td>
      <td>Vogue</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/disco" | relative_url }}">disco</a></li>
            <li style="font-size: calc(1rem * 74 / 100)"> <a href="{{ "/tags/house" | relative_url }}">house</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/disco" | relative_url }}">disco</a> # 46</li>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 55</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>6</td>
      <td>
          <img src="https://i.discogs.com/_ysI7jGzVOR7xPh6Com3H93w0tvoRdw0Q4NkHRjqFRg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1NzAw/MzgtMTQxMDc5MjMy/NC00NjQ3LmpwZWc.jpeg" alt="Cover for Madonna (reissue)" style="max-width:120px; height:auto;" />
      </td>
      <td>1991</td>
      <td>Madonna</td>
      <td>Madonna (reissue)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 3</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>8</td>
      <td>
          <img src="https://i.discogs.com/JdszE7-ml6nJ7ha3gA8FR_5arXMYG1DurCLeo8etbG4/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQxNjc0/OTktMTM3NjA3MzA2/OC00Nzk4LmpwZWc.jpeg" alt="Cover for Hung Up on Tokischa" style="max-width:120px; height:auto;" />
      </td>
      <td>1991</td>
      <td>Madonna</td>
      <td>Hung Up on Tokischa</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 5</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>26</td>
      <td>
          <img src="https://i.discogs.com/bkE4XZ_uQCrUBcfDM5NjmRdq2Rc7CJBHyNOOOCqPDck/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTM1Nzc3/OC0xNjE2NzU0MDU1/LTQ5MTcuanBlZw.jpeg" alt="Cover for Hollywood (Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>1991</td>
      <td>Madonna</td>
      <td>Hollywood (Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hollywood" | relative_url }}">hollywood</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/hollywood" | relative_url }}">hollywood</a> # 7</li>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 23</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 35</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>46</td>
      <td>
          <img src="https://i.discogs.com/4Pq-GnyyLv_gKbv3ZBTQIDgHOcLKxrV01porQwFQO7I/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE2NzYw/MTA2LTE2MDk2Njky/ODktNzgxNS5qcGVn.jpeg" alt="Cover for I Rise (Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>1991</td>
      <td>Madonna</td>
      <td>I Rise (Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 38</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 62</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>53</td>
      <td>
          <img src="https://i.discogs.com/CdooLZ3e4ccymwyIX4dD4Ivz0uIxelA6f8JzSopPWzQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzOTA1/NzI3LTE1NjM3NTY1/MDgtNTc4OC5qcGVn.jpeg" alt="Cover for Miles Away - The Remixes" style="max-width:120px; height:auto;" />
      </td>
      <td>1991</td>
      <td>Madonna</td>
      <td>Miles Away - The Remixes</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 44</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 75</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>54</td>
      <td>
          <img src="https://i.discogs.com/Oiqa2LrUfKz-3LrkAoFHD3pGDkZ3iIZGboP8harQtIY/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQ1MTU3/OTUtMTUzMjU0MjAx/OC0yNTA4LmpwZWc.jpeg" alt="Cover for Music (remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>1991</td>
      <td>Madonna</td>
      <td>Music (remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 45</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 82</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>58</td>
      <td>
          <img src="https://i.discogs.com/_ysI7jGzVOR7xPh6Com3H93w0tvoRdw0Q4NkHRjqFRg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1NzAw/MzgtMTQxMDc5MjMy/NC00NjQ3LmpwZWc.jpeg" alt="Cover for Gambler - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>1991</td>
      <td>Madonna</td>
      <td>Gambler - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 49</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>60</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3b24729931b8cd0b7cf3edf8abdab408.png" alt="Cover for The Power Of Good-Bye (Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>1992</td>
      <td>Madonna</td>
      <td>The Power Of Good-Bye (Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 51</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>75</td>
      <td>
          <img src="https://i.discogs.com/f4UVlJrHypagFSTBMJAZBny66hiBS7sCQRFhaw6JOQQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwNDM0/MjU0LTE0OTczNTg5/MDYtNDg2NS5qcGVn.jpeg" alt="Cover for Sorry [DJ Version] (DMD Maxi)" style="max-width:120px; height:auto;" />
      </td>
      <td>1992</td>
      <td>Madonna</td>
      <td>Sorry [DJ Version] (DMD Maxi)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 66</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>10</td>
      <td>
          <img src="https://i.discogs.com/7EZmAr5HwN5AwA0jc6z48qjRhaycwIBbbiMExNrF-Fw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwMDYw/OTYzLTE1MDQ0ODgz/ODktMTU2MS5qcGVn.jpeg" alt="Cover for Burning Up &#x2F; Physical Attraction" style="max-width:120px; height:auto;" />
      </td>
      <td>1993</td>
      <td>Madonna</td>
      <td>Burning Up &#x2F; Physical Attraction</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 8</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>18</td>
      <td>
          <img src="https://i.discogs.com/CdooLZ3e4ccymwyIX4dD4Ivz0uIxelA6f8JzSopPWzQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzOTA1/NzI3LTE1NjM3NTY1/MDgtNTc4OC5qcGVn.jpeg" alt="Cover for Justify My Love (Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>1993</td>
      <td>Madonna</td>
      <td>Justify My Love (Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 15</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 21</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>29</td>
      <td>
          <img src="https://i.discogs.com/s_Ca5vg9RHP4BVkFHYWUmdeputmBCjOPSO9E__2f9k4/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE2NzAz/NzktMTI0NTYwNDg2/MC5qcGVn.jpeg" alt="Cover for You Can Dance (Single Edits)" style="max-width:120px; height:auto;" />
      </td>
      <td>1995</td>
      <td>Madonna</td>
      <td>You Can Dance (Single Edits)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 24</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>33</td>
      <td>
          <img src="https://i.discogs.com/6D0iGkCPIn0wHxKCFB143eHlKLxEKbr-fjrTbAkOt1A/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTY3NjY4/NTQtMTQyNjE4OTIw/MS03MTQ0LmpwZWc.jpeg" alt="Cover for Celebration (Bonus Track Version)" style="max-width:120px; height:auto;" />
      </td>
      <td>1995</td>
      <td>Madonna</td>
      <td>Celebration (Bonus Track Version)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 28</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>55</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2400a60c00fc43a3bd39800a21e6e158.png" alt="Cover for Give Me All Your Luvin&#39; (Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>1995</td>
      <td>Madonna</td>
      <td>Give Me All Your Luvin&#39; (Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/edm" | relative_url }}">edm</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 46</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 85</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>98</td>
      <td>
      </td>
      <td>2000-09</td>
      <td>Madonna</td>
      <td>Madame X - Music From The Theater Xperience</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/portugal" | relative_url }}">portugal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 88</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>2</td>
      <td>
          <img src="https://i.discogs.com/IXNy_0aLJlvY2wb-NLLrw0LPg3kuK6GeiREMjs4G_Cg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzNzYy/MTI0LTE1NjA1NjU5/MzQtMTcxOC5qcGVn.jpeg" alt="Cover for Back That Up To The Beat" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Madonna</td>
      <td>Back That Up To The Beat</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 1</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>12</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5f9de1384d3617753e6c37a7e1c8fa51.png" alt="Cover for MATERIAL GWORRLLLLLLLL!" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Madonna</td>
      <td>MATERIAL GWORRLLLLLLLL!</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 10</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>14</td>
      <td>
          <img src="https://i.discogs.com/iZ98EEIz6qlBpxBBgSyEX8CIgC_82kO9l7OgxyfNbwA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEyOTc3/MzMyLTE1NDU3Mzc5/NTgtNzYwMS5qcGVn.jpeg" alt="Cover for Rebel Heart Tour (Live)" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Madonna</td>
      <td>Rebel Heart Tour (Live)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 11</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>35</td>
      <td>
          <img src="https://i.discogs.com/VF-SIusOeEj4VeX00wQJjRJa8mtyRZ1UGECSSky7EWA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTg2MzM1/NDUtMTQ2NTU3NDk4/NC0yMjk2LmpwZWc.jpeg" alt="Cover for Bitch i&#39;m Madonna (The Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Madonna</td>
      <td>Bitch i&#39;m Madonna (The Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 29</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>56</td>
      <td>
          <img src="https://i.discogs.com/Q-sWL9UL-dy-HFGT97baOsTGSBS54lcK5B13h6dlJdg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTY2NDMy/MTctMTQyMzc5NzU0/MS0yODk5LmpwZWc.jpeg" alt="Cover for You&#39;ll See (The Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Madonna</td>
      <td>You&#39;ll See (The Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ballad" | relative_url }}">ballad</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 47</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 87</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>61</td>
      <td>
          <img src="https://i.discogs.com/7so_fYlVNj0zFB-lkKOaY8cWagutZguSEoA9cSrhSfE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI2Mzc2/MzMtMTQwMjk5MTY2/Mi02NTU2LmpwZWc.jpeg" alt="Cover for Like a Virgin (2024 Remaster)" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Madonna</td>
      <td>Like a Virgin (2024 Remaster)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 52</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>72</td>
      <td>
          <img src="https://i.discogs.com/rjoqGN0nx3VYXkNcVLlrWJmSzI8P6NMJdhT-P2PYqEI/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTcwMjMy/Mi0xNjA0MDYyNTc2/LTI3ODcuanBlZw.jpeg" alt="Cover for Who&#39;s That Girl (Original Motion Picture Soundtrack)" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Madonna</td>
      <td>Who&#39;s That Girl (Original Motion Picture Soundtrack)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 62</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>73</td>
      <td>
          <img src="https://i.discogs.com/fZV-0GsfXANi95r-O7D0tMMDmTwMr3IM0gmx_mjw--o/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE0MDEw/NTgxLTE1NjYwMzQy/MTgtMzIwOC5qcGVn.jpeg" alt="Cover for Crave (Remixes Pt. 2)" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Madonna</td>
      <td>Crave (Remixes Pt. 2)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 63</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>91</td>
      <td>
          <img src="https://i.discogs.com/fZOd5aO8yc4b-ejSWHhd08-vLEXUXowWXW4fNcx3iDI/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTc4NDA4/MzEtMTQ0OTkzNDg3/MS02NDE1LmpwZWc.jpeg" alt="Cover for Celebration (feat. Akon)" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Madonna</td>
      <td>Celebration (feat. Akon)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 80</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>89</td>
      <td>
      </td>
      <td>2001-11-09</td>
      <td>Madonna</td>
      <td>GHV2: Greatest Hits, Volume 2</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 79</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>36</td>
      <td>
      </td>
      <td>2003</td>
      <td>Violeta</td>
      <td>el x venir</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>45</td>
      <td>
      </td>
      <td>2003</td>
      <td>Violeta</td>
      <td>LIBERTÁ</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>39</td>
      <td>
          <img src="https://i.discogs.com/7RPN2VcZRiJRJeWx6M5-IOFIr2GmWk44fVkaa_aRGr0/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwODc3/NTMzLTE1MTk0NTEy/NTMtNDM5OS5qcGVn.jpeg" alt="Cover for Deeper And Deeper (David&#39;s Radio Edit) [2022 Remaster]" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>Madonna</td>
      <td>Deeper And Deeper (David&#39;s Radio Edit) [2022 Remaster]</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 32</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>64</td>
      <td>
          <img src="https://i.discogs.com/ssttpLNTVE7naOK_AZDqHiLb0-5VtNfbdXrfuZ2GuNk/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwNzkw/MDI1LTE1MDQzNjQz/NTMtODAzOC5qcGVn.jpeg" alt="Cover for Deadpool &amp; Wolverine: Madonna&#39;s &quot;Like a Prayer&quot; - EP" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>Madonna</td>
      <td>Deadpool &amp; Wolverine: Madonna&#39;s &quot;Like a Prayer&quot; - EP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 54</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>42</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/adebb5af9c6447a3bcd1ac9fbcceb17f.png" alt="Cover for Confessions On A Dance Floor (Non-Stop Mix)" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-11-11</td>
      <td>Madonna</td>
      <td>Confessions On A Dance Floor (Non-Stop Mix)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/disco" | relative_url }}">disco</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 35</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>11</td>
      <td>
          <img src="https://i.discogs.com/aAUpj3gUmR4qoM-lZNy68t7vHjggioiwAe6iBJW1wLI/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1ODgw/NzAtMTUyODgzNDk5/OS03NjEwLmpwZWc.jpeg" alt="Cover for Evita: The Complete Motion Picture Music Soundtrack" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-01</td>
      <td>Madonna</td>
      <td>Evita: The Complete Motion Picture Music Soundtrack</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/musical" | relative_url }}">musical</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 9</li>
            <li> <a href="{{ "/tags/musical" | relative_url }}">musical</a> # 94</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>41</td>
      <td>
      </td>
      <td>2007-01</td>
      <td>Madonna</td>
      <td>The Confessions Tour (Int&#39;l Only DMD)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 34</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>37</td>
      <td>
      </td>
      <td>2007-05-16</td>
      <td>Madonna</td>
      <td>Hey You</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 30</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>13</td>
      <td>
      </td>
      <td>2007-07-05</td>
      <td>Nicki Minaj</td>
      <td>Last Time I Saw You - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>90</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c6c9c96d43b34c36aae8879ffc69bb6f.png" alt="Cover for Treehouse &amp; Mother Tongue (Robjn Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>2008</td>
      <td>Canon Blue</td>
      <td>Treehouse &amp; Mother Tongue (Robjn Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electro" | relative_url }}">electro</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/nashville" | relative_url }}">nashville</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/usa" | relative_url }}">usa</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/nashville" | relative_url }}">nashville</a> # 24</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>93</td>
      <td>
          <img src="https://i.discogs.com/mTK1YAZh9J5YXHZ9m8dOmq5mM2p90dhM54Dqqlnxid0/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE2OTI2/NjU3LTE2MTA2NDU4/MjEtOTA2MS5qcGVn.jpeg" alt="Cover for Gaga Daily" style="max-width:120px; height:auto;" />
      </td>
      <td>2008</td>
      <td>Lady Gaga</td>
      <td>Gaga Daily</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>15</td>
      <td>
          <img src="https://i.discogs.com/EcdQR_0NZdKbHZK-xHg5YmWtAerUg5GhjiEcZVKKt0c/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIxNTM0/MjAtMTQxOTk5OTg2/Mi00NTA3LmpwZWc.jpeg" alt="Cover for Like A Prayer (30th Anniversary)" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Madonna</td>
      <td>Like A Prayer (30th Anniversary)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 12</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>19</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3714e9479fe6d40d79946d3017231126.png" alt="Cover for Finally Enough Love: 50 Number Ones" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Madonna</td>
      <td>Finally Enough Love: 50 Number Ones</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/american" | relative_url }}">american</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 16</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 22</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>24</td>
      <td>
          <img src="https://i.discogs.com/YKJG_xByoJ7BMhE44bytSVhR--UqtWQdN3Z-7NaLpho/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTcxMTg1/NTktMTQzNDEyNjg5/OC02MzU5LmpwZWc.jpeg" alt="Cover for Ray of Light (Single Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Madonna</td>
      <td>Ray of Light (Single Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 21</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>31</td>
      <td>
          <img src="https://i.discogs.com/AOenbdA5fIJh_dDrwT4V5A_Q1qJcemiMq9hkNPmuBk0/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIyNDIz/MzE1LTE2NDY2Nzcz/NDQtNzc3Ny5qcGVn.jpeg" alt="Cover for Back That Up To The Beat - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Madonna</td>
      <td>Back That Up To The Beat - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 26</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>38</td>
      <td>
          <img src="https://i.discogs.com/CdooLZ3e4ccymwyIX4dD4Ivz0uIxelA6f8JzSopPWzQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzOTA1/NzI3LTE1NjM3NTY1/MDgtNTc4OC5qcGVn.jpeg" alt="Cover for American Life (The Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Madonna</td>
      <td>American Life (The Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 31</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 52</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>48</td>
      <td>
          <img src="https://i.discogs.com/Us1vTJDl7SED3HWZLmOmhRR6Lfo237W04avWUaMG0h8/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQ1MDYx/MjctMTU4ODg1MTQ4/OS0yOTY0LmpwZWc.jpeg" alt="Cover for Crave (Remixes Pt. 1)" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Madonna</td>
      <td>Crave (Remixes Pt. 1)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 39</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 68</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>51</td>
      <td>
          <img src="https://i.discogs.com/DsiHcNAWBq3oIkTt5PGC3q-E6vEqskLOgmKGC3dN6fs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTcwODk3/NDMtMTYwMzUyMjI0/My00MzUzLmpwZWc.jpeg" alt="Cover for Drowned World &#x2F; Substitute For Love (Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Madonna</td>
      <td>Drowned World &#x2F; Substitute For Love (Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 41</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>52</td>
      <td>
      </td>
      <td>2009</td>
      <td>Madonna</td>
      <td>Music From The Motion Picture &quot;Evita&quot;</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/musical" | relative_url }}">musical</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 42</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>57</td>
      <td>
          <img src="https://i.discogs.com/CdooLZ3e4ccymwyIX4dD4Ivz0uIxelA6f8JzSopPWzQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzOTA1/NzI3LTE1NjM3NTY1/MDgtNTc4OC5qcGVn.jpeg" alt="Cover for Give It 2 Me - The Remixes" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Madonna</td>
      <td>Give It 2 Me - The Remixes</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 48</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 88</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>49</td>
      <td>
          <img src="https://i.discogs.com/CdooLZ3e4ccymwyIX4dD4Ivz0uIxelA6f8JzSopPWzQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzOTA1/NzI3LTE1NjM3NTY1/MDgtNTc4OC5qcGVn.jpeg" alt="Cover for Don&#39;t Tell Me (The Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>2010</td>
      <td>Madonna</td>
      <td>Don&#39;t Tell Me (The Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 40</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>68</td>
      <td>
          <img src="https://i.discogs.com/rAwdTskVHnhY_7Ywk0dfDkLSbju9ZKPgupSZ455UQpg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI3NDEw/NzQtMTI5ODkxODgx/Mi5qcGVn.jpeg" alt="Cover for Love Profusion (The Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>2010</td>
      <td>Madonna</td>
      <td>Love Profusion (The Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>4</td>
      <td>
      </td>
      <td>2011-09-20</td>
      <td>Soojin</td>
      <td>AGASSY</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>100</td>
      <td>
      </td>
      <td>2012-03-26</td>
      <td>Madonna</td>
      <td>The Complete Studio Albums (1983-2008)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 90</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>44</td>
      <td>
          <img src="https://i.discogs.com/PEq5O9UxV7_BAsquXi12Hl-kUYlIVgMw1VVahII6_V4/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQzMTg4/NzgtMTUwNzM1MzUz/OS00MTMzLmpwZWc.jpeg" alt="Cover for Sorry (DJ Version)" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-06-20</td>
      <td>Madonna</td>
      <td>Sorry (DJ Version)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 37</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 61</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>70</td>
      <td>
          <img src="https://i.discogs.com/4XyZhiocdhZ7XZ2c6_dfhFiAWdODH-uO4Ldqy8_a6pQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQzMTg1/OTUtMTM2MTYyMzQ4/OS05NTY3LmpwZWc.jpeg" alt="Cover for Tokyo 1987" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-06-20</td>
      <td>Madonna</td>
      <td>Tokyo 1987</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 60</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>78</td>
      <td>
          <img src="https://i.discogs.com/Wtnledkw-001_ZtIoW4sQp4KHRLMkeSEGjSXOUwB118/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQ5NzM0/MTgtMTM5MDc1MDgy/Ni04OTY4LmpwZWc.jpeg" alt="Cover for Angel - EP" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-06-20</td>
      <td>Madonna</td>
      <td>Angel - EP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 69</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>69</td>
      <td>
      </td>
      <td>2014</td>
      <td>Ева Польна</td>
      <td>Поёт любовь (Deluxe Version)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 59</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>80</td>
      <td>
          <img src="https://i.discogs.com/NgDwbPLOI7HzWT9VafsCUs2DGq2l-VeDoBasqujI1zA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTYyMDcx/NzMtMTQxMzczMjUw/MC0yMTg1LmpwZWc.jpeg" alt="Cover for Mother Fish" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-05-27</td>
      <td>Sophie Villy</td>
      <td>Mother Fish</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/blues" | relative_url }}">blues</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folk" | relative_url }}">folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>17</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c54546807a7e201076012123741b13b4.png" alt="Cover for True Blue (35th Anniversary Edition)" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-03-06</td>
      <td>Madonna</td>
      <td>True Blue (35th Anniversary Edition)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance-pop" | relative_url }}">dance-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 14</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>22</td>
      <td>
          <img src="https://i.discogs.com/P09J0Jcuk7NU6mS1R0ZUZrJiKRa_NwRjlNiyX4Y4mDc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTY3MzU3/OTAtMTQyNTU4MzI3/NS03NTMzLmpwZWc.jpeg" alt="Cover for Rebel Heart (Super Deluxe)" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-03-06</td>
      <td>Madonna</td>
      <td>Rebel Heart (Super Deluxe)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 19</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>65</td>
      <td>
      </td>
      <td>2015-03-06</td>
      <td>Madonna</td>
      <td>Rebel Heart - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>20</td>
      <td>
          <img src="https://i.discogs.com/Q-sWL9UL-dy-HFGT97baOsTGSBS54lcK5B13h6dlJdg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTY2NDMy/MTctMTQyMzc5NzU0/MS0yODk5LmpwZWc.jpeg" alt="Cover for Nothing Really Matters (Remixes)" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-03-18</td>
      <td>Madonna</td>
      <td>Nothing Really Matters (Remixes)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 17</li>
            <li> <a href="{{ "/tags/remixes" | relative_url }}">remixes</a> # 25</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>25</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/65177924170eb62344e723a7b9d41f4e.png" alt="Cover for THE ALCHEMIST" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-03-02</td>
      <td>Qveen Herby</td>
      <td>THE ALCHEMIST</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>16</td>
      <td>
      </td>
      <td>2019</td>
      <td>María Escarmiento</td>
      <td>iCandy</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>66</td>
      <td>
          <img src="https://i.discogs.com/28ZdkxNdpajQ4GHA93oqm-faxO6ewwd8IuuHj0u0H-Q/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE5ODA5/OTA0LTE2Mjg1ODg4/MjgtNzI4MS5qcGVn.jpeg" alt="Cover for MDNA (Nightlife Edition)" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-06-14</td>
      <td>Madonna</td>
      <td>MDNA (Nightlife Edition)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/edm" | relative_url }}">edm</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 57</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>82</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ff636d7f14671d190b26143dd62ebf52.png" alt="Cover for Unreleased" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-03-18</td>
      <td>Addison Rae</td>
      <td>Unreleased</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/trap" | relative_url }}">trap</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>67</td>
      <td>
          <img src="https://i.discogs.com/_SAJHJCaccRAEui8KUg-nUf-5YmrNJ4Jp5QXBORxbO0/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI5MzQ5/OTY0LTE3MDM5NTc2/OTItMzY4MC5qcGVn.jpeg" alt="Cover for The Tortured Poets Department" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-11-12</td>
      <td>Taylor Swift</td>
      <td>The Tortured Poets Department</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/alt-pop" | relative_url }}">alt pop</a> # 36</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>85</td>
      <td>
          <img src="https://i.discogs.com/D8SmHDFNBMixOuNX0Oh396EZuMSVDcErxOZ1eV2Ep3k/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIxMjA2/MjUxLTE2Mzg0ODc2/MTEtNDg4My5qcGVn.jpeg" alt="Cover for Frozen" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-12-03</td>
      <td>Madonna &amp; Sickick</td>
      <td>Frozen</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 76</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>79</td>
      <td>
          <img src="https://i.discogs.com/wfu6z_d91ZumbB-5hq-lya2HkhQ2YcpWcF1UAX0XBgw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIyMzY3/MTQwLTE2NDYyOTQz/ODEtNTQ3My5qcGVn.jpeg" alt="Cover for Frozen (Fireboy DML Remix) - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-03-03</td>
      <td>Madonna &amp; Sickick</td>
      <td>Frozen (Fireboy DML Remix) - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 70</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>99</td>
      <td>
      </td>
      <td>2022-03-03</td>
      <td>Madonna &amp; Sickick</td>
      <td>Frozen (Fireboy DML Remix)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 89</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>83</td>
      <td>
          <img src="https://i.discogs.com/CeuuPXMckjPHWm1kQlUCehaSiST-qrAAqElAd8hXj0I/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIyNzA1/NzY5LTE2NDg3MDEw/NjYtNTM2MS5qcGVn.jpeg" alt="Cover for Frozen (feat. 070 Shake) - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-03-31</td>
      <td>Madonna &amp; Sickick</td>
      <td>Frozen (feat. 070 Shake) - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 73</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>59</td>
      <td>
          <img src="https://i.discogs.com/ljLkKrLtkLGeGw6QvhQqhLRfGRni1K7H2PGniXb4kqs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIzMjg3/NDQyLTE2NTMwMjEx/MzctOTU1OC5qcGVn.jpeg" alt="Cover for Frozen - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-05-19</td>
      <td>Madonna &amp; Sickick</td>
      <td>Frozen - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 50</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>84</td>
      <td>
          <img src="https://i.discogs.com/ljLkKrLtkLGeGw6QvhQqhLRfGRni1K7H2PGniXb4kqs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIzMjg3/NDQyLTE2NTMwMjEx/MzctOTU1OC5qcGVn.jpeg" alt="Cover for Frozen On Fire - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-05-19</td>
      <td>Madonna &amp; Sickick</td>
      <td>Frozen On Fire - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 75</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>95</td>
      <td>
      </td>
      <td>2022-05-19</td>
      <td>Madonna &amp; Sickick</td>
      <td>Frozen On Fire</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 86</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>74</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0a08e7532b6055b6b1c5b6e43e9ac40b.png" alt="Cover for MATERIAL GWORRLLLLLLLL! - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-08-05</td>
      <td>Madonna &amp; Saucy Santana</td>
      <td>MATERIAL GWORRLLLLLLLL! - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 64</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>27</td>
      <td>
          <img src="https://i.discogs.com/cORKMk_wIO9MnR87t93xYEKnBKJLFFMthnjIxbtdbQ8/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI0MTE1/NTE0LTE2NzQ4NDA0/ODYtNTE2NC5qcGVn.jpeg" alt="Cover for BREAK MY SOUL (THE QUEENS REMIX) - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-08-06</td>
      <td>Beyoncé &amp; Madonna</td>
      <td>BREAK MY SOUL (THE QUEENS REMIX) - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/house" | relative_url }}">house</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>62</td>
      <td>
          <img src="https://i.discogs.com/cORKMk_wIO9MnR87t93xYEKnBKJLFFMthnjIxbtdbQ8/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI0MTE1/NTE0LTE2NzQ4NDA0/ODYtNTE2NC5qcGVn.jpeg" alt="Cover for BREAK MY SOUL (THE QUEENS REMIX)" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-08-06</td>
      <td>Beyoncé &amp; Madonna</td>
      <td>BREAK MY SOUL (THE QUEENS REMIX)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/house" | relative_url }}">house</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>63</td>
      <td>
          <img src="https://i.discogs.com/lDlZsuZwzDyzAfhf1y8E5dj-xbTBgYa5gyuuGXeQbYo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI0NTM3/MDUzLTE2NjM0Mjc1/NjYtODUwNS5qcGVn.jpeg" alt="Cover for Hung Up on Tokischa - Single" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-09-16</td>
      <td>Madonna &amp; Tokischa</td>
      <td>Hung Up on Tokischa - Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 53</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>50</td>
      <td>
      </td>
      <td>2023-02-03</td>
      <td>María Escarmiento</td>
      <td>Sensación De Calor 2</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>1</td>
      <td>
      </td>
      <td>2023-06-09</td>
      <td>Sexyy Red</td>
      <td>Hood Hottest Princess</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>47</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c6ae145428bde533606e5cf89912c0fd.png" alt="Cover for AR" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-08-18</td>
      <td>Addison Rae</td>
      <td>AR</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/alt-pop" | relative_url }}">alt pop</a> # 14</li>
            <li> <a href="{{ "/tags/contemporary-rnb" | relative_url }}">contemporary rnb</a> # 57</li>
            <li> <a href="{{ "/tags/dance-pop" | relative_url }}">dance-pop</a> # 67</li>
            <li> <a href="{{ "/tags/teen-pop" | relative_url }}">teen pop</a> # 69</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>3</td>
      <td>
      </td>
      <td>2023-09-23</td>
      <td>(G)I-DLE</td>
      <td>Wife</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>7</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f7e3851640419792a95a2b729a7f860b.png" alt="Cover for HEADSPLIT" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-03-08</td>
      <td>Maggie Lindemann</td>
      <td>HEADSPLIT</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-metal" | relative_url }}">alternative metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-pop" | relative_url }}">alternative pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dark-pop" | relative_url }}">dark pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electropop" | relative_url }}">electropop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/nu-metal" | relative_url }}">nu metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop-punk" | relative_url }}">pop punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/punk-pop" | relative_url }}">punk pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/trap-metal" | relative_url }}">trap metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/punk-pop" | relative_url }}">punk pop</a> # 2</li>
            <li> <a href="{{ "/tags/dark-pop" | relative_url }}">dark pop</a> # 13</li>
            <li> <a href="{{ "/tags/trap-metal" | relative_url }}">trap metal</a> # 21</li>
            <li> <a href="{{ "/tags/hyperpop" | relative_url }}">hyperpop</a> # 67</li>
            <li> <a href="{{ "/tags/alternative-pop" | relative_url }}">alternative pop</a> # 73</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>28</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/787b99ed519694d755514a27a5ce3735.png" alt="Cover for Radical Optimism" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-05-03</td>
      <td>Dua Lipa</td>
      <td>Radical Optimism</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/europop" | relative_url }}">europop</a> # 32</li>
            <li> <a href="{{ "/tags/disco" | relative_url }}">disco</a> # 49</li>
            <li> <a href="{{ "/tags/dance-pop" | relative_url }}">dance-pop</a> # 88</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>34</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3a561e44d8ef4fd967325007a7cfe360.png" alt="Cover for My Everything - Tenth Anniversary Edition" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-08-22</td>
      <td>Ariana Grande</td>
      <td>My Everything - Tenth Anniversary Edition</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>71</td>
      <td>
      </td>
      <td></td>
      <td>Madonna</td>
      <td>Live on air 1984 - 1995 (live)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 61</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>86</td>
      <td>
      </td>
      <td></td>
      <td>Madonna</td>
      <td>GHV2 (Greatest Hits Volume 2)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 77</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>92</td>
      <td>
      </td>
      <td></td>
      <td>Madonna</td>
      <td>GHV2 Greatest Hits Volume 2</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 83</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>94</td>
      <td>
      </td>
      <td></td>
      <td>Evita Soundtrack</td>
      <td>Evita: The Complete Motion Picture Music Soundtrack</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/icon" | relative_url }}">icon</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/musical" | relative_url }}">musical</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/icon" | relative_url }}">icon</a> # 85</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>97</td>
      <td>
      </td>
      <td></td>
      <td>Mhtera Falaina Tyflh</td>
      <td>Plankton EP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folk" | relative_url }}">folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/greek" | relative_url }}">greek</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mother" | relative_url }}">mother</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
  </tbody>
</table>

### Related genres

- [icon]({{ "/tags/icon" | relative_url }})
- [remixes]({{ "/tags/remixes" | relative_url }})
- [pop]({{ "/tags/pop" | relative_url }})
- [dance]({{ "/tags/dance" | relative_url }})
- [electronic]({{ "/tags/electronic" | relative_url }})
- [musical]({{ "/tags/musical" | relative_url }})
- [rock]({{ "/tags/rock" | relative_url }})
- [trap]({{ "/tags/trap" | relative_url }})
- [folk]({{ "/tags/folk" | relative_url }})
- [edm]({{ "/tags/edm" | relative_url }})
