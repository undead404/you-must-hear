---
category: genre
date: 2025-12-06T11:49:41.709Z
permalink: tags/emoviolence
title: emoviolence
---

## emoviolence

Last updated: <time datetime="2025-12-06T11:49:41.709Z">Sat Dec 06 2025 13:49:41 GMT+0200 (Eastern European Standard Time)</time>

### Description

Emoviolence fuses the raw emotional intensity of screamo with the chaotic brutality of powerviolence, creating an aggressive and cathartic sound marked by dissonant guitar work, frantic tempo shifts, and anguished vocals. Emerging from the late 1990s DIY hardcore scene, it channels personal pain and social frustration through brief, explosive songs that prioritize visceral impact over traditional structure. The style emphasizes unfiltered emotion and sonic extremity, often incorporating moments of fragile melody within its overwhelming heaviness.

### 100 chronological releases

<table>
  <thead>
    <tr>
      <th>Place</th>
      <th>Cover</th>
      <th>Date</th>
      <th>Artist</th>
      <th>Album</th>
      <th>Tags</th>
      <th>Other places</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>77</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/52ace115395480e3a0735c18ff8acd7a.png" alt="Cover for Thank God, Me a Superstar, Trapped in This Fucking Hellhole" style="max-width:120px; height:auto;" />
      </td>
      <td>1985</td>
      <td>Threeway</td>
      <td>Thank God, Me a Superstar, Trapped in This Fucking Hellhole</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>88</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ba08510acf3f3dfcf6c75e02a27b21e7.png" alt="Cover for Industry" style="max-width:120px; height:auto;" />
      </td>
      <td>1993</td>
      <td>Honeywell</td>
      <td>Industry</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>39</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/883b7c8903a8ad523de6d3e133b60f7d.png" alt="Cover for Two Hours To Doom" style="max-width:120px; height:auto;" />
      </td>
      <td>1997</td>
      <td>Mörser</td>
      <td>Two Hours To Doom</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/deathcore" | relative_url }}">deathcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>41</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/eda0f214da7d40d6b8c8437b3b5f7b03.png" alt="Cover for Information Battle to Denounce the Genocide" style="max-width:120px; height:auto;" />
      </td>
      <td>1997</td>
      <td>Jenny Piccolo</td>
      <td>Information Battle to Denounce the Genocide</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>35</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2d30976a2e786101ec23608aac5e0b50.png" alt="Cover for In Love With Jetts &#x2F; The Lady Is a Cat" style="max-width:120px; height:auto;" />
      </td>
      <td>1997-08-12</td>
      <td>Antioch Arrow</td>
      <td>In Love With Jetts &#x2F; The Lady Is a Cat</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 89</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>54</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4c82d4c6ee45de9b7e5fb582e7089333.png" alt="Cover for Revolution Summer" style="max-width:120px; height:auto;" />
      </td>
      <td>1998</td>
      <td>Reversal of Man</td>
      <td>Revolution Summer</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 25</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>55</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8d1b908229e82e9202f7d7d12360cdbb.png" alt="Cover for Drome Triler of Puzzle Zoo People" style="max-width:120px; height:auto;" />
      </td>
      <td>1998</td>
      <td>Gasp</td>
      <td>Drome Triler of Puzzle Zoo People</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a> # 78</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>47</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d9a316014f12d57334ded8d7518e118c.png" alt="Cover for Orchid" style="max-width:120px; height:auto;" />
      </td>
      <td>1998</td>
      <td>Orchid</td>
      <td>Orchid</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>1</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c1c93b79d1d248a3ae5d006e10fb3362.png" alt="Cover for Chaos Is Me" style="max-width:120px; height:auto;" />
      </td>
      <td>1999-06-09</td>
      <td>Orchid</td>
      <td>Chaos Is Me</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 65 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a> # 20</li>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 91</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>53</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/416a89c89a7f8301bac8e905856fe9cc.png" alt="Cover for Demo" style="max-width:120px; height:auto;" />
      </td>
      <td>2000</td>
      <td>Tears of Avarel</td>
      <td>Demo</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/mathgrind" | relative_url }}">mathgrind</a> # 77</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>4</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/07a4b643b05a47fbc752b8b29a17def3.png" alt="Cover for Love Lost but Not Forgotten" style="max-width:120px; height:auto;" />
      </td>
      <td>2000</td>
      <td>Love Lost but Not Forgotten</td>
      <td>Love Lost but Not Forgotten</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>12</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/58db8981a1a006dc68461c4179f4fdec.png" alt="Cover for Document #5" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-04-26</td>
      <td>Pg.99</td>
      <td>Document #5</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>19</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/007a2e6f543f0d851b3d573a3ebdeb4c.png" alt="Cover for Seeing Means More Than Safety" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-05</td>
      <td>Jeromes Dream</td>
      <td>Seeing Means More Than Safety</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 33</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>16</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/72ee3377cad50ffd035f199f7b90e92f.png" alt="Cover for Hassan I Sabbah" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-09-03</td>
      <td>Hassan I Sabbah</td>
      <td>Hassan I Sabbah</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 49</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>5</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/41f675f03bb44d7f9d1db4e7fb694d52.png" alt="Cover for Dance Tonight! Revolution Tomorrow!" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-12-04</td>
      <td>Orchid</td>
      <td>Dance Tonight! Revolution Tomorrow!</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a> # 82</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>2</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d5b6b1e6966008b4aa63d8ac6b38dd91.png" alt="Cover for Completed" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Jeromes Dream</td>
      <td>Completed</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a> # 62</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>69</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/15a1cc097bad4f1e883b058f5e5c7fa1.png" alt="Cover for Document #8" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-06</td>
      <td>Pg.99</td>
      <td>Document #8</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>34</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f8c0a1ee79be7fdb4ddbab5166fc7742.png" alt="Cover for A Days Refrain&#x2F;Neil Perry split" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>A Days Refrain</td>
      <td>A Days Refrain&#x2F;Neil Perry split</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a> # 92</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>59</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/435375236cf94c069d3cd733d14efe32.png" alt="Cover for I" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>Bucket Full of Teeth</td>
      <td>I</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>38</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5aff0ae327980f72ddaccd4da64b0639.png" alt="Cover for Disinformation Fix" style="max-width:120px; height:auto;" />
      </td>
      <td>2003</td>
      <td>Usurp Synapse</td>
      <td>Disinformation Fix</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 6</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>71</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/59bf37d1a23dfcd6562f591828c37f28.png" alt="Cover for Discography" style="max-width:120px; height:auto;" />
      </td>
      <td>2003</td>
      <td>Portraits of Past</td>
      <td>Discography</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 7</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>93</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/507823b0bd5895f5fac5b21758f7a0f0.png" alt="Cover for Audio Prozac" style="max-width:120px; height:auto;" />
      </td>
      <td>2003</td>
      <td>Stray From the Path</td>
      <td>Audio Prozac</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>74</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b7a076d65e8835fe80ad9917849cad58.png" alt="Cover for ...And Little Lambs Eat Nations" style="max-width:120px; height:auto;" />
      </td>
      <td>2004-11-30</td>
      <td>Lights for Nero</td>
      <td>...And Little Lambs Eat Nations</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/jazz-rock" | relative_url }}">jazz rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>82</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/019ca6731bf349939a7f3951fc0872f6.png" alt="Cover for I Wrote Haikus About Cannibalism In Your Yearbook" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>I Wrote Haikus About Cannibalism in Your Yearbook</td>
      <td>I Wrote Haikus About Cannibalism In Your Yearbook</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>18</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4dc89ae04f874a9bcacbdab32e1d0c1a.png" alt="Cover for IV" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-03-22</td>
      <td>Bucket Full of Teeth</td>
      <td>IV</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a> # 54</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>22</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1d394dba4b174ed7a6b2bc0682e43176.png" alt="Cover for 黒斑の侵蝕" style="max-width:120px; height:auto;" />
      </td>
      <td>2007</td>
      <td>Heaven in Her Arms</td>
      <td>黒斑の侵蝕</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>65</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/309a603c5c084152a805bcb02a829799.png" alt="Cover for La Violence Thérapeutique" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-09-29</td>
      <td>29&#x2F;09</td>
      <td>La Violence Thérapeutique</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/deathgrind" | relative_url }}">deathgrind</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>57</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6a125cef6b090db28181a6ec9d56eb26.png" alt="Cover for Never Mind the Bombings, Here&#39;s Your Six Figures" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-06-23</td>
      <td>United Nations</td>
      <td>Never Mind the Bombings, Here&#39;s Your Six Figures</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>64</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8bfc4cd9ca02e99a23522f42f3336192.png" alt="Cover for Sovngarde" style="max-width:120px; height:auto;" />
      </td>
      <td>2012</td>
      <td>Lord Snow</td>
      <td>Sovngarde</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>68</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/901413435037457be523ac4f69d6e7de.png" alt="Cover for Lord Snow" style="max-width:120px; height:auto;" />
      </td>
      <td>2012</td>
      <td>Lord Snow</td>
      <td>Lord Snow</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>94</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/92aa465e1336007fb4aad9ccef89dcbb.png" alt="Cover for 死の手" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-04-30</td>
      <td>Father Figure</td>
      <td>死の手</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>6</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e483d19c31fd9844a7da2be235edd75a.png" alt="Cover for Lumens" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-06-15</td>
      <td>Beau Navire</td>
      <td>Lumens</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 15</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>97</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/17ea33e9fe5a4255bb31e0445b2b6633.png" alt="Cover for Demo" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-08</td>
      <td>Eyrie</td>
      <td>Demo</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/russian-emo" | relative_url }}">russian emo</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>3</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/91356921bad4f558d51f50940d303e0e.png" alt="Cover for Solitude" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-09-16</td>
      <td>Lord Snow</td>
      <td>Solitude</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a> # 91</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>89</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c7a83ee21611823c628df9ee0b6e79a6.png" alt="Cover for Discography" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-04-02</td>
      <td>Deers!</td>
      <td>Discography</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>23</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7c7ba00cbef1461dc6a15578d2298de0.png" alt="Cover for The Next Four Years" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-07-15</td>
      <td>United Nations</td>
      <td>The Next Four Years</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>83</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b39451dff5d279a22e09d0674b55122a.png" alt="Cover for Deathless" style="max-width:120px; height:auto;" />
      </td>
      <td>2015</td>
      <td>Ostraca</td>
      <td>Deathless</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise" | relative_url }}">noise</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>37</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e4c91933324558d2f1f829f165594b9a.png" alt="Cover for Metaphor Technology" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-05-28</td>
      <td>All My Wishes Were Thrown Down a Well and Should Die There</td>
      <td>Metaphor Technology</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>28</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7fd96f8e428811920f0c10ee907d0371.png" alt="Cover for demo" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-12-18</td>
      <td>Letters To Catalonia</td>
      <td>demo</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 79</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>43</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/91af045e904dc722d509901be785bfdc.png" alt="Cover for Collected" style="max-width:120px; height:auto;" />
      </td>
      <td>2016</td>
      <td>Saetia</td>
      <td>Collected</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a> # 73</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>76</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/07737ea4c584d707ae0855858f80630a.png" alt="Cover for Gravity and Grace" style="max-width:120px; height:auto;" />
      </td>
      <td>2016</td>
      <td>Respire</td>
      <td>Gravity and Grace</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-black-metal" | relative_url }}">post-black metal</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>49</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/89409d6dc1d450931e0c926521fbce75.png" alt="Cover for Faces of the Moving Year" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-03-10</td>
      <td>Ostraca</td>
      <td>Faces of the Moving Year</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>91</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fd3b016b0dca157903c76a6eb6615d5d.png" alt="Cover for KALEIDOSKOP" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-05-06</td>
      <td>kishote</td>
      <td>KALEIDOSKOP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-black-metal" | relative_url }}">post-black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>21</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/733d8fde9e13bff8c5244a9f94632fc9.png" alt="Cover for Last" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-05-24</td>
      <td>Ostraca</td>
      <td>Last</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>20</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f473e7bc178e6982ce2e00446e05cea7.png" alt="Cover for Enemy" style="max-width:120px; height:auto;" />
      </td>
      <td>2018</td>
      <td>Ostraca</td>
      <td>Enemy</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>50</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4ae477285b4041577f4bae69a6b0e360.png" alt="Cover for Commit More Arson" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-07-03</td>
      <td>Binary</td>
      <td>Commit More Arson</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>92</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/87b1f7c90b54d770a3b5962d27029282.png" alt="Cover for Residue" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-11-13</td>
      <td>Blind Girls</td>
      <td>Residue</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>60</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/265744544cd918b57d858a75420f3bb4.png" alt="Cover for XX" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-12-11</td>
      <td>Minaret</td>
      <td>XX</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>11</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fd57fa618deb72e12a4c08a49be58792.png" alt="Cover for 2020" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-02-13</td>
      <td>Shin Guard</td>
      <td>2020</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a> # 90</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>30</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f27a1e4f23a92cd1612a561234491993.png" alt="Cover for Live and Let&#39;s Die!" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-03-20</td>
      <td>Kid, Feral</td>
      <td>Live and Let&#39;s Die!</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 69</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>70</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1f77d632ba87b4796543570d518d2466.png" alt="Cover for Shadowmarks" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-04</td>
      <td>Lord Snow</td>
      <td>Shadowmarks</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>42</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/867dfcf425a9cbc5bcb121c7c7d85b56.png" alt="Cover for My First Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-07-19</td>
      <td>Peppa Pig</td>
      <td>My First Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-wave" | relative_url }}">new wave</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise" | relative_url }}">noise</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/noise" | relative_url }}">noise</a> # 23</li>
            <li> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a> # 41</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>46</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/76cc7b33d91d5242a8b615c06884bcc5.png" alt="Cover for Glorify" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-11-22</td>
      <td>Diploid</td>
      <td>Glorify</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>8</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4fe10d55e6d9e5db32550e51c32eeac9.png" alt="Cover for a hedgehog&#39;s dilemma" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-02-09</td>
      <td>44.caliberloveletter</td>
      <td>a hedgehog&#39;s dilemma</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/straight-edge" | relative_url }}">straight edge</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/straight-edge" | relative_url }}">straight edge</a> # 13</li>
            <li> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a> # 100</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>85</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4553a6fce8f63a270cf6e3878a60537b.png" alt="Cover for MMXXII" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-03-30</td>
      <td>Yearning</td>
      <td>MMXXII</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>25</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/81d15ebe4e1d754c067f7f1764f2f032.png" alt="Cover for Ritma" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-06-03</td>
      <td>Piri Reis</td>
      <td>Ritma</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>9</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c6a52ed5a2b94ad5921cbf94949df696.png" alt="Cover for carving initials" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-02-28</td>
      <td>herlovebeheadsdaisies</td>
      <td>carving initials</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>48</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/24570c82639dabd274bbdf6baa0d9bd5.png" alt="Cover for The Gray In Between" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-05-05</td>
      <td>Jeromes Dream</td>
      <td>The Gray In Between</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>17</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6439697d254210fd2e59b28dff329f52.png" alt="Cover for every day is sOmeOne&#39;s speciaL day" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-06-01</td>
      <td>olth</td>
      <td>every day is sOmeOne&#39;s speciaL day</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>32</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ea2d55a654fbde1debc9a43d260e8c4b.png" alt="Cover for Disaster" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-07</td>
      <td>Ostraca</td>
      <td>Disaster</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>29</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b12fc945eb06e7fbead233a28b2120b4.png" alt="Cover for Captain Jazz" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-12-01</td>
      <td>Captain Jazz</td>
      <td>Captain Jazz</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/midwest-emo" | relative_url }}">midwest emo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>66</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c05df9c8cea05c49cf69069499fc057e.png" alt="Cover for Obsidian Wreath" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-01-12</td>
      <td>Infant Island</td>
      <td>Obsidian Wreath</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-metal" | relative_url }}">post-metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>13</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4a12ac400e723c6007c6c7e4666d1b80.png" alt="Cover for Artificial Bouquet" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-03-29</td>
      <td>Frail Body</td>
      <td>Artificial Bouquet</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-metal" | relative_url }}">post-metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>36</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b397b2dce60bd31f5b700f4f0978e43c.png" alt="Cover for Salt Sermon" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-08-16</td>
      <td>Missouri Executive Order 44</td>
      <td>Salt Sermon</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>15</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8a8e903caf4949f5b9b3b28359bfcfd5.png" alt="Cover for You Pull The Strings That Make Us Dance" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Textbook Traitors</td>
      <td>You Pull The Strings That Make Us Dance</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 48</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>27</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/54e4c107f46584000194a729e7899c53.png" alt="Cover for iwrotehaikusaboutcanniibalisminyouryearbook 1-26" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Lil Kappo</td>
      <td>iwrotehaikusaboutcanniibalisminyouryearbook 1-26</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a> # 65</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>51</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b0f67aeaedd3ae6b04460520b069b808.png" alt="Cover for Circle Thinking" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>I Hate Sex</td>
      <td>Circle Thinking</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a> # 19</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>75</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fd4f2b1ef6400eae306e29e0fa66dfef.png" alt="Cover for Slow Burn" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Old Gray</td>
      <td>Slow Burn</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/spoken-word" | relative_url }}">spoken word</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/spoken-word" | relative_url }}">spoken word</a> # 3</li>
            <li> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a> # 75</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>7</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9bfab8dc09b69d2d67470efb6fb7b519.png" alt="Cover for fragmentary" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Letters To Catalonia</td>
      <td>fragmentary</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>10</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/33f56af783e6bd78929b2c62edd3f2e5.png" alt="Cover for Youth Novel" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Youth Novel</td>
      <td>Youth Novel</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>14</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8cc3894e8f3acd83a62b0ffa032f71e4.png" alt="Cover for An Exit Exists" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Blind Girls</td>
      <td>An Exit Exists</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>24</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4732ebad1d5ff0c4315854c604f80c29.png" alt="Cover for crows crows crows crows" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Alinah</td>
      <td>crows crows crows crows</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>26</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f343a75e376a0872b2adcac6f6e9388f.png" alt="Cover for Disentanglement" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Fluoride</td>
      <td>Disentanglement</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>31</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c7a8122474df4b1bcb2483bfe9d44419.png" alt="Cover for Dance Tonight! Revolution Tomorrow! + Chaos Is Me" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Orchid</td>
      <td>Dance Tonight! Revolution Tomorrow! + Chaos Is Me</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>33</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/83584a3efb43f133897046d62de1fd2c.png" alt="Cover for As Dust Collects" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>New Forms</td>
      <td>As Dust Collects</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>40</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/01a1a989cfb3e8c4f1b740547ffb6c6a.png" alt="Cover for How This All Ends" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Nuvolascura</td>
      <td>How This All Ends</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>45</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/dae77646526ba3baab192602cbcdd0e6.png" alt="Cover for This Bitter Garden" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>for your health</td>
      <td>This Bitter Garden</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>52</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/185d3eedcaea56679d0da453088c286d.png" alt="Cover for Wilting" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Votive</td>
      <td>Wilting</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>58</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ec3318c8389dc9f933355ac3a52ab1a1.png" alt="Cover for no necesitas palabras" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>mis sueños son de tu adiós</td>
      <td>no necesitas palabras</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>61</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c30a27fe01dd4cb5d11599aedf401cbc.png" alt="Cover for Have You Heard of the High Elves" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Lord Snow</td>
      <td>Have You Heard of the High Elves</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>62</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/574feebd5753c18a7f2fb35271fee9e5.png" alt="Cover for Self Titled" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Get Fucked</td>
      <td>Self Titled</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>63</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a4b7d97ddf43a131cc0924250a971be2.png" alt="Cover for I &#x2F; II &#x2F; III &#x2F; IV" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Bucket Full of Teeth</td>
      <td>I &#x2F; II &#x2F; III &#x2F; IV</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>72</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1a0109c73095ffadb53862e70a758a71.png" alt="Cover for Минское Море" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>ясность</td>
      <td>Минское Море</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>73</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a765b35eaf62ae814ea6288dfbea8eaa.png" alt="Cover for Forged from the Sun" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Cady</td>
      <td>Forged from the Sun</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/skramz" | relative_url }}">skramz</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>78</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2488953b21280938b8a76c29bd644240.png" alt="Cover for калейдоскоп упущенных мгновений" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Get Cold</td>
      <td>калейдоскоп упущенных мгновений</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>79</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/920b3c970b4dfac737367a748beeba78.png" alt="Cover for demo" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>becausewecouldnotstopevenfordeath</td>
      <td>demo</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>80</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/92950b79718a6b6fb5bd8e11b150e16f.png" alt="Cover for The History Behind The Mystery: Music To Kill Yourself By" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>In&#x2F;Humanity</td>
      <td>The History Behind The Mystery: Music To Kill Yourself By</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/powerviolence" | relative_url }}">powerviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>81</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fdf359aa9cbc4fa7ae7b9f7b7c3cab88.png" alt="Cover for La fine non è la fine" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>La Quiete</td>
      <td>La fine non è la fine</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>84</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a3f3574c123545517aab89e140dec527.png" alt="Cover for Demo 2025" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>thisisit.stalingrad</td>
      <td>Demo 2025</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>86</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/444ee48380a8e3dce416a35f0d1d0bda.png" alt="Cover for Hands In The Air!" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>told not to worry</td>
      <td>Hands In The Air!</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>87</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a49b849391b53b3fc5d95d06a00f927a.png" alt="Cover for approximation_of_a_human" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>.gif from god</td>
      <td>approximation_of_a_human</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>90</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8760d6e780093cf25a721318be924962.png" alt="Cover for liljekonvalj" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>...och sedan drunknade vi</td>
      <td>liljekonvalj</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>98</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c7a107f82cd41891b90148ce7407e13f.png" alt="Cover for Honeywell" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Honeywell</td>
      <td>Honeywell</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore-punk" | relative_url }}">hardcore punk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>99</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0d5732eabac0423eeb2ab7793587b2b2.png" alt="Cover for Tangled in the Fabric Halfway to the Floor" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Despot</td>
      <td>Tangled in the Fabric Halfway to the Floor</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>100</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9ccbe093618c9018b7d53f9f32e6377c.png" alt="Cover for Don&#39;t&#x2F;&#x2F;try&#x2F;&#x2F;let&#x2F;&#x2F;die" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Amitié_</td>
      <td>Don&#39;t&#x2F;&#x2F;try&#x2F;&#x2F;let&#x2F;&#x2F;die</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/emoviolence" | relative_url }}">emoviolence</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/screamo" | relative_url }}">screamo</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>44</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2fd41c182c5c864220fdd2cee994f247.png" alt="Cover for Manual para superar el Síndrome de Escapismo, o al menos intentarlo" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>La Chica De Las Camelias</td>
      <td>Manual para superar el Síndrome de Escapismo, o al menos intentarlo</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>56</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4d353d637172d58ad908ffe5404d9346.png" alt="Cover for s&#x2F;t" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Iwrotehaikusaboutcannibalisminyouryearbook</td>
      <td>s&#x2F;t</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>67</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/448169bc78094a4ea5e8dee5d0912dde.png" alt="Cover for Einerseits, Andererseits" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Danse Macabre</td>
      <td>Einerseits, Andererseits</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>95</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3d98470114772fd23853d8fa5225539a.png" alt="Cover for All Pain, No Feeling" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Waternoose</td>
      <td>All Pain, No Feeling</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>96</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/63dd95477eee7943a6d94e0d0b1902e4.png" alt="Cover for Second Nail" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>A Fine Boat, That Coffin!</td>
      <td>Second Nail</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
  </tbody>
</table>

### Related genres

- [screamo]({{ "/tags/screamo" | relative_url }})
- [emo]({{ "/tags/emo" | relative_url }})
- [skramz]({{ "/tags/skramz" | relative_url }})
- [hardcore]({{ "/tags/hardcore" | relative_url }})
- [post-hardcore]({{ "/tags/post-hardcore" | relative_url }})
- [powerviolence]({{ "/tags/powerviolence" | relative_url }})
- [grindcore]({{ "/tags/grindcore" | relative_url }})
- [hardcore punk]({{ "/tags/hardcore-punk" | relative_url }})
- [mathcore]({{ "/tags/mathcore" | relative_url }})
- [metalcore]({{ "/tags/metalcore" | relative_url }})
