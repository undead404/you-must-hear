---
category: genre
date: 2025-12-05T04:27:05.387Z
permalink: tags/chamber-rock
title: chamber rock
---

## chamber rock

Last updated: <time datetime="2025-12-05T04:27:05.387Z">Fri Dec 05 2025 06:27:05 GMT+0200 (Eastern European Standard Time)</time>

### Description

Chamber rock blends the intimacy and instrumentation of classical chamber music with rock&#39;s emotional intensity and song structures. It often features strings, piano, and unconventional arrangements that create dramatic, cinematic atmospheres while maintaining an indie sensibility. The style emphasizes textural depth and compositional sophistication over traditional rock power.

### 100 chronological releases

<table>
  <thead>
    <tr>
      <th>Place</th>
      <th>Cover</th>
      <th>Date</th>
      <th>Artist</th>
      <th>Album</th>
      <th>Tags</th>
      <th>Other places</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>74</td>
      <td>
      </td>
      <td>1977</td>
      <td>Various Artists</td>
      <td>Evermore</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/american" | relative_url }}">american</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folk-pop" | relative_url }}">folk pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>62</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5f07a73725bc453c9393f2c8d622c93b.png" alt="Cover for Heresie" style="max-width:120px; height:auto;" />
      </td>
      <td>1979</td>
      <td>Univers Zero</td>
      <td>Heresie</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a> # 26</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>76</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1fad45a32d8f8fc8b036c43030677b57.png" alt="Cover for Musique Pour L&#39;Odyssée" style="max-width:120px; height:auto;" />
      </td>
      <td>1979</td>
      <td>Art Zoyd</td>
      <td>Musique Pour L&#39;Odyssée</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>11</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8be66dae31244e1dbc04b7d9dbd4de61.png" alt="Cover for Génération sans futur" style="max-width:120px; height:auto;" />
      </td>
      <td>1980</td>
      <td>Art Zoyd</td>
      <td>Génération sans futur</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>49</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f11e8dfed69940b7bb8a1c903fda741c.png" alt="Cover for The World As It Is Today" style="max-width:120px; height:auto;" />
      </td>
      <td>1981</td>
      <td>Art Bears</td>
      <td>The World As It Is Today</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a> # 16</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>32</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a5676e5dd25840428c9784b49d538de1.png" alt="Cover for Odyshape" style="max-width:120px; height:auto;" />
      </td>
      <td>1981-06</td>
      <td>The Raincoats</td>
      <td>Odyshape</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-rock" | relative_url }}">avant-rock</a> # 45</li>
            <li> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a> # 67</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>85</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0e8e211d61244185982aeca873f82813.png" alt="Cover for Letters Home" style="max-width:120px; height:auto;" />
      </td>
      <td>1986</td>
      <td>News From Babel</td>
      <td>Letters Home</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a> # 48</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>51</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6c03497d8a47c8c181b07a82705f1171.png" alt="Cover for Même en Hiver&#x2F;Comme un Pinson Dans L&#39;Eau" style="max-width:120px; height:auto;" />
      </td>
      <td>1989</td>
      <td>L&#39;Ensemble Rayé</td>
      <td>Même en Hiver&#x2F;Comme un Pinson Dans L&#39;Eau</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a> # 36</li>
            <li> <a href="{{ "/tags/prog-rock" | relative_url }}">prog-rock</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>69</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ecca1f10f80a4613ac1fc5807960a317.png" alt="Cover for Introspezione" style="max-width:120px; height:auto;" />
      </td>
      <td>1989</td>
      <td>Opus Avantra</td>
      <td>Introspezione</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a> # 47</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>86</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fdff5de456c9f75d895d48f9b9b9b681.png" alt="Cover for In This Life" style="max-width:120px; height:auto;" />
      </td>
      <td>1989</td>
      <td>Thinking Plague</td>
      <td>In This Life</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ethnic-fusion" | relative_url }}">ethnic fusion</a> # 99</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>48</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/76c49aa401b64bee94fd8db98dee1058.png" alt="Cover for Quelques pièces détachées" style="max-width:120px; height:auto;" />
      </td>
      <td>1993</td>
      <td>L&#39;Ensemble Rayé</td>
      <td>Quelques pièces détachées</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a> # 27</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>18</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7fef7e77c652ea27acdf442ddafc0e4e.png" alt="Cover for Emile au jardin patrologique" style="max-width:120px; height:auto;" />
      </td>
      <td>1994</td>
      <td>Debile Menthol</td>
      <td>Emile au jardin patrologique</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a> # 39</li>
            <li> <a href="{{ "/tags/prog-rock" | relative_url }}">prog-rock</a> # 48</li>
            <li> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>40</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/de111bf38a470c8d1148fe8ca205d118.png" alt="Cover for Battre campagne" style="max-width:120px; height:auto;" />
      </td>
      <td>1994</td>
      <td>Debile Menthol</td>
      <td>Battre campagne</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/prog-rock" | relative_url }}">prog-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/prog-rock" | relative_url }}">prog-rock</a> # 21</li>
            <li> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a> # 65</li>
            <li> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a> # 89</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>66</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f41fecdb52a53534b6eb01075c1ac6bb.png" alt="Cover for Emile à la campagne" style="max-width:120px; height:auto;" />
      </td>
      <td>1994</td>
      <td>Debile Menthol</td>
      <td>Emile à la campagne</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a> # 60</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>63</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1a2d95dd2c9505ddc43a5562800fff5b.png" alt="Cover for Faust" style="max-width:120px; height:auto;" />
      </td>
      <td>1995</td>
      <td>Art Zoyd</td>
      <td>Faust</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a> # 99</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>29</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/69a40200180f9f506d69f510593ec299.png" alt="Cover for Failure &#x2F; Animus" style="max-width:120px; height:auto;" />
      </td>
      <td>1996</td>
      <td>Swans</td>
      <td>Failure &#x2F; Animus</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/america" | relative_url }}">america</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/american" | relative_url }}">american</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/american-underground" | relative_url }}">american underground</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/apocalyptic-folk" | relative_url }}">apocalyptic folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-rock" | relative_url }}">avant-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dark-folk" | relative_url }}">dark folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dark-pop" | relative_url }}">dark pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/darkwave" | relative_url }}">darkwave</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/neoclassical" | relative_url }}">neoclassical</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/neofolk" | relative_url }}">neofolk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-york" | relative_url }}">new york</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/no-wave" | relative_url }}">no wave</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/space-rock" | relative_url }}">space rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/usa" | relative_url }}">usa</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-rock" | relative_url }}">avant-rock</a> # 38</li>
            <li> <a href="{{ "/tags/american-underground" | relative_url }}">american underground</a> # 50</li>
            <li> <a href="{{ "/tags/america" | relative_url }}">america</a> # 83</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>77</td>
      <td>
      </td>
      <td>1996</td>
      <td>L&#39;Ensemble Rayé</td>
      <td>En Frac</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
            <li style="font-size: calc(1rem * 88 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
            <li style="font-size: calc(1rem * 79 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>5</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e9d337771901d79174f707459c1d2e61.png" alt="Cover for Thanks For The Ether" style="max-width:120px; height:auto;" />
      </td>
      <td>1996-08-06</td>
      <td>Rasputina</td>
      <td>Thanks For The Ether</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>43</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4a11775d0f4d4917bb51657b604b5903.png" alt="Cover for Closed for Business" style="max-width:120px; height:auto;" />
      </td>
      <td>1997-10-06</td>
      <td>Mansun</td>
      <td>Closed for Business</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/art-pop" | relative_url }}">art pop</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/gothic-pop" | relative_url }}">gothic pop</a> # 94</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>94</td>
      <td>
      </td>
      <td>1998</td>
      <td>Cédric Vuille&#x2F;Jean-20 Huguenin</td>
      <td>Un Royaume, une Espece de Vide</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a> # 80</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>96</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d91045c6c4b842fdbafa902dc2492238.png" alt="Cover for Scapegoat" style="max-width:120px; height:auto;" />
      </td>
      <td>1998</td>
      <td>Ensemble Nimbus</td>
      <td>Scapegoat</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>41</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/24d7074aedc84ff79fe0db8f68076c27.png" alt="Cover for How We Quit the Forest" style="max-width:120px; height:auto;" />
      </td>
      <td>1998-08-04</td>
      <td>Rasputina</td>
      <td>How We Quit the Forest</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/steampunk" | relative_url }}">steampunk</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/steampunk" | relative_url }}">steampunk</a> # 2</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>53</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/182d2737ef441f770de9c1f91fc302f8.png" alt="Cover for The Path" style="max-width:120px; height:auto;" />
      </td>
      <td>1999</td>
      <td>Light</td>
      <td>The Path</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/french" | relative_url }}">french</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>70</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a6fddb0b52a54c9accf3c05fe66b6301.png" alt="Cover for Songs for Walter" style="max-width:120px; height:auto;" />
      </td>
      <td>1999</td>
      <td>Gropius</td>
      <td>Songs for Walter</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>47</td>
      <td>
          <img src="https://i.discogs.com/RkC1ag_rgmhy_Rz0FP_hwzeCXrPEjX1-KeGr10EeDgs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE0ODEz/ODYtMTQ4OTU3MzUx/Mi03OTY2LmpwZWc.jpeg" alt="Cover for Ein Fest für Pu den Bären" style="max-width:120px; height:auto;" />
      </td>
      <td>2000</td>
      <td>L&#39;Ensemble Rayé</td>
      <td>Ein Fest für Pu den Bären</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a> # 62</li>
            <li> <a href="{{ "/tags/prog-rock" | relative_url }}">prog-rock</a> # 87</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>80</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/496cb4504d0941bac1dfeeef4a35ace7.png" alt="Cover for Indelicate" style="max-width:120px; height:auto;" />
      </td>
      <td>2000</td>
      <td>Gropius</td>
      <td>Indelicate</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>83</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4331fc7e25144e5aa15263233053a9ae.png" alt="Cover for Vis-à-Vis Movers Dance Company" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>L&#39;Ensemble Rayé</td>
      <td>Vis-à-Vis Movers Dance Company</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>45</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b8e9086667554bf7bed481d2d5166039.png" alt="Cover for Les Contrepoints Cardinaux" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>L&#39;Ensemble Rayé</td>
      <td>Les Contrepoints Cardinaux</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a> # 24</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>3</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/60a7e019213b7154684f5da7f62f40f9.png" alt="Cover for Cabin Fever" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>Rasputina</td>
      <td>Cabin Fever</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>17</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/be16473243f741eb8c28d28e0690dcc5.png" alt="Cover for Det Er Mig Der Holder Træerne Sammen" style="max-width:120px; height:auto;" />
      </td>
      <td>2002-04-02</td>
      <td>Under Byen</td>
      <td>Det Er Mig Der Holder Træerne Sammen</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>30</td>
      <td>
          <img src="https://i.discogs.com/eRZn_T3gO5nKNZFV6FcnLjgTu2pt80meuvIByMWqVfc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI0MDE1/MTYtMTUwMTI0MjI2/NC00MjU5LmpwZWc.jpeg" alt="Cover for Des Pas Rayes" style="max-width:120px; height:auto;" />
      </td>
      <td>2003</td>
      <td>Cédric Vuille</td>
      <td>Des Pas Rayes</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
            <li style="font-size: calc(1rem * 73 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
            <li style="font-size: calc(1rem * 51 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a> # 29</li>
            <li> <a href="{{ "/tags/prog-rock" | relative_url }}">prog-rock</a> # 32</li>
            <li> <a href="{{ "/tags/chamber" | relative_url }}">chamber</a> # 47</li>
            <li> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a> # 54</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>35</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/36fc54b9a038402a8d7a548bbc6ccc1d.png" alt="Cover for Live" style="max-width:120px; height:auto;" />
      </td>
      <td>2003</td>
      <td>Sleepytime Gorilla Museum</td>
      <td>Live</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/bass" | relative_url }}">bass</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folk" | relative_url }}">folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise" | relative_url }}">noise</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>12</td>
      <td>
          <img src="https://i.discogs.com/X7ZEZ3iRBz5UKAaDEGOiTJRnELhMopkILhn5O1k_3NU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTUyODMz/MC0xMjI2MDA3NjQw/LmpwZWc.jpeg" alt="Cover for Ghost Folks" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-01-31</td>
      <td>Natural Snow Buildings</td>
      <td>Ghost Folks</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folk" | relative_url }}">folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>64</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c03ba10168287f606d478778b500a5dd.png" alt="Cover for Choirs of the Eye" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-10-21</td>
      <td>Kayo Dot</td>
      <td>Choirs of the Eye</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-metal" | relative_url }}">post-metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-rock" | relative_url }}">avant-rock</a> # 17</li>
            <li> <a href="{{ "/tags/post-metal" | relative_url }}">post-metal</a> # 27</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>68</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/263dd84e1e134642831fa74f4784c36e.png" alt="Cover for Frustration Plantation" style="max-width:120px; height:auto;" />
      </td>
      <td>2004-03-16</td>
      <td>Rasputina</td>
      <td>Frustration Plantation</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/dark-cabaret" | relative_url }}">dark cabaret</a> # 84</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>67</td>
      <td>
          <img src="https://i.discogs.com/irFWgCt-5g1kG7Na2CHFoaHYHocgkkqJcAnezv2vXqo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTYwMzcw/Mi0xMTM3MzUzNzk5/LmpwZWc.jpeg" alt="Cover for Frustration Plantation (bonus disc: Poor Relations in the Shed Out Back)" style="max-width:120px; height:auto;" />
      </td>
      <td>2004-03-16</td>
      <td>Rasputina</td>
      <td>Frustration Plantation (bonus disc: Poor Relations in the Shed Out Back)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>8</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e5e00692fc9045f8b78c1e511038af82.png" alt="Cover for The Folded Palm" style="max-width:120px; height:auto;" />
      </td>
      <td>2004-09-14</td>
      <td>Frog Eyes</td>
      <td>The Folded Palm</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/psychedelic-rock" | relative_url }}">psychedelic rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>93</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/398692518aee4dfaa45705faf0d4db62.png" alt="Cover for Théâtre De La Poudrière" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>L&#39;Ensemble Rayé</td>
      <td>Théâtre De La Poudrière</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>7</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4c2a02a9900eff56a6d1181c6a59a34e.png" alt="Cover for On Tour" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-11-13</td>
      <td>Yann Tiersen</td>
      <td>On Tour</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/french" | relative_url }}">french</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>20</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ef50d606bec844cb8435773e4aa3d087.png" alt="Cover for Perplexions" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-12</td>
      <td>Melora Creager</td>
      <td>Perplexions</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/singer-songwriter" | relative_url }}">singer-songwriter</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>72</td>
      <td>
          <img src="https://i.discogs.com/50RlCHK5DUCB4tDXQjgRou7XdEk_D6H2GMQ3VS-NCkA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE4MjUw/NTktMTY1NTQ5MDE4/My05NzI0LmpwZWc.jpeg" alt="Cover for Objet Abandonné en Mer" style="max-width:120px; height:auto;" />
      </td>
      <td>2007</td>
      <td>Nicolas Bernier + Simon Trottier</td>
      <td>Objet Abandonné en Mer</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-rock" | relative_url }}">avant-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/canada" | relative_url }}">canada</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electroacoustic" | relative_url }}">electroacoustic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/montreal" | relative_url }}">montreal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>22</td>
      <td>
          <img src="https://i.discogs.com/traDkEIEa_G21JAfBJ8erO4S1ZniLG1SqwCHb-mZdsQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTExNDM0/MDctMTE5NTU4MDY3/MS5qcGVn.jpeg" alt="Cover for le mépris (aer011)" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-11-08</td>
      <td>Le Mépris</td>
      <td>le mépris (aer011)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/contemporary-classical" | relative_url }}">contemporary classical</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/japan" | relative_url }}">japan</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/japanese" | relative_url }}">japanese</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/piano" | relative_url }}">piano</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>13</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d67c3d07768948a1b49a2f97295660ec.png" alt="Cover for We Became Your Family When You Died" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-01-01</td>
      <td>Bullets In Madison</td>
      <td>We Became Your Family When You Died</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>24</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/956d2e3d692e4bc5cbddf0fae31a68af.png" alt="Cover for Siamesisk" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-02-18</td>
      <td>Under Byen</td>
      <td>Siamesisk</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>16</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d2a61c37964c4695b8cd5c1fc092d7cf.png" alt="Cover for The Nautilus Years" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-03-10</td>
      <td>Surrounded</td>
      <td>The Nautilus Years</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-pop" | relative_url }}">indie pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indietronica" | relative_url }}">indietronica</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/sweden" | relative_url }}">sweden</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/swedish" | relative_url }}">swedish</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>28</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d1d98d97c8af4344c317fd54a23e305b.png" alt="Cover for Good Nature" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-03-17</td>
      <td>Youthmovies</td>
      <td>Good Nature</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/britpop" | relative_url }}">britpop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/progressive-folk" | relative_url }}">progressive folk</a> # 10</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>36</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f67a0c79706741b1c126080fcf0ca463.png" alt="Cover for Polyp EP" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-11-03</td>
      <td>Youthmovies</td>
      <td>Polyp EP</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>95</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4024978476b14d9cbd50439acf62b676.png" alt="Cover for Squarcicatrici" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Squarcicatrici</td>
      <td>Squarcicatrici</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/italian" | relative_url }}">italian</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/jazzcore" | relative_url }}">jazzcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>99</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9dd174efc43547a2abf8355505d41ea9.png" alt="Cover for Little Enchanted Void" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Mindflower</td>
      <td>Little Enchanted Void</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>15</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5ecb2f8ac41649a8b40010cccf21d52b.png" alt="Cover for Sorry Monsters, I Have To Grow" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-01-07</td>
      <td>A Journey Down the Well</td>
      <td>Sorry Monsters, I Have To Grow</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/chamber" | relative_url }}">chamber</a> # 9</li>
            <li> <a href="{{ "/tags/choir" | relative_url }}">choir</a> # 42</li>
            <li> <a href="{{ "/tags/strings" | relative_url }}">strings</a> # 53</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>50</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b5b1c5d75898f942a348686eb5df6aae.png" alt="Cover for Great Surround" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-03-09</td>
      <td>The Fatales</td>
      <td>Great Surround</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/contemporary-classical" | relative_url }}">contemporary classical</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-wave" | relative_url }}">new wave</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/ethereal" | relative_url }}">ethereal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>34</td>
      <td>
      </td>
      <td>2009-03-24</td>
      <td>Monokle</td>
      <td>[iD.053] - Monokle - Tesaurus (LP)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic-pop" | relative_url }}">electronic pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/russia" | relative_url }}">russia</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/russian" | relative_url }}">russian</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/electronic-pop" | relative_url }}">electronic pop</a> # 63</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>78</td>
      <td>
          <img src="https://i.discogs.com/_28qNBq80jXaCB9be07Iwd0aoLxBcQq9oVYWQJeMKWA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIzNTI4/OTQtMTI3ODk3MDEz/MS5wbmc.jpeg" alt="Cover for Floppy Disk" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-07-13</td>
      <td>Barbagallo</td>
      <td>Floppy Disk</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/italian" | relative_url }}">italian</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/avant-folk" | relative_url }}">avant-folk</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/acid-folk" | relative_url }}">acid folk</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>25</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3e0a2470d97e4f6f982dd159dd8fd2d1.png" alt="Cover for Songs From Mirage" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-09-11</td>
      <td>Aranis</td>
      <td>Songs From Mirage</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/y2k-prog" | relative_url }}">y2k prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>6</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f2f83759a50360ff954861c223a5e777.png" alt="Cover for evermore" style="max-width:120px; height:auto;" />
      </td>
      <td>2010</td>
      <td>Taylor Swift</td>
      <td>evermore</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/folk-pop" | relative_url }}">folk pop</a> # 2</li>
            <li> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a> # 56</li>
            <li> <a href="{{ "/tags/chamber-folk" | relative_url }}">chamber folk</a> # 65</li>
            <li> <a href="{{ "/tags/indie-folk" | relative_url }}">indie folk</a> # 85</li>
            <li> <a href="{{ "/tags/indie-pop" | relative_url }}">indie pop</a> # 88</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>57</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e1f18c59ea1ede7320f5981d28f77dfe.png" alt="Cover for Faire" style="max-width:120px; height:auto;" />
      </td>
      <td>2010</td>
      <td>Cédric Vuille</td>
      <td>Faire</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/switzerland" | relative_url }}">switzerland</a> # 58</li>
            <li> <a href="{{ "/tags/chamber" | relative_url }}">chamber</a> # 60</li>
            <li> <a href="{{ "/tags/prog-rock" | relative_url }}">prog-rock</a> # 68</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>81</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/80728feb2d5f41b7a9ea8bf0749f1819.png" alt="Cover for Clivages" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-01-26</td>
      <td>Univers Zero</td>
      <td>Clivages</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a> # 35</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>10</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/cf587097b1f84ac5b534bcb946a0e82a.png" alt="Cover for A Chorus of Storytellers" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-02-01</td>
      <td>The Album Leaf</td>
      <td>A Chorus of Storytellers</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>46</td>
      <td>
      </td>
      <td>2010-04-22</td>
      <td>Flëur</td>
      <td>Тысяча светлых ангелов</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>42</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2b46e9c227864e10884499146c3e571b.png" alt="Cover for Sister Kinderhook" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-06-15</td>
      <td>Rasputina</td>
      <td>Sister Kinderhook</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/freak-folk" | relative_url }}">freak folk</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/freak-folk" | relative_url }}">freak folk</a> # 49</li>
            <li> <a href="{{ "/tags/dark-cabaret" | relative_url }}">dark cabaret</a> # 77</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>59</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bd8e892e46df4f30cf9756ac0e3d2b08.png" alt="Cover for Let Me Come Home" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-10-25</td>
      <td>Broken Records</td>
      <td>Let Me Come Home</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/alternative-folk" | relative_url }}">alternative folk</a> # 63</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>23</td>
      <td>
          <img src="https://i.discogs.com/4BIIEb_0PJxQ9gr6DttMUoBe5t-6vi9M424MpyF7Ses/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQwNzM2/NTYtMTM1NDM1NjYw/OS01OTAwLmpwZWc.jpeg" alt="Cover for Lech-Lecha" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-01-19</td>
      <td>Puffin on My Side</td>
      <td>Lech-Lecha</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient-rock" | relative_url }}">ambient rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/europe" | relative_url }}">europe</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/instrumental-rock" | relative_url }}">instrumental rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/italian" | relative_url }}">italian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/italy" | relative_url }}">italy</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ambient-rock" | relative_url }}">ambient rock</a> # 11</li>
            <li> <a href="{{ "/tags/italy" | relative_url }}">italy</a> # 94</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>9</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/966d071fd45145b09087d8c36d6ce6a0.png" alt="Cover for Brick City Ghosts" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-06-20</td>
      <td>Riding Alone for Thousands of Miles</td>
      <td>Brick City Ghosts</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/america" | relative_url }}">america</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/american" | relative_url }}">american</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/american-underground" | relative_url }}">american underground</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/instrumental-rock" | relative_url }}">instrumental rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-jersey" | relative_url }}">new jersey</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/organic-electronica" | relative_url }}">organic electronica</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/piano" | relative_url }}">piano</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/usa" | relative_url }}">usa</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/organic-electronica" | relative_url }}">organic electronica</a> # 4</li>
            <li> <a href="{{ "/tags/new-jersey" | relative_url }}">new jersey</a> # 7</li>
            <li> <a href="{{ "/tags/american-underground" | relative_url }}">american underground</a> # 17</li>
            <li> <a href="{{ "/tags/america" | relative_url }}">america</a> # 17</li>
            <li> <a href="{{ "/tags/instrumental-rock" | relative_url }}">instrumental rock</a> # 18</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>38</td>
      <td>
          <img src="https://i.discogs.com/bVYOfDrxQxK-iamLh02iQo4lqHATN7YHVVBl8Rp87EA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1MTk1/NjU1LTE1ODc5MzU2/NTItNzQ3OC5qcGVn.jpeg" alt="Cover for Daytrotter Session - 2&#x2F;23&#x2F;2010" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-06-22</td>
      <td>Balmorhea</td>
      <td>Daytrotter Session - 2&#x2F;23&#x2F;2010</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/america" | relative_url }}">america</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/american" | relative_url }}">american</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/american-underground" | relative_url }}">american underground</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/austin" | relative_url }}">austin</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/texas" | relative_url }}">texas</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/usa" | relative_url }}">usa</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/austin" | relative_url }}">austin</a> # 24</li>
            <li> <a href="{{ "/tags/american-underground" | relative_url }}">american underground</a> # 60</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>26</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2e98fceb90064c84b4dd3f845710be19.png" alt="Cover for Send me a Postcard" style="max-width:120px; height:auto;" />
      </td>
      <td>2012</td>
      <td>3 Mice</td>
      <td>Send me a Postcard</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/prog-rock" | relative_url }}">prog-rock</a> # 38</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>82</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/24967998fb3e53fc31ac046ca79e356a.png" alt="Cover for Recreo" style="max-width:120px; height:auto;" />
      </td>
      <td>2012</td>
      <td>Karenautas</td>
      <td>Recreo</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/argentina" | relative_url }}">argentina</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>44</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4a1b3b5028adb28f4d69435dafa5cbf1.png" alt="Cover for Присутствие" style="max-width:120px; height:auto;" />
      </td>
      <td>2013</td>
      <td>Елена Войнаровская</td>
      <td>Присутствие</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/singer-songwriter" | relative_url }}">singer-songwriter</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>52</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3ab4d215a2911c374e205e3173e7908c.png" alt="Cover for Твоя правда" style="max-width:120px; height:auto;" />
      </td>
      <td>2013</td>
      <td>Елена Войнаровская</td>
      <td>Твоя правда</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/singer-songwriter" | relative_url }}">singer-songwriter</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/dark-cabaret" | relative_url }}">dark cabaret</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>79</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f8e6e574bbe84080c05bf02c37bcde6b.png" alt="Cover for In Rood" style="max-width:120px; height:auto;" />
      </td>
      <td>2013</td>
      <td>Aranis</td>
      <td>In Rood</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/belgium" | relative_url }}">belgium</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>33</td>
      <td>
      </td>
      <td>2013-02-27</td>
      <td>Aidan Baker &amp; A-Sun Amissa</td>
      <td>Scarpe Sensée</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-folk" | relative_url }}">avant-folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-rock" | relative_url }}">avant-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/canada" | relative_url }}">canada</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/drone" | relative_url }}">drone</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/drone-rock" | relative_url }}">drone rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/duo" | relative_url }}">duo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-folk" | relative_url }}">experimental folk</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/improvisation" | relative_url }}">improvisation</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/minimalism" | relative_url }}">minimalism</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/psychedelic-rock" | relative_url }}">psychedelic rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-folk" | relative_url }}">experimental folk</a> # 15</li>
            <li> <a href="{{ "/tags/avant-rock" | relative_url }}">avant-rock</a> # 50</li>
            <li> <a href="{{ "/tags/drone-rock" | relative_url }}">drone rock</a> # 59</li>
            <li> <a href="{{ "/tags/improvisation" | relative_url }}">improvisation</a> # 62</li>
            <li> <a href="{{ "/tags/duo" | relative_url }}">duo</a> # 100</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>37</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e3bb22e05bbd40888525449a43cb2bdb.png" alt="Cover for asil liseli" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-04-19</td>
      <td>Nult</td>
      <td>asil liseli</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient-rock" | relative_url }}">ambient rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/barcelona" | relative_url }}">barcelona</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/catalan" | relative_url }}">catalan</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/europe" | relative_url }}">europe</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/european" | relative_url }}">european</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/spain" | relative_url }}">spain</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/spanish" | relative_url }}">spanish</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/barcelona" | relative_url }}">barcelona</a> # 20</li>
            <li> <a href="{{ "/tags/ambient-rock" | relative_url }}">ambient rock</a> # 21</li>
            <li> <a href="{{ "/tags/catalan" | relative_url }}">catalan</a> # 53</li>
            <li> <a href="{{ "/tags/spain" | relative_url }}">spain</a> # 96</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>97</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/790fdd61a4e649b8c901f620089e1fc3.png" alt="Cover for Protokol" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-10-07</td>
      <td>Under Byen</td>
      <td>Protokol</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>39</td>
      <td>
          <img src="https://i.discogs.com/5NTKlxgxyGbwIY9M6qr33hpUGLEPAvB3U3SKAsWu6KY/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTcyNzc4/OTUtMTQzNzg0NjI5/NS0zMTYxLmpwZWc.jpeg" alt="Cover for Пока все спят" style="max-width:120px; height:auto;" />
      </td>
      <td>2014</td>
      <td>Голуби и Безумные Кашевары</td>
      <td>Пока все спят</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/jazz-fusion" | relative_url }}">jazz fusion</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/psychedelic-pop" | relative_url }}">psychedelic pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>65</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c4e7a4bd6bee366540982ac763730c2f.png" alt="Cover for Push" style="max-width:120px; height:auto;" />
      </td>
      <td>2014</td>
      <td>Pingvinorkestern</td>
      <td>Push</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/sweden" | relative_url }}">sweden</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>89</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7b1ea61cd7d14247c2a05ec472fa2617.png" alt="Cover for For Those Who Caught the Sun in Flight" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-02</td>
      <td>Tomorrow We Sail</td>
      <td>For Those Who Caught the Sun in Flight</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>1</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/38349cb8c20531de43837d3809f15c52.png" alt="Cover for So There" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-09-11</td>
      <td>Ben Folds</td>
      <td>So There</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/piano" | relative_url }}">piano</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a> # 89</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>31</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c8624a32d066b6b48a72afd44689e33f.png" alt="Cover for Tanta Furia" style="max-width:120px; height:auto;" />
      </td>
      <td>2016</td>
      <td>Birdeatsbaby</td>
      <td>Tanta Furia</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/dark-cabaret" | relative_url }}">dark cabaret</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/dark-cabaret" | relative_url }}">dark cabaret</a> # 94</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>61</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4fcc573d190283128d343e0ede3e6f05.png" alt="Cover for Assault on the Tyranny of Reason" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-09-23</td>
      <td>Emmett Elvin</td>
      <td>Assault on the Tyranny of Reason</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>60</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b46c88ce4b6a3b595498364a1f3639d2.png" alt="Cover for Hoping Against Hope" style="max-width:120px; height:auto;" />
      </td>
      <td>2017</td>
      <td>Thinking Plague</td>
      <td>Hoping Against Hope</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>56</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/55449f1faab1749bd7b05d1a88829e21.png" alt="Cover for Thin Black Duke" style="max-width:120px; height:auto;" />
      </td>
      <td>2017</td>
      <td>Oxbow</td>
      <td>Thin Black Duke</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>54</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/db164318ca5e1ddbc2fc1b6650e19a87.png" alt="Cover for 3 O&#39;Clock" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-12-11</td>
      <td>Blonde Redhead</td>
      <td>3 O&#39;Clock</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a> # 88</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>91</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1568882070335d86bec669c1f832df04.png" alt="Cover for Накидка" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-03-14</td>
      <td>Third Mind Movement</td>
      <td>Накидка</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/spoken-word" | relative_url }}">spoken word</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>55</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6d4f6d11a4e4426debb503afffddbafb.png" alt="Cover for Constance and the Waiting" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-06-19</td>
      <td>Troot</td>
      <td>Constance and the Waiting</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>100</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/49152c57edca8768851b862e47c954b5.png" alt="Cover for Презентация Подарка" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-09-28</td>
      <td>Third Mind Movement</td>
      <td>Презентация Подарка</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/neo-psychedelia" | relative_url }}">neo-psychedelia</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-pop" | relative_url }}">progressive pop</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>88</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2fb4da542fd62a95d9c9402f89940044.png" alt="Cover for Yuna" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-12-02</td>
      <td>Yagull</td>
      <td>Yuna</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/contemporary-folk" | relative_url }}">contemporary folk</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>73</td>
      <td>
          <img src="https://i.discogs.com/casvHPa6EBrwZQ_KHvyqX6sL1l575TcFI8qTBk9QOFM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE0NzYx/Nzc0LTE1ODEwOTQ4/NzMtMjMzMS5qcGVn.jpeg" alt="Cover for beko_80" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-02-07</td>
      <td>Ecovillage</td>
      <td>beko_80</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient-rock" | relative_url }}">ambient rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/duo" | relative_url }}">duo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/sweden" | relative_url }}">sweden</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/swedish" | relative_url }}">swedish</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ambient-rock" | relative_url }}">ambient rock</a> # 66</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>58</td>
      <td>
          <img src="https://i.discogs.com/SGn3aTTMUkzWy0HEzbGRkCQYN13-0gTlS2z763coqxQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE3Njgy/NjM0LTE2MTQ4NTM0/OTUtOTMwOC5qcGVn.jpeg" alt="Cover for Caress of Compassion" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-09-18</td>
      <td>Compassionizer</td>
      <td>Caress of Compassion</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/russia" | relative_url }}">russia</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>2</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2b1ed6bdaba7aa57d94c1bdfb805e972.png" alt="Cover for evermore (deluxe version)" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-12-18</td>
      <td>Taylor Swift</td>
      <td>evermore (deluxe version)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folk" | relative_url }}">folk</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/folk" | relative_url }}">folk</a> # 6</li>
            <li> <a href="{{ "/tags/chamber-pop" | relative_url }}">chamber pop</a> # 14</li>
            <li> <a href="{{ "/tags/folk-pop" | relative_url }}">folk pop</a> # 35</li>
            <li> <a href="{{ "/tags/indie-folk" | relative_url }}">indie folk</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>75</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/aaabbe086977e3a235bf75d13fe0dcbd.png" alt="Cover for Asceta" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-01-16</td>
      <td>Asceta</td>
      <td>Asceta</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>21</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ec9934ca4bdef2dce1121fffda766e0c.png" alt="Cover for Familia" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-06-02</td>
      <td>Sophie Ellis-Bextor</td>
      <td>Familia</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/latin" | relative_url }}">latin</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
            <li style="font-size: calc(1rem * 56.99999999999999 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 56.99999999999999 / 100)"> <a href="{{ "/tags/indie-pop" | relative_url }}">indie pop</a></li>
            <li style="font-size: calc(1rem * 56.99999999999999 / 100)"> <a href="{{ "/tags/singer-songwriter" | relative_url }}">singer-songwriter</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>14</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/acd444154dac98f45b7147a7ac759974.png" alt="Cover for Lament" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Lacrimosa</td>
      <td>Lament</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/gothic-metal" | relative_url }}">gothic metal</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/dark-rock" | relative_url }}">dark rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/dark-rock" | relative_url }}">dark rock</a> # 25</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>19</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/95fff3d5ce3544b7c4e0931334c8f874.png" alt="Cover for Штормовое предупреждение" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Flёur</td>
      <td>Штормовое предупреждение</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-pop" | relative_url }}">art pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/progressive-pop" | relative_url }}">progressive pop</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/symphonic-rock" | relative_url }}">symphonic rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/symphonic-rock" | relative_url }}">symphonic rock</a> # 76</li>
            <li> <a href="{{ "/tags/progressive-pop" | relative_url }}">progressive pop</a> # 92</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>27</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7691ed26e84040a3a80dd6d3c7d8f06f.png" alt="Cover for Albert Marcoeur" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Albert Marcoeur</td>
      <td>Albert Marcoeur</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/art-rock" | relative_url }}">art rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/prog-rock" | relative_url }}">prog-rock</a> # 39</li>
            <li> <a href="{{ "/tags/avant-prog" | relative_url }}">avant-prog</a> # 87</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>92</td>
      <td>
      </td>
      <td></td>
      <td>Marco Manzella e Antonella Scalia</td>
      <td>Oltre...</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient-rock" | relative_url }}">ambient rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/classical" | relative_url }}">classical</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/duo" | relative_url }}">duo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electroacoustic" | relative_url }}">electroacoustic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/europe" | relative_url }}">europe</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-rock" | relative_url }}">experimental rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/italian" | relative_url }}">italian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/italy" | relative_url }}">italy</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-rock" | relative_url }}">post-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/symphonic-rock" | relative_url }}">symphonic rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ambient-rock" | relative_url }}">ambient rock</a> # 98</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>4</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/be24c7e60d244da89410c91a892aed61.png" alt="Cover for No More Shall We Part" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Nick Cave &amp; the Bad Seeds</td>
      <td>No More Shall We Part</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 75 / 100)"> <a href="{{ "/tags/alternative-rock" | relative_url }}">alternative rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>71</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/03a551ddf70a9f9971db8510dca80073.png" alt="Cover for Telepass" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Les évadés</td>
      <td>Telepass</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/canada" | relative_url }}">canada</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/nu-jazz" | relative_url }}">nu jazz</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/fusion" | relative_url }}">fusion</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>87</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3a12c6b91e33ed5df8cec8f3eeb7e603.png" alt="Cover for Sarah Anne" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Torquem</td>
      <td>Sarah Anne</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/france" | relative_url }}">france</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>90</td>
      <td>
      </td>
      <td></td>
      <td>Debbie Chou</td>
      <td>Little Prince</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chamber-rock" | relative_url }}">chamber rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>84</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/271c55f9e4cb41bab0cee4e2ece239dc.png" alt="Cover for Тысяча светлых ангелов" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Flёur</td>
      <td>Тысяча светлых ангелов</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>98</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/cb0ba1ac4e317b33d7d21685dfd3d5b2.png" alt="Cover for Still. -Kaum Etwas Endet So, Wie Es Anfängt-" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Knoedel</td>
      <td>Still. -Kaum Etwas Endet So, Wie Es Anfängt-</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
  </tbody>
</table>

### Related genres

- [avant-prog]({{ "/tags/avant-prog" | relative_url }})
- [progressive rock]({{ "/tags/progressive-rock" | relative_url }})
- [post-rock]({{ "/tags/post-rock" | relative_url }})
- [art rock]({{ "/tags/art-rock" | relative_url }})
- [experimental rock]({{ "/tags/experimental-rock" | relative_url }})
- [switzerland]({{ "/tags/switzerland" | relative_url }})
- [ambient]({{ "/tags/ambient" | relative_url }})
- [instrumental]({{ "/tags/instrumental" | relative_url }})
- [chamber pop]({{ "/tags/chamber-pop" | relative_url }})
- [alternative rock]({{ "/tags/alternative-rock" | relative_url }})
