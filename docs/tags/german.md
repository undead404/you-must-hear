---
category: genre
date: 2025-12-06T03:26:45.900Z
permalink: tags/german
title: german
---

## german

Last updated: <time datetime="2025-12-06T03:26:45.900Z">Sat Dec 06 2025 05:26:45 GMT+0200 (Eastern European Standard Time)</time>

### Description

&quot;German&quot; refers to popular music sung in the German language, spanning rock, pop, punk, and metal styles that emerged prominently in the 1980s. The genre reflects German cultural identity and often features direct, poetic lyrics addressing social themes. It ranges from aggressive industrial metal to melodic pop-rock.

### 100 chronological releases

<table>
  <thead>
    <tr>
      <th>Place</th>
      <th>Cover</th>
      <th>Date</th>
      <th>Artist</th>
      <th>Album</th>
      <th>Tags</th>
      <th>Other places</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>62</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d0eb8c3f1cb329c8e6e406acf93bfcf1.png" alt="Cover for Tour De France (2009 Remaster)" style="max-width:120px; height:auto;" />
      </td>
      <td>1983</td>
      <td>Kraftwerk</td>
      <td>Tour De France (2009 Remaster)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>13</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b722159fd2a452f1eb8fc1a9d1a01736.png" alt="Cover for Nena" style="max-width:120px; height:auto;" />
      </td>
      <td>1983-01-14</td>
      <td>Nena</td>
      <td>Nena</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ndw" | relative_url }}">ndw</a> # 20</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>71</td>
      <td>
      </td>
      <td>1984</td>
      <td>Herbert Grönemeyer</td>
      <td>4630 Bochum</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>14</td>
      <td>
      </td>
      <td>1994</td>
      <td>Udo Jürgens</td>
      <td>Zeig mir den Platz an der Sonne</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/austrian" | relative_url }}">austrian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/schlager" | relative_url }}">schlager</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/austrian" | relative_url }}">austrian</a> # 1</li>
            <li> <a href="{{ "/tags/schlager" | relative_url }}">schlager</a> # 1</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>54</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a0cee992ae55b93a77684e1d5b7d3835.png" alt="Cover for Uuaarrgh!" style="max-width:120px; height:auto;" />
      </td>
      <td>1994</td>
      <td>Wizo</td>
      <td>Uuaarrgh!</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/punk-rock" | relative_url }}">punk rock</a></li>
            <li style="font-size: calc(1rem * 81 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 51 / 100)"> <a href="{{ "/tags/deutschpunk" | relative_url }}">deutschpunk</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/deutschpunk" | relative_url }}">deutschpunk</a> # 1</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>36</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fcfef96515de40d3bc3be86f76a43ad3.png" alt="Cover for Dish Of The Day" style="max-width:120px; height:auto;" />
      </td>
      <td>1995-12-07</td>
      <td>Fool&#39;s Garden</td>
      <td>Dish Of The Day</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/britpop" | relative_url }}">britpop</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/britpop" | relative_url }}">britpop</a> # 63</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>9</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/dac5b8cff0b2cabc2b2ce7248da214df.png" alt="Cover for Sehnsucht" style="max-width:120px; height:auto;" />
      </td>
      <td>1997-08-22</td>
      <td>Rammstein</td>
      <td>Sehnsucht</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a> # 3</li>
            <li> <a href="{{ "/tags/industrial" | relative_url }}">industrial</a> # 16</li>
            <li> <a href="{{ "/tags/metal" | relative_url }}">metal</a> # 86</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>64</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/82d816f7c84143c8abac1dfb8b946ae1.png" alt="Cover for Rock&#39;n&#39;Roll Realschule" style="max-width:120px; height:auto;" />
      </td>
      <td>1998</td>
      <td>Die Ärzte</td>
      <td>Rock&#39;n&#39;Roll Realschule</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>83</td>
      <td>
      </td>
      <td>1999</td>
      <td>Heinz Rudolf Kunze</td>
      <td>Nonstop</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>96</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bb69d501fa694847c85993e3770b1f22.png" alt="Cover for Tanzwut" style="max-width:120px; height:auto;" />
      </td>
      <td>1999-02-26</td>
      <td>Tanzwut</td>
      <td>Tanzwut</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>48</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ea8e8d938260f70bedfeb516c6c48195.png" alt="Cover for Liebe Ist für Alle Da (Special Edition)" style="max-width:120px; height:auto;" />
      </td>
      <td>1999-08-31</td>
      <td>Rammstein</td>
      <td>Liebe Ist für Alle Da (Special Edition)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a> # 18</li>
            <li> <a href="{{ "/tags/industrial" | relative_url }}">industrial</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>2</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0c275c90c66f4f4ef3f880864b2c4d62.png" alt="Cover for Mutter" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-03-27</td>
      <td>Rammstein</td>
      <td>Mutter</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a> # 1</li>
            <li> <a href="{{ "/tags/industrial" | relative_url }}">industrial</a> # 5</li>
            <li> <a href="{{ "/tags/metal" | relative_url }}">metal</a> # 21</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>22</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a70c6ccae8804635c628f3715926b7b1.png" alt="Cover for Endlich Urlaub!" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-10-22</td>
      <td>Farin Urlaub</td>
      <td>Endlich Urlaub!</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>43</td>
      <td>
          <img src="https://i.discogs.com/pJiaKqh2N-cH1yKgcYxIGe2mmGxFgXBuFwcMPwg-rrw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTY1MjE5/MS0xNTAyNDQyNTY3/LTQ1MDYuanBlZw.jpeg" alt="Cover for Noch 5 Minuten Mutti" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>Deichkind</td>
      <td>Noch 5 Minuten Mutti</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german-hip-hop" | relative_url }}">german hip hop</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/german-hip-hop" | relative_url }}">german hip hop</a> # 5</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>53</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bdfb5a4f68ce1d8c8da13b0b97ae979b.png" alt="Cover for Tocotronic" style="max-width:120px; height:auto;" />
      </td>
      <td>2002-06-10</td>
      <td>Tocotronic</td>
      <td>Tocotronic</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>59</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0352a40d59c44bb1d5aaa6b67c413540.png" alt="Cover for Mensch" style="max-width:120px; height:auto;" />
      </td>
      <td>2002-08-05</td>
      <td>Herbert Grönemeyer</td>
      <td>Mensch</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>25</td>
      <td>
      </td>
      <td>2002-10-28</td>
      <td>Kettcar</td>
      <td>Du Und Wieviel Von Deinen Freunden</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/hamburg" | relative_url }}">hamburg</a> # 6</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>40</td>
      <td>
          <img src="https://i.discogs.com/juXxpF_kjpd04M60yzhC40AE8rZyEqLqJBS7f1LZ3nI/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTU2ODAw/Ny0xNTE4NjMzMjI4/LTc2NjIuanBlZw.jpeg" alt="Cover for Guten Tag" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-02-03</td>
      <td>Wir sind Helden</td>
      <td>Guten Tag</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>4</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/db582554192c61db6bdcaab3c34dabe5.png" alt="Cover for Die Reklamation" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-07-04</td>
      <td>Wir sind Helden</td>
      <td>Die Reklamation</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 90 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>82</td>
      <td>
      </td>
      <td>2003-09-01</td>
      <td>Beginner</td>
      <td>Blast Action Heroes</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>8</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6608b6968d694c708b284f792150b44f.png" alt="Cover for Smack Smash" style="max-width:120px; height:auto;" />
      </td>
      <td>2004</td>
      <td>Beatsteaks</td>
      <td>Smack Smash</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/punk-rock" | relative_url }}">punk rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>79</td>
      <td>
          <img src="https://i.discogs.com/fUxKu5AZDsFvoQrOXavEQq0c2P6siTK39-Iiog7Jq1E/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEyNzIz/MDctMTQ1NzcyNDQ3/My0yNDQyLmpwZWc.jpeg" alt="Cover for old firehand ep" style="max-width:120px; height:auto;" />
      </td>
      <td>2004</td>
      <td>Museum</td>
      <td>old firehand ep</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>78</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4582cefa9b5376cbf156ccb9a44caba7.png" alt="Cover for Es wird Morgen" style="max-width:120px; height:auto;" />
      </td>
      <td>2004-08-30</td>
      <td>2raumwohnung</td>
      <td>Es wird Morgen</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>7</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/15cef01a698c40a2c0d209277dc8d643.png" alt="Cover for Es ist Juli" style="max-width:120px; height:auto;" />
      </td>
      <td>2004-09-20</td>
      <td>Juli</td>
      <td>Es ist Juli</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/deutschrock" | relative_url }}">deutschrock</a> # 20</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>27</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f3c7a5524680418f8a3c0c5a6878065a.png" alt="Cover for exit wounds ep" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>Museum</td>
      <td>exit wounds ep</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 73 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 51 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/germany" | relative_url }}">germany</a> # 42</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>89</td>
      <td>
      </td>
      <td>2005</td>
      <td>Culcha Candela</td>
      <td>Next Generation</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/reggae" | relative_url }}">reggae</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/berlin" | relative_url }}">berlin</a> # 18</li>
            <li> <a href="{{ "/tags/reggae" | relative_url }}">reggae</a> # 81</li>
            <li> <a href="{{ "/tags/dancehall" | relative_url }}">dancehall</a> # 96</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>73</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a43f669cbe0e4583b8f56580e197eaa1.png" alt="Cover for Drums Not Dead" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>Liars</td>
      <td>Drums Not Dead</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>80</td>
      <td>
      </td>
      <td>2005-01-17</td>
      <td>Tocotronic</td>
      <td>Pure Vernunft darf niemals siegen</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/berlin" | relative_url }}">berlin</a> # 44</li>
            <li> <a href="{{ "/tags/deutschrock" | relative_url }}">deutschrock</a> # 85</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>20</td>
      <td>
      </td>
      <td>2005-03-07</td>
      <td>Kettcar</td>
      <td>Von Spatzen und Tauben, Dächern und Händen</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>19</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/33bec6cdabc24538c5fe02170a07c098.png" alt="Cover for Am Wasser gebaut" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-03-21</td>
      <td>Fettes Brot</td>
      <td>Am Wasser gebaut</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>99</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/146f751a56e04674a19b7e2b7538feac.png" alt="Cover for Kamikazeherz" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-04-22</td>
      <td>Bosse</td>
      <td>Kamikazeherz</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/germany" | relative_url }}">germany</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hollywood" | relative_url }}">hollywood</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/hollywood" | relative_url }}">hollywood</a> # 2</li>
            <li> <a href="{{ "/tags/germany" | relative_url }}">germany</a> # 3</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>56</td>
      <td>
      </td>
      <td>2005-08-15</td>
      <td>Tokio Hotel</td>
      <td>Durch Den Monsun</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 54 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 54 / 100)"> <a href="{{ "/tags/pop-rock" | relative_url }}">pop rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>67</td>
      <td>
      </td>
      <td>2005-08-29</td>
      <td>Ohrbooten</td>
      <td>Spieltrieb</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/berlin" | relative_url }}">berlin</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/reggae" | relative_url }}">reggae</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/berlin" | relative_url }}">berlin</a> # 9</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>5</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1151d64f7fc05192728070512fc9cee3.png" alt="Cover for Von hier an blind" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-09-23</td>
      <td>Wir sind Helden</td>
      <td>Von hier an blind</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 79 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>70</td>
      <td>
      </td>
      <td>2005-10-07</td>
      <td>Muff Potter</td>
      <td>Von Wegen</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>12</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8a78bfaaa4d9471a9bc737cc2606bd6d.png" alt="Cover for Buchstaben über der Stadt" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-02-03</td>
      <td>Tomte</td>
      <td>Buchstaben über der Stadt</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>35</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/83e2edfc5a674e05aedcea100a151855.png" alt="Cover for Weit weg" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-05-19</td>
      <td>Clueso</td>
      <td>Weit weg</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>68</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/73083420db4c4ec89cbdc917fba162e7.png" alt="Cover for Mercedes Dance" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-08-04</td>
      <td>Jan Delay</td>
      <td>Mercedes Dance</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/reggae" | relative_url }}">reggae</a></li>
            <li style="font-size: calc(1rem * 86 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 75 / 100)"> <a href="{{ "/tags/funk" | relative_url }}">funk</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/hamburg" | relative_url }}">hamburg</a> # 43</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>32</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7168bd5894ff4a2dca78e952d241975f.png" alt="Cover for Fotos" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-09-29</td>
      <td>Fotos</td>
      <td>Fotos</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>47</td>
      <td>
          <img src="https://i.discogs.com/TY_OqNJP6PVxC8qpSMagw_O8uexWqBkpYEX8KnZCKb4/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIxNTI3/NDMxLTE2NDA3ODU5/MDEtODY5NC5wbmc.jpeg" alt="Cover for Türlich, türlich" style="max-width:120px; height:auto;" />
      </td>
      <td>2007</td>
      <td>Jan Delay</td>
      <td>Türlich, türlich</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>50</td>
      <td>
      </td>
      <td>2007-01-01</td>
      <td>Tokio Hotel</td>
      <td>Monsoon</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop-rock" | relative_url }}">pop rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>10</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/12d549702d354f81cf68be4af8bbd165.png" alt="Cover for Limbo Messiah" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-03-30</td>
      <td>Beatsteaks</td>
      <td>Limbo Messiah</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>42</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8c098e010eb7c93013a39dde8aa83935.png" alt="Cover for Somewhere Far Beyond (Remastered 2007)" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-06-15</td>
      <td>Blind Guardian</td>
      <td>Somewhere Far Beyond (Remastered 2007)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/heavy-metal" | relative_url }}">heavy metal</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/power-metal" | relative_url }}">power metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/epic-metal" | relative_url }}">epic metal</a> # 14</li>
            <li> <a href="{{ "/tags/speed-metal" | relative_url }}">speed metal</a> # 65</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>61</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2154dd35f55b45b2ac6da23834028dec.png" alt="Cover for Vom selben Stern" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-06-29</td>
      <td>Ich + Ich</td>
      <td>Vom selben Stern</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/deutschrock" | relative_url }}">deutschrock</a> # 77</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>49</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/98cdc3945de9d8fbe7bcf95eacd257e7.png" alt="Cover for Made In Germany 1995 - 2011 (Special Edition)" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-09-18</td>
      <td>Rammstein</td>
      <td>Made In Germany 1995 - 2011 (Special Edition)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/metal" | relative_url }}">metal</a> # 57</li>
            <li> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a> # 79</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>29</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e915972c9da4d941a00a6bab4202b7b3.png" alt="Cover for Lustprinzip" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-10-26</td>
      <td>Egotronic</td>
      <td>Lustprinzip</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/electropunk" | relative_url }}">electropunk</a> # 12</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>60</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c0a4860813e14baa4daa46a6c5a932d5.png" alt="Cover for Ins offene Messer" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-02-15</td>
      <td>Jennifer Rostock</td>
      <td>Ins offene Messer</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/germany" | relative_url }}">germany</a> # 65</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>91</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ac2857ad3cc9eb4fa082974a49ae0f79.png" alt="Cover for Strom Und Drang" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-03-14</td>
      <td>Fettes Brot</td>
      <td>Strom Und Drang</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>16</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7e0453464c5640569d2dd1b3694e4e7f.png" alt="Cover for The Devil, You + Me" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-05-02</td>
      <td>The Notwist</td>
      <td>The Devil, You + Me</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/germany" | relative_url }}">germany</a> # 27</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>57</td>
      <td>
          <img src="https://i.discogs.com/AmiZG5OmSqObBrcRGiiCd8dqBBhzbne32a3wGoFOuG8/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTU1NzUz/MzMtMTM5Njk3Njg0/NC05MDE1LmpwZWc.jpeg" alt="Cover for Suende" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-08-22</td>
      <td>Eisbrecher</td>
      <td>Suende</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/industrial" | relative_url }}">industrial</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>69</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/48e54a65f6fe47e49b1ac22ef998aae2.png" alt="Cover for Licht" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-08-29</td>
      <td>Die Apokalyptischen Reiter</td>
      <td>Licht</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/epic-metal" | relative_url }}">epic metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-death-metal" | relative_url }}">melodic death metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/epic-metal" | relative_url }}">epic metal</a> # 3</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>21</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4f2215b8f5da47b88388ec67d621b495.png" alt="Cover for Stadtaffe" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-09-26</td>
      <td>Peter Fox</td>
      <td>Stadtaffe</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dancehall" | relative_url }}">dancehall</a></li>
            <li style="font-size: calc(1rem * 51 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/dancehall" | relative_url }}">dancehall</a> # 4</li>
            <li> <a href="{{ "/tags/strings" | relative_url }}">strings</a> # 78</li>
            <li> <a href="{{ "/tags/orchestra" | relative_url }}">orchestra</a> # 89</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>100</td>
      <td>
      </td>
      <td>2008-11-14</td>
      <td>Tomte</td>
      <td>Heureka</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>26</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bc4267da68304d008e298c53580bc305.png" alt="Cover for Stamp On The Ground" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Italobrothers</td>
      <td>Stamp On The Ground</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/techno" | relative_url }}">techno</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/trance" | relative_url }}">trance</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/techno" | relative_url }}">techno</a> # 44</li>
            <li> <a href="{{ "/tags/trance" | relative_url }}">trance</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>85</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/807905e9b8414432fadd899f16e9a467.png" alt="Cover for Boheme" style="max-width:120px; height:auto;" />
      </td>
      <td>2009</td>
      <td>Annett Louisan</td>
      <td>Boheme</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 81 / 100)"> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/deutschpop" | relative_url }}">deutschpop</a> # 48</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>30</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/aa115a5498b94d68b79b732724625904.png" alt="Cover for Rules" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-02-27</td>
      <td>The Whitest Boy Alive</td>
      <td>Rules</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 65 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>34</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/915844ddc72c43c591eb43babeacd1ed.png" alt="Cover for Unter dem Eis" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-03-06</td>
      <td>Eisblume</td>
      <td>Unter dem Eis</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>97</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/442e3c1225f042a7bf26b4be9eec4426.png" alt="Cover for Dis wo ich herkomm" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-03-27</td>
      <td>Samy Deluxe</td>
      <td>Dis wo ich herkomm</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/hamburg" | relative_url }}">hamburg</a> # 19</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>46</td>
      <td>
          <img src="https://i.discogs.com/EYsrzVxwMS6SouBxwDgCocDBYOM1iG6qDkPTa6vrg8M/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIwMzM3/MDctMTQ0NDkwOTMx/OS0xNjg4LmpwZWc.jpeg" alt="Cover for Schöne Neue Welt" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-08-28</td>
      <td>Culcha Candela</td>
      <td>Schöne Neue Welt</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dancehall" | relative_url }}">dancehall</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/reggae" | relative_url }}">reggae</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/dancehall" | relative_url }}">dancehall</a> # 24</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>37</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/283d8cfb05a8b9bf8f555f58a1e7f6b7.png" alt="Cover for Stars Ep" style="max-width:120px; height:auto;" />
      </td>
      <td>2010</td>
      <td>BEAT!BEAT!BEAT!</td>
      <td>Stars Ep</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/britpop" | relative_url }}">britpop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-pop" | relative_url }}">indie pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>94</td>
      <td>
      </td>
      <td>2010-02-26</td>
      <td>Oomph!</td>
      <td>Truth or Dare</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>88</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/eda3e418d91d4a3785831644664af49c.png" alt="Cover for Katzengold" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-05-28</td>
      <td>Frittenbude</td>
      <td>Katzengold</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/new-wave" | relative_url }}">new wave</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>17</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/68ec0581a6947953cda563f86082ba5b.png" alt="Cover for Bring mich nach Hause" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-08-27</td>
      <td>Wir sind Helden</td>
      <td>Bring mich nach Hause</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>55</td>
      <td>
      </td>
      <td>2010-12-10</td>
      <td>Italobrothers</td>
      <td>Stamp!</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>98</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/39615bc32c1f43d1ad4bf5c7ab82aeb5.png" alt="Cover for I Love You, Dude" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-06-15</td>
      <td>Digitalism</td>
      <td>I Love You, Dude</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/indie-dance" | relative_url }}">indie dance</a> # 21</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>45</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d7a573a370ec4486c45263224b00085a.png" alt="Cover for Die Hölle muss warten" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-02-03</td>
      <td>Eisbrecher</td>
      <td>Die Hölle muss warten</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>38</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4687f80216364d30a87dabaeca304c79.png" alt="Cover for auch" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-04-13</td>
      <td>Die Ärzte</td>
      <td>auch</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>65</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3e1a1f8dae8349af865d109c1aa2796e.png" alt="Cover for &quot;Ballast der Republik&quot; plus Jubiläums-Album &quot;Die Geister, die wir riefen&quot;" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-05-04</td>
      <td>Die Toten Hosen</td>
      <td>&quot;Ballast der Republik&quot; plus Jubiläums-Album &quot;Die Geister, die wir riefen&quot;</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>58</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/218a89dd7da64dd49b067d97e1b15b9f.png" alt="Cover for Whatever (Maxi Edition)" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-06-28</td>
      <td>Cro</td>
      <td>Whatever (Maxi Edition)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german-hip-hop" | relative_url }}">german hip hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/german-hip-hop" | relative_url }}">german hip hop</a> # 7</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>72</td>
      <td>
          <img src="https://i.discogs.com/BgaPQCD05bZGqWd06kRTvlvX6BaGLezzGCvgBkwnLPE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTQ3NzEy/MDUtMTM3NTAwNjIx/My0zMDQ0LmpwZWc.jpeg" alt="Cover for Sommer, Sonne, Kaktus!" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-07</td>
      <td>Helge Schneider</td>
      <td>Sommer, Sonne, Kaktus!</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/comedy" | relative_url }}">comedy</a> # 49</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>81</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5920e075d3d9427a93595d49d38a8573.png" alt="Cover for Triebwerke" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-08-02</td>
      <td>Alligatoah</td>
      <td>Triebwerke</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a> # 7</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>24</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6acd50c04f57408b9d22a0263f4ae000.png" alt="Cover for Hinterland" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-09-06</td>
      <td>Casper</td>
      <td>Hinterland</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>18</td>
      <td>
      </td>
      <td>2013-10-11</td>
      <td>Jupiter Jones</td>
      <td>Das Gegenteil von Allem</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>95</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/dc19a9b2016246fec628e87095ad6085.png" alt="Cover for Close To The Glass" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-02-12</td>
      <td>The Notwist</td>
      <td>Close To The Glass</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>75</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1511d08a4b7a42dc3aa48c482c497c41.png" alt="Cover for In Schwarz" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-09-12</td>
      <td>Kraftklub</td>
      <td>In Schwarz</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>15</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1ce47cb608864cf2cc087749ab92866d.png" alt="Cover for Unity" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-09-26</td>
      <td>TheFatRat</td>
      <td>Unity</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/britpop" | relative_url }}">britpop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/chiptune" | relative_url }}">chiptune</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/glitch-hop" | relative_url }}">glitch hop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/chiptune" | relative_url }}">chiptune</a> # 7</li>
            <li> <a href="{{ "/tags/glitch-hop" | relative_url }}">glitch hop</a> # 10</li>
            <li> <a href="{{ "/tags/britpop" | relative_url }}">britpop</a> # 82</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>76</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d84fe7ae6ac54c87cca718f4390af559.png" alt="Cover for Alles brennt" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-10-31</td>
      <td>Zugezogen Maskulin</td>
      <td>Alles brennt</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/berlin" | relative_url }}">berlin</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/berlin" | relative_url }}">berlin</a> # 2</li>
            <li> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a> # 6</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>90</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/92e1033210c14810c894271aaeabb3c6.png" alt="Cover for Crackstreet Boys II X Version" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-12-05</td>
      <td>Trailerpark</td>
      <td>Crackstreet Boys II X Version</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a> # 8</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>87</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a273543cfd08d8de97a03e04acd7eb2a.png" alt="Cover for Chrome - EP" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-04-24</td>
      <td>DAT ADAM</td>
      <td>Chrome - EP</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/cloud-rap" | relative_url }}">cloud rap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>93</td>
      <td>
          <img src="https://i.discogs.com/D99-d21cVT_YCNb2C9tSJR7-qy8k7P0G9GBLNB0Qr5U/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTcwMzg2/NDMtMTQzMjI4MDYw/Ni0zNDY3LmpwZWc.jpeg" alt="Cover for Wieder geil!" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-05-22</td>
      <td>We Butter the Bread With Butter</td>
      <td>Wieder geil!</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/deathcore" | relative_url }}">deathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/electronicore" | relative_url }}">electronicore</a> # 23</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>6</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fc9f279785f4a4218485ae1353a64aee.png" alt="Cover for MTV Unplugged" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-07-03</td>
      <td>Cro</td>
      <td>MTV Unplugged</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german-hip-hop" | relative_url }}">german hip hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/german-hip-hop" | relative_url }}">german hip hop</a> # 1</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>86</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5d8a452f9706fc7b7418c3b946880f0b.png" alt="Cover for Coma Blue" style="max-width:120px; height:auto;" />
      </td>
      <td>2016</td>
      <td>Annisokay</td>
      <td>Coma Blue</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/emo" | relative_url }}">emo</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-hardcore" | relative_url }}">post-hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metalcore" | relative_url }}">progressive metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/progressive-metalcore" | relative_url }}">progressive metalcore</a> # 28</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>52</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a93b22958217b8d0b8b46aa4846eca11.png" alt="Cover for Uncovered" style="max-width:120px; height:auto;" />
      </td>
      <td>2017</td>
      <td>Robin Schulz</td>
      <td>Uncovered</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electro" | relative_url }}">electro</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/house" | relative_url }}">house</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/techno" | relative_url }}">techno</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/techno" | relative_url }}">techno</a> # 73</li>
            <li> <a href="{{ "/tags/electro" | relative_url }}">electro</a> # 97</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>3</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b8d877c1ed295ef441cb65a446bfa5b3.png" alt="Cover for No Roots" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-02-03</td>
      <td>Alice Merton</td>
      <td>No Roots</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-pop" | relative_url }}">indie pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/female-vocalists" | relative_url }}">female vocalists</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>23</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/afca227aa8393333cb2e750cb6d4f929.png" alt="Cover for Blossom (Deluxe)" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-03</td>
      <td>Milky Chance</td>
      <td>Blossom (Deluxe)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-rock" | relative_url }}">indie rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/reggae" | relative_url }}">reggae</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/singer-songwriter" | relative_url }}">singer-songwriter</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/reggae" | relative_url }}">reggae</a> # 89</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>1</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/43c0edef306cf09b0f9b70e2a389a20a.png" alt="Cover for Summer Air" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-08-04</td>
      <td>Italobrothers</td>
      <td>Summer Air</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/techno" | relative_url }}">techno</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/trance" | relative_url }}">trance</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/trance" | relative_url }}">trance</a> # 1</li>
            <li> <a href="{{ "/tags/techno" | relative_url }}">techno</a> # 6</li>
            <li> <a href="{{ "/tags/dance" | relative_url }}">dance</a> # 42</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>63</td>
      <td>
          <img src="https://i.discogs.com/7hi52jIilwhuY94cB_QbrrfeOoyZLxle4U83gMpGNAo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwODQ3/NTM3LTE1MDUyODk2/NjQtNjc2Mi5qcGVn.jpeg" alt="Cover for #DIY" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-09-29</td>
      <td>trettmann</td>
      <td>#DIY</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german-rap" | relative_url }}">german rap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/german-rap" | relative_url }}">german rap</a> # 1</li>
            <li> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a> # 4</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>41</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bd27500b4712fb169201d942da5ac8a6.png" alt="Cover for I" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-02-16</td>
      <td>Felix Jaehn</td>
      <td>I</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/house" | relative_url }}">house</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/tropical-house" | relative_url }}">tropical house</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/tropical-house" | relative_url }}">tropical house</a> # 36</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>33</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8527b9b0b76fa0a74a56b7b021098b2d.png" alt="Cover for Chabos wissen wer der Babo ist" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-02-23</td>
      <td>Haftbefehl</td>
      <td>Chabos wissen wer der Babo ist</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german-hip-hop" | relative_url }}">german hip hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a> # 2</li>
            <li> <a href="{{ "/tags/german-hip-hop" | relative_url }}">german hip hop</a> # 4</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>28</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f849c69aa8e059da90f64f95b70f1675.png" alt="Cover for They Might&#39;ve Even Loved Me" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-03-23</td>
      <td>NoMBe</td>
      <td>They Might&#39;ve Even Loved Me</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/indie-pop" | relative_url }}">indie pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>11</td>
      <td>
          <img src="https://i.discogs.com/v2wWJQ93maztOi6RGigi1ntrROoipPEHDiV8uAcMotE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTExODYw/Mjg3LTE1MjM2MzQ3/MzMtNTEzOC5qcGVn.jpeg" alt="Cover for Treehouse" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-04-13</td>
      <td>Sofi Tukker</td>
      <td>Treehouse</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>66</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d6a72e13bc6544bfc66913be346fb987.png" alt="Cover for Savior" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-03-15</td>
      <td>Any Given Day</td>
      <td>Savior</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/christian" | relative_url }}">christian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/christian-metalcore" | relative_url }}">christian metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/christian-rock" | relative_url }}">christian rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-metalcore" | relative_url }}">melodic metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/christian-metalcore" | relative_url }}">christian metalcore</a> # 4</li>
            <li> <a href="{{ "/tags/christian" | relative_url }}">christian</a> # 25</li>
            <li> <a href="{{ "/tags/melodic-metalcore" | relative_url }}">melodic metalcore</a> # 25</li>
            <li> <a href="{{ "/tags/christian-rock" | relative_url }}">christian rock</a> # 40</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>74</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/27bf61ef89cf4ce1ccd72f9d7993c30f.png" alt="Cover for Niveau Weshalb Warum" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-08-16</td>
      <td>Deichkind</td>
      <td>Niveau Weshalb Warum</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>84</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/16e96b3df1d231d979bde8ec35648394.png" alt="Cover for Das ist alles von der Kunstfreiheit gedeckt" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-04-30</td>
      <td>Danger Dan</td>
      <td>Das ist alles von der Kunstfreiheit gedeckt</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>92</td>
      <td>
      </td>
      <td>2022</td>
      <td>Lorenz Büffel</td>
      <td>Johnny Däpp</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>77</td>
      <td>
          <img src="https://i.discogs.com/ygXP2rJ1S0C_dsjR_aWMDJNQgRNj-PeQ2MBe2dlvtlM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTU2Mjc5/NC0xMjA2NDgzNTQ2/LmpwZWc.jpeg" alt="Cover for Ich will nur dass du weißt" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-01-27</td>
      <td>SDP</td>
      <td>Ich will nur dass du weißt</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/berlin" | relative_url }}">berlin</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/berlin" | relative_url }}">berlin</a> # 3</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>31</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3e4a28cb8b7d431db65cbe186b24c705.png" alt="Cover for Lila Wolken" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-11-17</td>
      <td>Marteria</td>
      <td>Lila Wolken</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/deutschrap" | relative_url }}">deutschrap</a> # 1</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>51</td>
      <td>
          <img src="https://i.discogs.com/91dviHomhuUGo_DpsUbRJU82CNcftN3iBX-kMia5u1I/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTExMjA2/ODctMTIxMTIyMTQ3/OC5qcGVn.jpeg" alt="Cover for Eishockey Hits" style="max-width:120px; height:auto;" />
      </td>
      <td>2023</td>
      <td>Various Artists</td>
      <td>Eishockey Hits</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>39</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/adbb89e74bbac0af039c55d6fc19be9c.png" alt="Cover for Palmen aus Plastik 2" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-06-01</td>
      <td>Bonez MC</td>
      <td>Palmen aus Plastik 2</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>44</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1c083113c276431a31734b0525f20131.png" alt="Cover for Diamonds" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-01-26</td>
      <td>Any Given Day</td>
      <td>Diamonds</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/christian" | relative_url }}">christian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/christian-metalcore" | relative_url }}">christian metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/christian-rock" | relative_url }}">christian rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/german" | relative_url }}">german</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-metalcore" | relative_url }}">melodic metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/christian-metalcore" | relative_url }}">christian metalcore</a> # 3</li>
            <li> <a href="{{ "/tags/melodic-metalcore" | relative_url }}">melodic metalcore</a> # 16</li>
            <li> <a href="{{ "/tags/christian" | relative_url }}">christian</a> # 17</li>
            <li> <a href="{{ "/tags/christian-rock" | relative_url }}">christian rock</a> # 28</li>
        </ul>
      </td>
    </tr>
  </tbody>
</table>

### Related genres

- [rock]({{ "/tags/rock" | relative_url }})
- [electronic]({{ "/tags/electronic" | relative_url }})
- [pop]({{ "/tags/pop" | relative_url }})
- [punk rock]({{ "/tags/punk-rock" | relative_url }})
- [metal]({{ "/tags/metal" | relative_url }})
- [germany]({{ "/tags/germany" | relative_url }})
- [rap]({{ "/tags/rap" | relative_url }})
- [alternative]({{ "/tags/alternative" | relative_url }})
- [hip-hop]({{ "/tags/hip-hop" | relative_url }})
- [deutschrock]({{ "/tags/deutschrock" | relative_url }})
