---
category: genre
date: 2025-12-05T10:09:46.456Z
permalink: tags/korean
title: korean
---

## korean

Last updated: <time datetime="2025-12-05T10:09:46.456Z">Fri Dec 05 2025 12:09:46 GMT+0200 (Eastern European Standard Time)</time>

### Description

Korean pop, or K-pop, blends electronic dance music, hip-hop, and R&amp;B with meticulously produced vocals and synchronized choreography. Originating from South Korea&#39;s entertainment industry, it emphasizes visual spectacle, catchy hooks, and highly trained idol groups. The genre has become a global phenomenon through its polished production and dedicated fan engagement.

### 100 chronological releases

<table>
  <thead>
    <tr>
      <th>Place</th>
      <th>Cover</th>
      <th>Date</th>
      <th>Artist</th>
      <th>Album</th>
      <th>Tags</th>
      <th>Other places</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>34</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/adfd483aefff814f0b458cc0e6af917a.png" alt="Cover for SEVENTEEN 4th Mini Album ‘Al1’" style="max-width:120px; height:auto;" />
      </td>
      <td>2000</td>
      <td>Seventeen</td>
      <td>SEVENTEEN 4th Mini Album ‘Al1’</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/asian" | relative_url }}">asian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/asian-pop" | relative_url }}">asian pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/asian-pop" | relative_url }}">asian pop</a> # 2</li>
            <li> <a href="{{ "/tags/asian" | relative_url }}">asian</a> # 2</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>7</td>
      <td>
          <img src="https://i.discogs.com/HTvlz1oykB5kXe0_bqnD2pYZfcepCseBdp52OGTCbXw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIxMjU1/NTE0LTE2NDAzMzE4/NTItODU2OC5qcGVn.jpeg" alt="Cover for ELEVEN" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-07-14</td>
      <td>Ive</td>
      <td>ELEVEN</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 16</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>51</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d953e475498170491ae8de9529c9a668.png" alt="Cover for ˣ‿ˣ (SMiLEY)" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>Yena</td>
      <td>ˣ‿ˣ (SMiLEY)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop-rock" | relative_url }}">pop rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/pop-rock" | relative_url }}">pop rock</a> # 46</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>49</td>
      <td>
          <img src="https://i.discogs.com/_Ml50wPVTuiQMWADkTRlBeE4FlqfCLCGKSF92exMOIw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwMzc5/ODE5LTE0OTYzMjEw/NjEtOTU2Ny5qcGVn.jpeg" alt="Cover for Birthday" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-01-21</td>
      <td>Somi</td>
      <td>Birthday</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/edm" | relative_url }}">edm</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/edm" | relative_url }}">edm</a> # 6</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>64</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/14be12ed3b7f1430db734c34fca0d63e.png" alt="Cover for Goblin" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-01-23</td>
      <td>Sulli</td>
      <td>Goblin</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative" | relative_url }}">alternative</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient-pop" | relative_url }}">ambient pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dream-pop" | relative_url }}">dream pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ambient-pop" | relative_url }}">ambient pop</a> # 7</li>
            <li> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a> # 10</li>
            <li> <a href="{{ "/tags/dream-pop" | relative_url }}">dream pop</a> # 48</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>93</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/249aa374d6b7c1bf7ed4c8685216a090.png" alt="Cover for V" style="max-width:120px; height:auto;" />
      </td>
      <td>2014</td>
      <td>Jay Park</td>
      <td>V</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>33</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e5f51d0b8cc478922d7d2aa5436a5fe6.png" alt="Cover for The Planet" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-11-19</td>
      <td>BTS</td>
      <td>The Planet</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 100</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>15</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d28320c78ab10d38ad55c8be8fc21374.png" alt="Cover for The Most Beautiful Moment In Life Pt.2" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-11-30</td>
      <td>BTS</td>
      <td>The Most Beautiful Moment In Life Pt.2</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 50</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>2</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5c8c46eaf900eb735b2464ceeda708f1.png" alt="Cover for BE" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-03-15</td>
      <td>BTS</td>
      <td>BE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 7</li>
            <li> <a href="{{ "/tags/pop" | relative_url }}">pop</a> # 9</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>27</td>
      <td>
          <img src="https://i.discogs.com/XmpunCd6OfvueaTEMSgd1wMRsaO-7gtcqiun1ZBVq98/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTkyOTQ3/NTAtMTQ3ODExMzUz/Mi02MTMyLnBuZw.jpeg" alt="Cover for Page Two" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-04-25</td>
      <td>TWICE</td>
      <td>Page Two</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>94</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0663d55f348e97e2260115ddb476a957.png" alt="Cover for Love&amp;Letter Repackage Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-04-25</td>
      <td>Seventeen</td>
      <td>Love&amp;Letter Repackage Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>17</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/95b042ed368f9b5d36060504519a96ce.png" alt="Cover for The Most Beautiful Moment In Life Pt.1" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-05-02</td>
      <td>BTS</td>
      <td>The Most Beautiful Moment In Life Pt.1</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>32</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ad2f42cfa70d7fe14ca91ae784cc38b8.png" alt="Cover for [+ +]" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-05-27</td>
      <td>Loona</td>
      <td>[+ +]</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>47</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d69c5ca95d0b1aac216b3cee5d9bf3b3.png" alt="Cover for We BOOM - The 3rd Mini Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-08-27</td>
      <td>NCT DREAM</td>
      <td>We BOOM - The 3rd Mini Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rnb" | relative_url }}">rnb</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/dance" | relative_url }}">dance</a> # 63</li>
            <li> <a href="{{ "/tags/rnb" | relative_url }}">rnb</a> # 96</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>68</td>
      <td>
          <img src="https://i.discogs.com/x9L1AWKfXC0e9HWGhPYJznny9vM8TzKXsVPCQu8kG2k/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwOTcy/ODYzLTE1MDc0Njc5/MjUtMzIzNi5qcGVn.jpeg" alt="Cover for Miss Me?" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-10-19</td>
      <td>I.O.I</td>
      <td>Miss Me?</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>35</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d8d279f03485bc6df179d0fcf4bb08f0.png" alt="Cover for Palette" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-04-21</td>
      <td>IU</td>
      <td>Palette</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ballad" | relative_url }}">ballad</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ballad" | relative_url }}">ballad</a> # 3</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>39</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3139556807ae012f06d65ed1e2a7233c.png" alt="Cover for NCT#127 LIMITLESS - The 2nd Mini Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-06-14</td>
      <td>NCT 127</td>
      <td>NCT#127 LIMITLESS - The 2nd Mini Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>46</td>
      <td>
          <img src="https://i.discogs.com/RKCAYIlg7W21zxtesf3v6ebZmAJ1X85URwtiWpVOjNc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwOTU1/MjM1LTE1OTU0MzY1/NjAtOTk0MS5wbmc.jpeg" alt="Cover for Love Yourself 承 &#39;Her&#39;" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-09-18</td>
      <td>BTS</td>
      <td>Love Yourself 承 &#39;Her&#39;</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 33</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>80</td>
      <td>
          <img src="https://i.discogs.com/uGpPR9-q11myj2gYwc-VQwnJjGLMtaXr__g7PMzXJHI/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIwMTQx/NDA0LTE2NjQ0NTkz/NDUtMjM5NS5qcGVn.jpeg" alt="Cover for MIXTAPE" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-01-08</td>
      <td>STRAY KIDS</td>
      <td>MIXTAPE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rock" | relative_url }}">rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>76</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/05ed3fae3b45b2fcda6c80e76c4c8aed.png" alt="Cover for Agust D" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-02-21</td>
      <td>Agust D</td>
      <td>Agust D</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/rap" | relative_url }}">rap</a> # 50</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>99</td>
      <td>
          <img src="https://i.discogs.com/dZhrFtFlPiLk-ZeJ_NWntrFp39takz1LgPugcSlekbc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzMjU3/NjgyLTE1NTA4NjM5/MjEtNzkwNi5qcGVn.jpeg" alt="Cover for I AM NOT" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-03-28</td>
      <td>STRAY KIDS</td>
      <td>I AM NOT</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop-rock" | relative_url }}">pop rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/pop-rock" | relative_url }}">pop rock</a> # 84</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>60</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a2cd62957d2fbaed096f4d88bb6a6a95.png" alt="Cover for Red Moon" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-07-16</td>
      <td>MAMAMOO</td>
      <td>Red Moon</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>20</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fade42d3959e3671e66d5448efca2d54.png" alt="Cover for SQUARE ONE" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-08-08</td>
      <td>BLACKPINK</td>
      <td>SQUARE ONE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 31</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>97</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c6bb70bc982139e52813d67b60c329a3.png" alt="Cover for SUNRISE" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-10-17</td>
      <td>DAY6</td>
      <td>SUNRISE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-rock" | relative_url }}">k-rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-rock" | relative_url }}">k-rock</a> # 2</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>48</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ef194a5f86210595505acd7ca2b5db7d.png" alt="Cover for TREASURE EP.2: Zero to One" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-01-15</td>
      <td>ATEEZ</td>
      <td>TREASURE EP.2: Zero to One</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>67</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/13b92f58d904682dbda17e601ecf403e.png" alt="Cover for Song Request 신청곡 (feat. SUGA of BTS)" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-01-22</td>
      <td>Lee So Ra</td>
      <td>Song Request 신청곡 (feat. SUGA of BTS)</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ballad" | relative_url }}">ballad</a> # 5</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>92</td>
      <td>
          <img src="https://i.discogs.com/xlKbmyc1aA6F-9-w4U1HoNMuoZA0jKPzwHfEX_fvLdc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1MDcw/NTY4LTE1ODYyODg4/MDEtMzc2NS5qcGVn.jpeg" alt="Cover for IT&#39;Z DIFFERENT" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-02-12</td>
      <td>Itzy</td>
      <td>IT&#39;Z DIFFERENT</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 30</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>96</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3037cd10162ccdc45eddec7271f87f46.png" alt="Cover for HEART*IZ" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-04-02</td>
      <td>IZ*ONE</td>
      <td>HEART*IZ</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dance" | relative_url }}">dance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/dance" | relative_url }}">dance</a> # 69</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>88</td>
      <td>
          <img src="https://i.discogs.com/_YVaPKohxk-oUkwM_tPsMcUi7JN4KubjynokQdle7aA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzNzg3/ODMyLTE1NjExMzIw/NDktMjA1Ni5qcGVn.jpeg" alt="Cover for Clé 2 : Yellow Wood" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-06-19</td>
      <td>STRAY KIDS</td>
      <td>Clé 2 : Yellow Wood</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>11</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b0bcf147a98ee8609d584be18627fb29.png" alt="Cover for Uh-Oh" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-06-26</td>
      <td>(G)I-DLE</td>
      <td>Uh-Oh</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a> # 23</li>
            <li> <a href="{{ "/tags/rap" | relative_url }}">rap</a> # 23</li>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 37</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>58</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0953dec93349b400ff4bc3249caacde3.png" alt="Cover for TREASURE EP.FIN: All To Action" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-10-09</td>
      <td>ATEEZ</td>
      <td>TREASURE EP.FIN: All To Action</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>45</td>
      <td>
          <img src="https://i.discogs.com/KcvZo0qhtfr4ocfMxL_6BtsdUvR7HQI1r54fGlAMaNE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwNTI5/NDI3LTE0OTk0NzE1/MDgtOTM1OS5qcGVn.jpeg" alt="Cover for TWICETAGRAM" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-11-20</td>
      <td>TWICE</td>
      <td>TWICETAGRAM</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>61</td>
      <td>
          <img src="https://i.discogs.com/VpLLxETkUCxQ7YxpneMSCDztOCIxqjmBO4DPDRZPCLg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE0NjQz/NTEyLTE1Nzg3NzYw/MjYtODU5My5qcGVn.jpeg" alt="Cover for Clé : Levanter" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-12-09</td>
      <td>STRAY KIDS</td>
      <td>Clé : Levanter</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>98</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c76ca4d7659292b13295c79ea9180713.png" alt="Cover for BLOOM*IZ" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-02-18</td>
      <td>IZ*ONE</td>
      <td>BLOOM*IZ</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 68 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>8</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d5c851fbcd5e26877735d694202b5b60.png" alt="Cover for KAZINO" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-04-29</td>
      <td>Bibi</td>
      <td>KAZINO</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>52</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b330dbdc1b9f9fa20057f1681beb6122.png" alt="Cover for Reload" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-04-29</td>
      <td>NCT DREAM</td>
      <td>Reload</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>50</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2998abc4c73550e78749e21846afa431.png" alt="Cover for D-2" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-05-22</td>
      <td>Agust D</td>
      <td>D-2</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/rap" | relative_url }}">rap</a> # 20</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>23</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f5fd1e7141882412fa11fb15cd886264.png" alt="Cover for Monster - The 1st Mini Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-07-06</td>
      <td>Red Velvet - IRENE &amp; SEULGI</td>
      <td>Monster - The 1st Mini Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>36</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/599352c69ff9fc684cddb7e2dee19b93.png" alt="Cover for DUMDi DUMDi" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-08-03</td>
      <td>(G)I-DLE</td>
      <td>DUMDi DUMDi</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>14</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/fa216d5b637d25cf5fcbae07e633061d.png" alt="Cover for -77.82X-78.29" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-09-21</td>
      <td>Everglow</td>
      <td>-77.82X-78.29</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 48</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>65</td>
      <td>
          <img src="https://i.discogs.com/TJuRaub-cJJG9zI225fiBEgfc6_--uLMvrcgT9GqWQQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1OTk4/MzE4LTE2MDE2NDU3/NDAtMjc3OC5wbmc.jpeg" alt="Cover for The Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-10-02</td>
      <td>BLACKPINK</td>
      <td>The Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/dance-pop" | relative_url }}">dance-pop</a> # 9</li>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 18</li>
            <li> <a href="{{ "/tags/electropop" | relative_url }}">electropop</a> # 45</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>90</td>
      <td>
          <img src="https://i.discogs.com/B4dEO4R_LwW3pad0PvxPRYVHFyPdYkYqtGO7fKJkSUE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTk4Nzk0/MDEtMTQ4ODUwODc5/Ny00OTEyLmpwZWc.jpeg" alt="Cover for You Never Walk Alone" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-11-20</td>
      <td>BTS</td>
      <td>You Never Walk Alone</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 61</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>40</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/803f9874bdd2bcb58c7a8116303bac82.png" alt="Cover for One-reeler &#x2F; Act IV" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-12-08</td>
      <td>IZ*ONE</td>
      <td>One-reeler &#x2F; Act IV</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>70</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/825b700c1ff1852b6f9a8aa1ebde500f.png" alt="Cover for [Dystopia : Road to Utopia]" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-01-26</td>
      <td>Dreamcatcher</td>
      <td>[Dystopia : Road to Utopia]</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>54</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/cbf41475d0f9024bd831ab2ca35df252.png" alt="Cover for Eenie Meenie" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-02-15</td>
      <td>CHUNG HA</td>
      <td>Eenie Meenie</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>41</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6142779919e8c8c6cb086f5d27a41e05.png" alt="Cover for ZERO : FEVER Part.1" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-03-01</td>
      <td>ATEEZ</td>
      <td>ZERO : FEVER Part.1</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>77</td>
      <td>
          <img src="https://i.discogs.com/s1uEUqSwiurIAgHhspfLtYBHA0SU5oK31AShFHykGho/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIzNDYz/MDIzLTE2NTQzNjM1/NzYtODgxMS5qcGVn.jpeg" alt="Cover for SET" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-03-15</td>
      <td>Woodz</td>
      <td>SET</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>24</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/be1e560ca312ad18977ec59da3ddc101.png" alt="Cover for We play" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-03-17</td>
      <td>Weeekly</td>
      <td>We play</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 83</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>28</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a375aef404f1d6b4ddbf163fd3585a0b.png" alt="Cover for IU 5th Album &#39;LILAC&#39;" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-03-25</td>
      <td>IU</td>
      <td>IU 5th Album &#39;LILAC&#39;</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>26</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/90b0d3e896d6af8ed6be806666ccb175.png" alt="Cover for Last Melody" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-05-05</td>
      <td>Everglow</td>
      <td>Last Melody</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 87</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>1</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/dc9b7a0ed21655b53d9b28e12d05ce7e.png" alt="Cover for Next Level" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-05-17</td>
      <td>aespa</td>
      <td>Next Level</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 5</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>63</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e527f86c7b540a86082ae852c2e6a7ef.png" alt="Cover for Hello - Special Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-05-31</td>
      <td>Joy</td>
      <td>Hello - Special Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>95</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/52f91af94c4395b41da186a8f4c44e42.png" alt="Cover for Hello Future - The 1st Album Repackage" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-06-29</td>
      <td>NCT DREAM</td>
      <td>Hello Future - The 1st Album Repackage</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>79</td>
      <td>
          <img src="https://i.discogs.com/LlbktP8OoKWSa_ane2tsCwYiJxoHm44kW-PAQjFuyiQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIyNzY0/NzkxLTE2NDkxMTEy/MjMtOTQ0My5wbmc.jpeg" alt="Cover for Iite Cool" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-07-23</td>
      <td>DPR LIVE</td>
      <td>Iite Cool</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>22</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4434aa6b3c568b58a3b261739f762c94.png" alt="Cover for The Chaos Chapter: FIGHT OR ESCAPE" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-08-17</td>
      <td>TOMORROW X TOGETHER</td>
      <td>The Chaos Chapter: FIGHT OR ESCAPE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 78</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>44</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/afe85a750ff284385ae9697c32f6118d.png" alt="Cover for SYNK : PARALLEL LINE - Special Digital Single" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-09-14</td>
      <td>aespa</td>
      <td>SYNK : PARALLEL LINE - Special Digital Single</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>59</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/26222e2d62e652d5cc6c3bfdfdcfc2b9.png" alt="Cover for Sticker - The 3rd Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-09-17</td>
      <td>NCT 127</td>
      <td>Sticker - The 3rd Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>81</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bac603ed9d8527b5dd522e2de7cf4a36.png" alt="Cover for Girls Planet 999 - Creation Mission" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-10-08</td>
      <td>Girls Planet 999</td>
      <td>Girls Planet 999 - Creation Mission</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>9</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0d6f0c646a5cda4c17f390b8fb28542b.png" alt="Cover for MAP OF THE SOUL : 7" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-10-11</td>
      <td>BTS</td>
      <td>MAP OF THE SOUL : 7</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>82</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1e01f27d92724340942fad5853ff839b.png" alt="Cover for THE GIRLS (BLACKPINK THE GAME OST)" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-04-27</td>
      <td>BLACKPINK</td>
      <td>THE GIRLS (BLACKPINK THE GAME OST)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>29</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2b55dd52225eeeeadbe44c1056c7f7c0.png" alt="Cover for Seventeen 4th Album &#39;Face the Sun&#39;" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-05-27</td>
      <td>Seventeen</td>
      <td>Seventeen 4th Album &#39;Face the Sun&#39;</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 93</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>38</td>
      <td>
          <img src="https://i.discogs.com/wlyGtUjTRNSk3bYxxdSa3CaNn4XWlhceNDwMCL8TNJ4/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIzNTQy/NTYyLTE2NTQ5NzYy/MjAtMjkzOC5qcGVn.jpeg" alt="Cover for PRooF" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-06-10</td>
      <td>BTS</td>
      <td>PRooF</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>10</td>
      <td>
          <img src="https://i.discogs.com/cpYQHt8yHUWk4DQgy0fOtZcKTSjx2hu50V9QLXSrqRM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIzOTA2/MzI0LTE2Nzc5NTI0/NzMtNTg2MS5wbmc.jpeg" alt="Cover for HOPE ON THE STREET VOL.1" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-07-15</td>
      <td>J-Hope</td>
      <td>HOPE ON THE STREET VOL.1</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/house" | relative_url }}">house</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/house" | relative_url }}">house</a> # 7</li>
            <li> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a> # 21</li>
            <li> <a href="{{ "/tags/rap" | relative_url }}">rap</a> # 21</li>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 35</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>31</td>
      <td>
          <img src="https://i.discogs.com/NO1qZgNjpUKvDsml9EEBYoeFfDezHDwkddI4Gju-TKY/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI0MTE3/NDA0LTE2NTk3ODM1/OTktNzgxOC5qcGVn.jpeg" alt="Cover for we need love" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-07-19</td>
      <td>STAYC</td>
      <td>we need love</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 98</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>4</td>
      <td>
          <img src="https://i.discogs.com/QVnlReSoNTj_kDm8fQO1G1tzsOMRxTWSrY_-OzNeJ64/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI0MzY1/MDE4LTE2NjE5MDAw/NDctNzQ0OC5qcGVn.jpeg" alt="Cover for After LIKE" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-08-22</td>
      <td>Ive</td>
      <td>After LIKE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 11</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>62</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5ee2a81a81d671983816b4b725c53385.png" alt="Cover for BETWEEN 1&amp;2" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-08-26</td>
      <td>TWICE</td>
      <td>BETWEEN 1&amp;2</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 43</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>16</td>
      <td>
          <img src="https://i.discogs.com/KmO77xiaUQ2dJGAfW7kLXMMLL_SuHguRhVSKhnN_d1Q/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI0NTMy/NjA0LTE2NjQ0NjA1/MDYtNTI2My5qcGVn.jpeg" alt="Cover for bOrN pInK" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-09-16</td>
      <td>BLACKPINK</td>
      <td>bOrN pInK</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
            <li style="font-size: calc(1rem * 54 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 22</li>
            <li> <a href="{{ "/tags/pop" | relative_url }}">pop</a> # 51</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>71</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c80dc7ca44445fe8b6d826395fccaa9b.png" alt="Cover for Lowlife Princess: Noir" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-11-18</td>
      <td>Bibi</td>
      <td>Lowlife Princess: Noir</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>73</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9812fcb0317a46ef41fb79af1243a27e.png" alt="Cover for ‘The ReVe Festival 2022 - Feel My Rhythm’" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-11-28</td>
      <td>Red Velvet</td>
      <td>‘The ReVe Festival 2022 - Feel My Rhythm’</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 49</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>56</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/74d3503667af1cf0ffca7572f7aade04.png" alt="Cover for Spin Off : From The Witness" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-12-30</td>
      <td>ATEEZ</td>
      <td>Spin Off : From The Witness</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>85</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c0107a8389c04e11872f720bc2f67864.png" alt="Cover for ISTJ - The 3rd Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2023</td>
      <td>NCT DREAM</td>
      <td>ISTJ - The 3rd Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>86</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d2c00fd29eee364cf7b8cb4f6b5f8e18.png" alt="Cover for TEENFRESH" style="max-width:120px; height:auto;" />
      </td>
      <td>2023</td>
      <td>STAYC</td>
      <td>TEENFRESH</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>25</td>
      <td>
          <img src="https://i.discogs.com/3Zwwy9JDyt3JCZpZFS2Wpq7pqVO133wcYk6iUe6FRKg/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI2MTg1/NTkyLTE2NzcwNTU0/OTctNDM4OC5qcGVn.jpeg" alt="Cover for Pandora&#39;s Box" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-01-25</td>
      <td>MAVE:</td>
      <td>Pandora&#39;s Box</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 86</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>6</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3be392093ae2785d0123665241047b03.png" alt="Cover for The Beginning: Cupid" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-02-24</td>
      <td>Fifty Fifty</td>
      <td>The Beginning: Cupid</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 14</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>3</td>
      <td>
          <img src="https://i.discogs.com/iuIeyC80rJvzq4_rNMI8eILgCNmuCTnWWo-UjcBtcDM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI2NTcz/NTM0LTE2ODAwMTU3/MzAtODk3MC5qcGVn.jpeg" alt="Cover for Face" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-03-24</td>
      <td>jimin</td>
      <td>Face</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 10</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>5</td>
      <td>
          <img src="https://i.discogs.com/blNIN4AstyRw4ZNJUNQwEdh5R9UgGFHnKPazHjvaAYs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI2NDEz/NjU1LTE2ODA5ODY2/MzMtNDkwNS5qcGVn.jpeg" alt="Cover for Me" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-03-31</td>
      <td>JiSoo</td>
      <td>Me</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 12</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>21</td>
      <td>
          <img src="https://i.discogs.com/qhNVKHSAwSQgA08kRAvVlFf7kip43kJFdx8wO6mlIwc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI2NzE3/Mjg4LTE2ODEyMzQw/NDgtODI5My5qcGVn.jpeg" alt="Cover for I&#39;ve IVE" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-04-10</td>
      <td>Ive</td>
      <td>I&#39;ve IVE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>66</td>
      <td>
          <img src="https://i.discogs.com/3__8HN67AIzg_OcGRquoIpayh7yCWRX4NYGoGMmoMZQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI2OTI4/MDM1LTE2ODI4NDA3/NzktMjA1OS5qcGVn.jpeg" alt="Cover for Over The Moon" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-04-12</td>
      <td>Lee Chae Yeon</td>
      <td>Over The Moon</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>53</td>
      <td>
          <img src="https://i.discogs.com/0NkyoPhtHuJKBd7zAiW4zSyPoihdN-TfkhqG6ei8AsE/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI4OTU0/NjAwLTE3MDAzMzgy/MjktNTU3OC5qcGVn.jpeg" alt="Cover for Down to Earth" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-04</td>
      <td>Taeyang</td>
      <td>Down to Earth</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>89</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c82e440a482cf56cfdb3962aa4f4079e.png" alt="Cover for DISHARMONY : FIND OUT" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-06-09</td>
      <td>P1Harmony</td>
      <td>DISHARMONY : FIND OUT</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>84</td>
      <td>
          <img src="https://i.discogs.com/6iKEvi8oU_yuDOVEOET9uEoLZY7ZY0lzX2StQIKGrT8/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI4MDQy/OTE0LTE2OTI3MDEx/MTMtMTgzNi5qcGVn.jpeg" alt="Cover for Impossible" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-08-21</td>
      <td>RiIZE</td>
      <td>Impossible</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>18</td>
      <td>
          <img src="https://i.discogs.com/kidvFDvvpFN9KA58U28GUfhqYafNbc91yOQY76E9mZA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI4MjQx/MTQzLTE2OTQ1MjM0/MTMtNzUyNy5qcGVn.jpeg" alt="Cover for Get A Guitar" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-09-04</td>
      <td>RiIZE</td>
      <td>Get A Guitar</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 65</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>78</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a960846f70880f2d0c30cddd8f9c9011.png" alt="Cover for Fact Check - The 5th Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-10-06</td>
      <td>NCT 127</td>
      <td>Fact Check - The 5th Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>30</td>
      <td>
          <img src="https://i.discogs.com/WJpqiobtZw29WxfachDwGfkeeBEF2iegZJ71tcXQwnw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI4OTYx/Nzc5LTE3MDA0MDk3/NzUtOTg3OC5qcGVn.jpeg" alt="Cover for I&#39;VE MINE" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-10-13</td>
      <td>Ive</td>
      <td>I&#39;VE MINE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 96</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>83</td>
      <td>
          <img src="https://i.discogs.com/Pf0nryNu_XUFmvmdQK_9cUeFOCJaErZJ0Kq7p_wXfOU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI4NjMw/NDU2LTE2OTc2Mzgw/MjgtMjUyMy5qcGVn.jpeg" alt="Cover for Howl" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-10-18</td>
      <td>Chuu</td>
      <td>Howl</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>55</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c6830defaf76e25fe01e541b0cdc2592.png" alt="Cover for Seventeen 4th Album Repackage &#39;SECTOR 17&#39;" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-10-23</td>
      <td>Seventeen</td>
      <td>Seventeen 4th Album Repackage &#39;SECTOR 17&#39;</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>43</td>
      <td>
          <img src="https://i.discogs.com/MtyU2eaiENOJeSl3DscRIMN0XnS-JvESpV6xw5vB9NU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI5MTY4/Mjc4LTE3MDIyNTE1/NDctMTk5MC5wbmc.jpeg" alt="Cover for Born to be XX" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-11-09</td>
      <td>Kiss Of Life</td>
      <td>Born to be XX</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>75</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d90294466cd904feb236115f793b813d.png" alt="Cover for 2 Baddies - The 4th Album" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-12-22</td>
      <td>NCT 127</td>
      <td>2 Baddies - The 4th Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ballad" | relative_url }}">ballad</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/ballad" | relative_url }}">ballad</a> # 8</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>19</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4aa0a84456e661b2bf735ffa93313236.png" alt="Cover for Supernatural" style="max-width:120px; height:auto;" />
      </td>
      <td>2024</td>
      <td>NewJeans</td>
      <td>Supernatural</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/j-pop" | relative_url }}">j-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/japanese" | relative_url }}">japanese</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/j-pop" | relative_url }}">j-pop</a> # 1</li>
            <li> <a href="{{ "/tags/japanese" | relative_url }}">japanese</a> # 3</li>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 67</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>91</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8f5f7ff48d22c0603d276c9e9ff708a6.png" alt="Cover for ZERO : FEVER EPILOGUE" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-02-28</td>
      <td>ATEEZ</td>
      <td>ZERO : FEVER EPILOGUE</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/male-vocalists" | relative_url }}">male vocalists</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/male-vocalists" | relative_url }}">male vocalists</a> # 7</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>12</td>
      <td>
          <img src="https://i.discogs.com/C0hc1XCicG5_wbJoNlPbin7ribRpwh2H-f3aXp-aR_U/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI4NTQ5/NzY4LTE2OTY5MTU1/NTMtNTMzNC5qcGVn.jpeg" alt="Cover for on the street (with J. Cole)" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-03-29</td>
      <td>J-Hope</td>
      <td>on the street (with J. Cole)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/rap" | relative_url }}">rap</a> # 34</li>
            <li> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a> # 39</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>87</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/be2236fbf8545ec795cc4a22721dbb48.png" alt="Cover for BABYMONS7ER" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-04-01</td>
      <td>BABYMONSTER</td>
      <td>BABYMONS7ER</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>13</td>
      <td>
          <img src="https://i.discogs.com/Z_2ln7Dr4IbEViJbZPWb5JEDvyLn9Zn295v-xn0Nskc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTMwNzE3/MDExLTE3MTYxMjYx/MDYtNzQ5My5qcGVn.jpeg" alt="Cover for Midas Touch" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-04-04</td>
      <td>Kiss Of Life</td>
      <td>Midas Touch</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 44</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>37</td>
      <td>
          <img src="https://i.discogs.com/3g8h0JYRVjXJ0CADDsof33EVsXpNbzN9KYaAKKTZGkA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE1ODgx/OTM3LTE2MjEyNTQ5/NzItMzc3My5qcGVn.jpeg" alt="Cover for GO Live" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-07-19</td>
      <td>STRAY KIDS</td>
      <td>GO Live</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>57</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/45286c895828df3dff6cf04775ee2300.png" alt="Cover for Episode1 : Love" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>So!YoON!</td>
      <td>Episode1 : Love</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-rnb" | relative_url }}">alternative rnb</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rnb" | relative_url }}">rnb</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/alternative-rnb" | relative_url }}">alternative rnb</a> # 41</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>69</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bebc6963de33caea5acb4a5860003041.png" alt="Cover for KILL MY DOUBT" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>ITZY</td>
      <td>KILL MY DOUBT</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a> # 94</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>42</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2520db7790f62d330a1713c4c75ee09f.png" alt="Cover for minisode 3: TOMORROW" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>TOMORROW X TOGETHER</td>
      <td>minisode 3: TOMORROW</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>72</td>
      <td>
      </td>
      <td></td>
      <td>Jeon Soyeon</td>
      <td>Windy</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>74</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/41e2b71debed0ca0908ded498d731dc9.png" alt="Cover for poppop - The 2nd Mini Album" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>NCT WISH</td>
      <td>poppop - The 2nd Mini Album</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>100</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d46adf37fd8afb1b6cf82bd0483908f8.png" alt="Cover for QURIOUS" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>woo!ah!</td>
      <td>QURIOUS</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/k-pop" | relative_url }}">k-pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/korean" | relative_url }}">korean</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
  </tbody>
</table>

### Related genres

- [k-pop]({{ "/tags/k-pop" | relative_url }})
- [rap]({{ "/tags/rap" | relative_url }})
- [hip-hop]({{ "/tags/hip-hop" | relative_url }})
- [female vocalists]({{ "/tags/female-vocalists" | relative_url }})
- [pop]({{ "/tags/pop" | relative_url }})
- [asian]({{ "/tags/asian" | relative_url }})
- [rock]({{ "/tags/rock" | relative_url }})
- [male vocalists]({{ "/tags/male-vocalists" | relative_url }})
- [rnb]({{ "/tags/rnb" | relative_url }})
- [ballad]({{ "/tags/ballad" | relative_url }})
