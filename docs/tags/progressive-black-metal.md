---
category: genre
date: 2025-12-06T09:54:06.140Z
permalink: tags/progressive-black-metal
title: progressive black metal
---

## progressive black metal

Last updated: <time datetime="2025-12-06T09:54:06.140Z">Sat Dec 06 2025 11:54:06 GMT+0200 (Eastern European Standard Time)</time>

### Description

Progressive black metal expands upon traditional black metal&#39;s raw aggression and atmospheric darkness by incorporating complex song structures, unconventional time signatures, and influences from jazz, classical, and avant-garde music. Emerging in the mid-1990s as black metal artists sought greater musical ambition, the genre maintains the foundational tremolo-picked guitars and blast beats while embracing experimentation with clean vocals, acoustic passages, and philosophical lyrical themes. The result is an intellectually challenging yet emotionally intense style that pushes beyond black metal&#39;s orthodox boundaries.

### 100 chronological releases

<table>
  <thead>
    <tr>
      <th>Place</th>
      <th>Cover</th>
      <th>Date</th>
      <th>Artist</th>
      <th>Album</th>
      <th>Tags</th>
      <th>Other places</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>52</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/008573aefc7c5193d2343bcf676dee8e.png" alt="Cover for Min Tid Skal Komme" style="max-width:120px; height:auto;" />
      </td>
      <td>1995</td>
      <td>Fleurety</td>
      <td>Min Tid Skal Komme</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>81</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/40bbe2eb3b502c3da26080468182efde.png" alt="Cover for Ghastly Funeral Theatre" style="max-width:120px; height:auto;" />
      </td>
      <td>1997-04</td>
      <td>Sigh</td>
      <td>Ghastly Funeral Theatre</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>68</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4590ba1eda081c491aaf2dfeec389462.png" alt="Cover for Orchid (2023 Abbey Road Remaster)" style="max-width:120px; height:auto;" />
      </td>
      <td>1998-08-18</td>
      <td>Opeth</td>
      <td>Orchid (2023 Abbey Road Remaster)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-black-metal" | relative_url }}">melodic black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-death-metal" | relative_url }}">melodic death metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-death-metal" | relative_url }}">progressive death metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>74</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b191ba34f95087895eef35a41b9f3362.png" alt="Cover for Scenario IV: Dread Dreams" style="max-width:120px; height:auto;" />
      </td>
      <td>1999</td>
      <td>Sigh</td>
      <td>Scenario IV: Dread Dreams</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>99</td>
      <td>
      </td>
      <td>1999-09-01</td>
      <td>Thy Catafalque</td>
      <td>Sublunary Tragedies</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/symphonic-black-metal" | relative_url }}">symphonic black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>73</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5e73e262bf6e4458aacff2fed6ff692b.png" alt="Cover for Solace" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-04-12</td>
      <td>Lengsel</td>
      <td>Solace</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>61</td>
      <td>
          <img src="https://i.discogs.com/S7tDeiK7wcTsupG_1HXkn_xQmYgkeG5nY1YetrK0HAc/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTc2ODc2/NC0xNTI4Nzg1OTUw/LTEyMjguanBlZw.jpeg" alt="Cover for Quintessence" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-04-17</td>
      <td>Borknagar</td>
      <td>Quintessence</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>83</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/68214b0c52fc46aab4d186d1be836cab.png" alt="Cover for Phormula" style="max-width:120px; height:auto;" />
      </td>
      <td>2000-05</td>
      <td>Ephel Duath</td>
      <td>Phormula</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>100</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/85ee6b8bc8c54cd388de0ab94bd5e0bf.png" alt="Cover for In the Distance" style="max-width:120px; height:auto;" />
      </td>
      <td>2001</td>
      <td>Vediog Svaor</td>
      <td>In the Distance</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-black-metal" | relative_url }}">experimental black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-black-metal" | relative_url }}">experimental black metal</a> # 19</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>58</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2c2c67725db78aa3b5f7179e44d23011.png" alt="Cover for Death&#39;s Design" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-03-02</td>
      <td>Diabolical Masquerade</td>
      <td>Death&#39;s Design</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 79 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 79 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 79 / 100)"> <a href="{{ "/tags/symphonic-black-metal" | relative_url }}">symphonic black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>7</td>
      <td>
      </td>
      <td>2001-10-22</td>
      <td>Emperor</td>
      <td>Prometheus: The Discipline Of Fire &amp; Demise</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 75 / 100)"> <a href="{{ "/tags/symphonic-black-metal" | relative_url }}">symphonic black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/symphonic-black-metal" | relative_url }}">symphonic black metal</a> # 40</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>59</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b56766dd6cd54581943b25b549dfad3d.png" alt="Cover for Empiricism" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-10-22</td>
      <td>Borknagar</td>
      <td>Empiricism</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>26</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c155c9a0861543dec3931ea9514b5c8b.png" alt="Cover for Monumension" style="max-width:120px; height:auto;" />
      </td>
      <td>2001-11-14</td>
      <td>Enslaved</td>
      <td>Monumension</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 54 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>19</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f817c173ef8e1b80373b6a6a21fb5d9b.png" alt="Cover for &#39;N Crugu Bradului" style="max-width:120px; height:auto;" />
      </td>
      <td>2002-10-03</td>
      <td>Negură Bunget</td>
      <td>&#39;N Crugu Bradului</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>89</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1acc1a05e8264d95b86a554d81e7fa0e.png" alt="Cover for 1000 Thoughts Of Violence" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-02-01</td>
      <td>Kekal</td>
      <td>1000 Thoughts Of Violence</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>12</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e0cd71257df04508c58a652d589aa6c6.png" alt="Cover for Below the Lights" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-05-05</td>
      <td>Enslaved</td>
      <td>Below the Lights</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>65</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b5697b3da7c3d2a898fe21f59317581e.png" alt="Cover for Into the Oriental Phantasma" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-06-02</td>
      <td>Kadenzza</td>
      <td>Into the Oriental Phantasma</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>51</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d2cf06157ec57bd0a86306aa39ad59ab.png" alt="Cover for Isa" style="max-width:120px; height:auto;" />
      </td>
      <td>2004-11-01</td>
      <td>Enslaved</td>
      <td>Isa</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>54</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/14269e073db44dd4cd87a2865c062111.png" alt="Cover for Slaves" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>1349</td>
      <td>Slaves</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-death-metal" | relative_url }}">black death metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/death-metal" | relative_url }}">death metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/norway" | relative_url }}">norway</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/norwegian" | relative_url }}">norwegian</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/norwegian-black-metal" | relative_url }}">norwegian black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/true-black-metal" | relative_url }}">true black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/true-norwegian-black-metal" | relative_url }}">true norwegian black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/true-black-metal" | relative_url }}">true black metal</a> # 9</li>
            <li> <a href="{{ "/tags/black-death-metal" | relative_url }}">black death metal</a> # 13</li>
            <li> <a href="{{ "/tags/true-norwegian-black-metal" | relative_url }}">true norwegian black metal</a> # 24</li>
            <li> <a href="{{ "/tags/norwegian-black-metal" | relative_url }}">norwegian black metal</a> # 38</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>18</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/eaf96560795a4b4dcbd0b61e5b20ead4.png" alt="Cover for A Umbra Omega" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>Dødheimsgard</td>
      <td>A Umbra Omega</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>56</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/83a4a9ba27c361859d659ebc14eddf9a.png" alt="Cover for Morningrise (2023 Abbey Road Remaster)" style="max-width:120px; height:auto;" />
      </td>
      <td>2005</td>
      <td>Opeth</td>
      <td>Morningrise (2023 Abbey Road Remaster)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-death-metal" | relative_url }}">progressive death metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>24</td>
      <td>
      </td>
      <td>2005-05-18</td>
      <td>Negură Bunget</td>
      <td>Inarborat kosmos</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pagan-black-metal" | relative_url }}">pagan black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/romania" | relative_url }}">romania</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/romania" | relative_url }}">romania</a> # 2</li>
            <li> <a href="{{ "/tags/pagan-black-metal" | relative_url }}">pagan black metal</a> # 12</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>75</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e29bc525a9c34a6ab2fa46ef306d2d18.png" alt="Cover for Intra" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-05-30</td>
      <td>Ram-Zet</td>
      <td>Intra</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/avant-garde-metal" | relative_url }}">avant-garde metal</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>92</td>
      <td>
          <img src="https://i.discogs.com/RAP1V2di2Gi92-pJPQOSDCeyX3bVBaSVdeuMcXhUSYk/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTcyMjU2/MS0xMzQ0ODQ5NTg3/LTIxOTkuanBlZw.jpeg" alt="Cover for Welterwerk" style="max-width:120px; height:auto;" />
      </td>
      <td>2006</td>
      <td>Drottnar</td>
      <td>Welterwerk</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/unblack-metal" | relative_url }}">unblack metal</a> # 32</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>17</td>
      <td>
      </td>
      <td>2006-01</td>
      <td>Mirrorthrone</td>
      <td>Carriers of Dust</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/symphonic-black-metal" | relative_url }}">symphonic black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a> # 15</li>
            <li> <a href="{{ "/tags/symphonic-black-metal" | relative_url }}">symphonic black metal</a> # 61</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>69</td>
      <td>
          <img src="https://i.discogs.com/V6sLv12eQ1MTyaQNd8FM_nUIgPKh7Asy7-P3GNxQ2Ys/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTY2NDA2/NC0xMTQ1MDI4NDc0/LmpwZWc.jpeg" alt="Cover for Black Church" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-02-22</td>
      <td>Enochian Crescent</td>
      <td>Black Church</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>3</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3813f71670c34805b1db8359291ba41a.png" alt="Cover for The Adversary" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-04-10</td>
      <td>Ihsahn</td>
      <td>The Adversary</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a> # 92</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>23</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0e4a1881d18548a882b177b96c068a66.png" alt="Cover for Ruun" style="max-width:120px; height:auto;" />
      </td>
      <td>2006-05-02</td>
      <td>Enslaved</td>
      <td>Ruun</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a> # 86</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>30</td>
      <td>
      </td>
      <td>2006-10-13</td>
      <td>Ikuinen Kaamos</td>
      <td>The Forlorn</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>57</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/11c06c89fdff4e109cc6a1439aa096f1.png" alt="Cover for Nekro" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-03-12</td>
      <td>Taake</td>
      <td>Nekro</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/true-black-metal" | relative_url }}">true black metal</a> # 12</li>
            <li> <a href="{{ "/tags/norwegian-black-metal" | relative_url }}">norwegian black metal</a> # 43</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>40</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5526f2f13c2942f38aadcb6a42fb856c.png" alt="Cover for The Latter Rain" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-03-29</td>
      <td>In Vain</td>
      <td>The Latter Rain</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>13</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e8f1e329f794e2a6d0f0ab3035a4c083.png" alt="Cover for The Aurora Veil" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-04-21</td>
      <td>Ne Obliviscaris</td>
      <td>The Aurora Veil</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>25</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/19da75c436ecc5b7bf8b51e74e6e38de.png" alt="Cover for Circadian Rhythm" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-04-28</td>
      <td>Thurisaz</td>
      <td>Circadian Rhythm</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/dark-metal" | relative_url }}">dark metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-black-metal" | relative_url }}">melodic black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/dark-metal" | relative_url }}">dark metal</a> # 17</li>
            <li> <a href="{{ "/tags/melodic-black-metal" | relative_url }}">melodic black metal</a> # 63</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>95</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2483a13a9083f966ea49ec13895be3c5.png" alt="Cover for Bellowing Sea - Racked By Tempest" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-06-11</td>
      <td>In Lingua Mortua</td>
      <td>Bellowing Sea - Racked By Tempest</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/symphonic-black-metal" | relative_url }}">symphonic black metal</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>86</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/dc4f486a2b3940e0ae311706401a917f.png" alt="Cover for FAS - Ite, Maledicti, in ignem Aeternum" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-07-16</td>
      <td>Deathspell Omega</td>
      <td>FAS - Ite, Maledicti, in ignem Aeternum</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a> # 10</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>39</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/27addd8729d04570b04a17e94c9e607d.png" alt="Cover for The Corpse of Rebirth" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-01-22</td>
      <td>A Forest of Stars</td>
      <td>The Corpse of Rebirth</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 54 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a> # 72</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>15</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/de57926bc9b2437994aa97b6117cdb66.png" alt="Cover for Gangrene" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-04</td>
      <td>Mirrorthrone</td>
      <td>Gangrene</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>9</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3dfad7f33ceb4d4fad01ba43ba6c16a6.png" alt="Cover for angL" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-05-26</td>
      <td>Ihsahn</td>
      <td>angL</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 67 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a> # 91</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>66</td>
      <td>
          <img src="https://i.discogs.com/QUhxulvkVGPaGItv6540I3OoCxYaB7SI-1RanhfAvb0/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE0MTI3/MDYtMTIyNjg4NTI1/OC5qcGVn.jpeg" alt="Cover for Inorganic Rites" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-07-11</td>
      <td>Krallice</td>
      <td>Inorganic Rites</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>21</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2d99859cd26fdba37cfd1936ea82cfd2.png" alt="Cover for Vertebrae" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-09-26</td>
      <td>Enslaved</td>
      <td>Vertebrae</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>97</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bff306a1352343048979bf1a50e166f2.png" alt="Cover for Privilegivm" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-09-18</td>
      <td>Secrets of the Moon</td>
      <td>Privilegivm</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>98</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6249a295160f49449c05a2d82c134bdd.png" alt="Cover for Dimensional Bleedthrough" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-11-10</td>
      <td>Krallice</td>
      <td>Dimensional Bleedthrough</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>41</td>
      <td>
          <img src="https://i.discogs.com/MTpMjpx6qbyXB3tVXNzxwgA_eXNKfeYDHEuUKoNJHhQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIyMDE2/NTAtMTI2OTUyMzU5/NC5qcGVn.jpeg" alt="Cover for Vîrstele pamîntului" style="max-width:120px; height:auto;" />
      </td>
      <td>2010</td>
      <td>Negura Bunget</td>
      <td>Vîrstele pamîntului</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>62</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/cc8bc462fc8a47d68239e51fa4fdc5be.png" alt="Cover for After" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-01-25</td>
      <td>Ihsahn</td>
      <td>After</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>10</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ac879828e0c54ebe966962fe05656176.png" alt="Cover for Universal" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-02-22</td>
      <td>Borknagar</td>
      <td>Universal</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>60</td>
      <td>
      </td>
      <td>2010-06-01</td>
      <td>A Forest of Stars</td>
      <td>Opportunistic Thieves of Spring</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 81 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>1</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f0526348f1f84ca698db25a2e7761f2f.png" alt="Cover for Axioma Ethica Odini" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-09-27</td>
      <td>Enslaved</td>
      <td>Axioma Ethica Odini</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a> # 45</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>2</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e0893f3b967546a48815bc41c424e04e.png" alt="Cover for Paracletus" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-11-08</td>
      <td>Deathspell Omega</td>
      <td>Paracletus</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 57.99999999999999 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a> # 3</li>
            <li> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a> # 46</li>
            <li> <a href="{{ "/tags/french-black-metal" | relative_url }}">french black metal</a> # 72</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>90</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4a095a104258488a849e9832abc7b0c0.png" alt="Cover for VII: Född förlorare" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-05-11</td>
      <td>Shining</td>
      <td>VII: Född förlorare</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/suicidal-black-metal" | relative_url }}">suicidal black metal</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>53</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9a01a9b92be5ae69434219aeac55ce29.png" alt="Cover for The Sleeping Gods" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-05</td>
      <td>Enslaved</td>
      <td>The Sleeping Gods</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 67 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/psychedelic-metal" | relative_url }}">psychedelic metal</a> # 61</li>
            <li> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a> # 85</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>16</td>
      <td>
      </td>
      <td>2011-05</td>
      <td>Deathspell Omega</td>
      <td>Diabolus Absconditus</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>34</td>
      <td>
      </td>
      <td>2011-11-02</td>
      <td>Panopticon</td>
      <td>Social Disservices</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>11</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e7f58b01e7214cf39a37439b38ed0e14.png" alt="Cover for Urd" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-03-26</td>
      <td>Borknagar</td>
      <td>Urd</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>6</td>
      <td>
      </td>
      <td>2012-05-07</td>
      <td>Ne Obliviscaris</td>
      <td>Portal Of I</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a> # 50</li>
            <li> <a href="{{ "/tags/melodic-black-metal" | relative_url }}">melodic black metal</a> # 98</li>
            <li> <a href="{{ "/tags/progressive-death-metal" | relative_url }}">progressive death metal</a> # 99</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>50</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c2b1c45bc7c8816e1d2fbf9d6cf18857.png" alt="Cover for Drought" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-06-22</td>
      <td>Deathspell Omega</td>
      <td>Drought</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>85</td>
      <td>
      </td>
      <td>2012-07-05</td>
      <td>Blut aus Nord</td>
      <td>What Once Was... Liber I</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>46</td>
      <td>
      </td>
      <td>2012-08-25</td>
      <td>Krallice</td>
      <td>Years Past Matter</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a> # 71</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>32</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3b1a6f7e14ec4e529a781eede16cf3dd.png" alt="Cover for RIITIIR" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-09-28</td>
      <td>Enslaved</td>
      <td>RIITIIR</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a> # 88</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>76</td>
      <td>
      </td>
      <td>2012-09-28</td>
      <td>Dordeduh</td>
      <td>Dar de duh</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>27</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b764dd81c19490ae5bcf6ab4ec43a01d.png" alt="Cover for Redefining Darkness" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-10-29</td>
      <td>Shining</td>
      <td>Redefining Darkness</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/depressive-black-metal" | relative_url }}">depressive black metal</a> # 72</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>94</td>
      <td>
          <img src="https://i.discogs.com/glfsdelWNk15Nmz0jSrhss5pfg3B15mwM9Y00QM1qPU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTUwMDM0/OTEtMTM4MTg2MTYx/MS03NzUwLmpwZWc.jpeg" alt="Cover for The Three Transcendental Keys" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-10-28</td>
      <td>Throne of Katarsis</td>
      <td>The Three Transcendental Keys</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/norway" | relative_url }}">norway</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/orthodox-black-metal" | relative_url }}">orthodox black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/orthodox-black-metal" | relative_url }}">orthodox black metal</a> # 22</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>87</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c48532ee956a3f1d4dc33f7bac436b35.png" alt="Cover for Augur Nox" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-11-19</td>
      <td>Code</td>
      <td>Augur Nox</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>70</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1bef92408caba9e4a46f803bde57c167.png" alt="Cover for Extance" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-01-20</td>
      <td>Aenaon</td>
      <td>Extance</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a> # 3</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>72</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d98a497970dd6b79b10267eec81e3f3b.png" alt="Cover for Oi Magoi" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-01-20</td>
      <td>Hail Spirit Noir</td>
      <td>Oi Magoi</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 54 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>96</td>
      <td>
          <img src="https://i.discogs.com/wpy7YBpaH20rS4F-h_8tob0opdzO_3ufJUNVqy7XS-U/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTYzOTU5/NTItMTQxODE5MTU4/MC02NzE5LmpwZWc.jpeg" alt="Cover for Singularity" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-09-23</td>
      <td>Singularity</td>
      <td>Singularity</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>47</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/561dfc6d135977bba88f0a4a127ff3a9.png" alt="Cover for Citadel" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-11-07</td>
      <td>Ne Obliviscaris</td>
      <td>Citadel</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a> # 26</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>71</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/963fd9ea833753497a5ff1b46c622bf9.png" alt="Cover for Hiraeth" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-11-07</td>
      <td>Ne Obliviscaris</td>
      <td>Hiraeth</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>31</td>
      <td>
      </td>
      <td>2014-11-18</td>
      <td>Fen</td>
      <td>Carrion Skies</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-black-metal" | relative_url }}">post-black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/post-black-metal" | relative_url }}">post-black metal</a> # 36</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>77</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/07c72d3a82562097bc24238eeb5f761d.png" alt="Cover for Bridging Realms" style="max-width:120px; height:auto;" />
      </td>
      <td>2015</td>
      <td>Dreadnought</td>
      <td>Bridging Realms</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>20</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/b7e659870172412fc15f03500d7ff653.png" alt="Cover for Beware The Sword You Cannot See" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-02-27</td>
      <td>A Forest of Stars</td>
      <td>Beware The Sword You Cannot See</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-garde-metal" | relative_url }}">avant-garde metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-garde-metal" | relative_url }}">avant-garde metal</a> # 38</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>4</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9f293bf701cf80f9cf8df2a00cc16c7a.png" alt="Cover for In Times" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-03-06</td>
      <td>Enslaved</td>
      <td>In Times</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 77 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 54 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>48</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f1dcb73da4bb4b55ae6c22d60c623a6d.png" alt="Cover for IX - Everyone, Everything, Everywhere, Ends" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-04-20</td>
      <td>Shining</td>
      <td>IX - Everyone, Everything, Everywhere, Ends</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/suicidal-black-metal" | relative_url }}">suicidal black metal</a> # 16</li>
            <li> <a href="{{ "/tags/depressive-black-metal" | relative_url }}">depressive black metal</a> # 57</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>44</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0858240dd167805ad12eb08655a91cfd.png" alt="Cover for Sarabande to Nihil" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-04</td>
      <td>Ne Obliviscaris</td>
      <td>Sarabande to Nihil</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-black-metal" | relative_url }}">melodic black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>36</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f498c03866d52f828ca8e7ca968f4fea.png" alt="Cover for Scar Sighted" style="max-width:120px; height:auto;" />
      </td>
      <td>2016</td>
      <td>Leviathan</td>
      <td>Scar Sighted</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>43</td>
      <td>
          <img src="https://i.discogs.com/SceHD8j1v_QB7a8W7isMQWn1gtZ1_WLlk8AWQLuJr6A/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTc5NDM3/MjMtMTQ1MjE0Njk2/My01NzczLmpwZWc.jpeg" alt="Cover for Hyperion" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-01-12</td>
      <td>Krallice</td>
      <td>Hyperion</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>8</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6499ff87e6a9800aa8a64257a3b5fddb.png" alt="Cover for Winter Thrice" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-01-21</td>
      <td>Borknagar</td>
      <td>Winter Thrice</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>82</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9d5c582dfec1c5bd89eb71a58d367814.png" alt="Cover for Arktis." style="max-width:120px; height:auto;" />
      </td>
      <td>2016-04-08</td>
      <td>Ihsahn</td>
      <td>Arktis.</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a> # 84</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>88</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/71af6e8741c85efc5c8e78f341c000e3.png" alt="Cover for Hypnosophy" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-11-02</td>
      <td>Aenaon</td>
      <td>Hypnosophy</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/extreme-progressive-metal" | relative_url }}">extreme progressive metal</a> # 56</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>45</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c28ce3258d016ff4b604408cf451af1e.png" alt="Cover for Khram" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-11-04</td>
      <td>Arkona</td>
      <td>Khram</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folk-metal" | relative_url }}">folk metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pagan-metal" | relative_url }}">pagan metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/russian" | relative_url }}">russian</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/pagan-metal" | relative_url }}">pagan metal</a> # 52</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>79</td>
      <td>
          <img src="https://i.discogs.com/e48epPKgGSzRkGQAv21X-VFZIfoPc6tyA2uhoeq6zRw/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTkyOTMz/MjItMTQ4MzAyNDg0/My04NDQ2LmpwZWc.jpeg" alt="Cover for The Synarchy of Molten Bones" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-11-08</td>
      <td>Deathspell Omega</td>
      <td>The Synarchy of Molten Bones</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/orthodox-black-metal" | relative_url }}">orthodox black metal</a> # 38</li>
            <li> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a> # 68</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>28</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/6a4eb4413026836fbf52cbc76cdeca5c.png" alt="Cover for The Xun Protectorate" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-11-25</td>
      <td>Khonsu</td>
      <td>The Xun Protectorate</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/industrial-black-metal" | relative_url }}">industrial black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/industrial-black-metal" | relative_url }}">industrial black metal</a> # 6</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>67</td>
      <td>
          <img src="https://i.discogs.com/dv2Ciu4Qd-W048-OOeF6HqtQWybbnhP5o11UPiIdYGA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTk1OTg5/NzItMTU1NDkwODEx/MS0xMjk4LmpwZWc.jpeg" alt="Cover for Ons Vrije Fatum" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-01-09</td>
      <td>Laster</td>
      <td>Ons Vrije Fatum</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/blackgaze" | relative_url }}">blackgaze</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-black-metal" | relative_url }}">experimental black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/post-black-metal" | relative_url }}">post-black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-black-metal" | relative_url }}">experimental black metal</a> # 8</li>
            <li> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a> # 25</li>
            <li> <a href="{{ "/tags/post-black-metal" | relative_url }}">post-black metal</a> # 87</li>
            <li> <a href="{{ "/tags/blackgaze" | relative_url }}">blackgaze</a> # 100</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>63</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/65cd79f3298cfbca2262cb356e558dd8.png" alt="Cover for Diaspora" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-08-11</td>
      <td>Cormorant</td>
      <td>Diaspora</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/death-metal" | relative_url }}">death metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/melodic-black-metal" | relative_url }}">melodic black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>42</td>
      <td>
          <img src="https://i.discogs.com/PR-NNWf0JtdR8Oj-rcLzOcTirJRCG4q42qHAXgUwZ9U/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwNzQ4/NDg4LTE1MDM1OTU1/NzktNjU5Mi5qcGVn.jpeg" alt="Cover for Of Erthe And Axen: Act I" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-08-22</td>
      <td>Xanthochroid</td>
      <td>Of Erthe And Axen: Act I</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/folk-metal" | relative_url }}">folk metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/symphonic-black-metal" | relative_url }}">symphonic black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>37</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/698e3fb663bed78712170cc2d273daa2.png" alt="Cover for E" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-10-13</td>
      <td>Enslaved</td>
      <td>E</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>84</td>
      <td>
          <img src="https://i.discogs.com/YdolV-CLadPapegHOl3NQ0YpNI-tHiTVcYAT-BEfjoA/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwMTE1/MTk0LTE0OTE4NzE3/MzQtMTQzMi5qcGVn.jpeg" alt="Cover for Loüm" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-01-09</td>
      <td>Krallice</td>
      <td>Loüm</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a> # 31</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>93</td>
      <td>
          <img src="https://i.discogs.com/jDyORN9jV2Gi8bEHdYVcoQVF9GTg2TwGuY39odazxFU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzMDQ3/MTE2LTE1NDcxMDUx/MDMtMTAwMi5qcGVn.jpeg" alt="Cover for Wolf" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-01-09</td>
      <td>Krallice</td>
      <td>Wolf</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a> # 80</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>33</td>
      <td>
          <img src="https://i.discogs.com/iM7g4N8dytU2Hi9-f0F3c4_e8yk0XZ-0NV38G0THSKo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzMjE5/MjY0LTE1NTAxNzA1/OTQtNTgxOS5qcGVn.jpeg" alt="Cover for Achatius" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-02-15</td>
      <td>Funereal Presence</td>
      <td>Achatius</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/usbm" | relative_url }}">usbm</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/usbm" | relative_url }}">usbm</a> # 6</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>35</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/799c06bcb64848e88fd1fa22d370e942.png" alt="Cover for VI - Klagopsalmer" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-11-22</td>
      <td>Shining</td>
      <td>VI - Klagopsalmer</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 70 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>22</td>
      <td>
          <img src="https://i.discogs.com/WrVQSp_tmCZR64XYxGbwt6cWimW93fkxbNqnq_IA8fs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE0NDkx/MjAyLTE1NzU2Mjgx/NzUtOTk2My5qcGVn.jpeg" alt="Cover for The Dead Light" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-12-06</td>
      <td>Fen</td>
      <td>The Dead Light</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/blackgaze" | relative_url }}">blackgaze</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/blackgaze" | relative_url }}">blackgaze</a> # 43</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>29</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/a3df4e5b6d9a3e1a4da10f03d53bc5ab.png" alt="Cover for Spectres from the Old World" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-02-28</td>
      <td>Dark Fortress</td>
      <td>Spectres from the Old World</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>78</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9d1150eeabf54c6292eceb5a2513f01c.png" alt="Cover for Mardraum" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-10-02</td>
      <td>Enslaved</td>
      <td>Mardraum</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>80</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5286e4a7446f94d5718f6ceffb2e7fc7.png" alt="Cover for Utgard" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-10-02</td>
      <td>Enslaved</td>
      <td>Utgard</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>64</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7bf68c63f9f66c097d1dddab657b5ae2.png" alt="Cover for Demonic Wealth" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-03-05</td>
      <td>Krallice</td>
      <td>Demonic Wealth</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/atmospheric-black-metal" | relative_url }}">atmospheric black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/lo-fi" | relative_url }}">lo-fi</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/avant-garde-black-metal" | relative_url }}">avant-garde black metal</a> # 22</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>49</td>
      <td>
          <img src="https://i.discogs.com/lG5L2aBiEmiF3aSap9-rQO8BQSUJ_IeAQyJmLreR2BU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIwMDUz/MzMzLTE2MzAzNDMy/NzUtNjE4NS5qcGVn.jpeg" alt="Cover for Katartisis" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-08-27</td>
      <td>Serpent Column</td>
      <td>Katartisis</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>91</td>
      <td>
          <img src="https://i.discogs.com/Y7KlEYsK_wKynjiyF-eSRzGt3beAE35iaH4YIzA8__A/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIyNDUx/NjUzLTE2NDcyNDM0/ODItODc2MS5qcGVn.jpeg" alt="Cover for Oheň hoří tam, kde padl" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-03-07</td>
      <td>Kostnatění</td>
      <td>Oheň hoří tam, kde padl</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/raw-black-metal" | relative_url }}">raw black metal</a> # 89</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>14</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7b218fbd763e9ebe48f615370d327540.png" alt="Cover for Heimdal" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-03-03</td>
      <td>Enslaved</td>
      <td>Heimdal</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>55</td>
      <td>
          <img src="https://i.discogs.com/mveSzDv_OK00p9ztmbMGrPR3JqgMQ13zEcxhEjrugkM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI5MTEy/MDUyLTE3MDE3Mjgz/ODgtODQwNy5qcGVn.jpeg" alt="Cover for ÚPAL" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-05-26</td>
      <td>Kostnatění</td>
      <td>ÚPAL</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>38</td>
      <td>
          <img src="https://i.discogs.com/GCy1MJVyaIiphdteZ1aZt5i4QKFz7aXLqquHzg9cCRs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTI5MjQ5/MDIwLTE3MDMwMDk2/NDAtMTg1MC5qcGVn.jpeg" alt="Cover for Ozymandian" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-12-19</td>
      <td>Hauntologist</td>
      <td>Ozymandian</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-black-metal" | relative_url }}">experimental black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/polish-black-metal" | relative_url }}">polish black metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-black-metal" | relative_url }}">experimental black metal</a> # 4</li>
            <li> <a href="{{ "/tags/polish-black-metal" | relative_url }}">polish black metal</a> # 6</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>5</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9e2c04233d23451c8d51baf766935831.png" alt="Cover for Prometheus: The Discipline of Fire &amp; Demise" style="max-width:120px; height:auto;" />
      </td>
      <td></td>
      <td>Emperor</td>
      <td>Prometheus: The Discipline of Fire &amp; Demise</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-black-metal" | relative_url }}">progressive black metal</a></li>
            <li style="font-size: calc(1rem * 80 / 100)"> <a href="{{ "/tags/black-metal" | relative_url }}">black metal</a></li>
            <li style="font-size: calc(1rem * 53 / 100)"> <a href="{{ "/tags/symphonic-black-metal" | relative_url }}">symphonic black metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/symphonic-black-metal" | relative_url }}">symphonic black metal</a> # 67</li>
        </ul>
      </td>
    </tr>
  </tbody>
</table>

### Related genres

- [black metal]({{ "/tags/black-metal" | relative_url }})
- [progressive metal]({{ "/tags/progressive-metal" | relative_url }})
- [atmospheric black metal]({{ "/tags/atmospheric-black-metal" | relative_url }})
- [metal]({{ "/tags/metal" | relative_url }})
- [avant-garde black metal]({{ "/tags/avant-garde-black-metal" | relative_url }})
- [death metal]({{ "/tags/death-metal" | relative_url }})
- [avant-garde metal]({{ "/tags/avant-garde-metal" | relative_url }})
- [post-black metal]({{ "/tags/post-black-metal" | relative_url }})
- [melodic black metal]({{ "/tags/melodic-black-metal" | relative_url }})
- [progressive death metal]({{ "/tags/progressive-death-metal" | relative_url }})
