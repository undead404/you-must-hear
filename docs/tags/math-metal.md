---
category: genre
date: 2025-12-09T05:13:56.366Z
permalink: tags/math-metal
title: math metal
---

## math metal

Last updated: <time datetime="2025-12-09T05:13:56.366Z">Tue Dec 09 2025 07:13:56 GMT+0200 (Eastern European Standard Time)</time>

### Description

Math metal is a technically complex subgenre of heavy metal characterized by odd time signatures, polyrhythms, and intricate guitar work that creates a mechanically precise yet chaotic sound. Emerging in the 1990s from progressive and djent scenes, it appeals to musicians and fans who appreciate mathematical precision in extreme music. The genre blends aggressive distortion with calculated compositional structures that challenge conventional rhythm and melody.

### 100 chronological releases

<table>
  <thead>
    <tr>
      <th>Place</th>
      <th>Cover</th>
      <th>Date</th>
      <th>Artist</th>
      <th>Album</th>
      <th>Tags</th>
      <th>Other places</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>84</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/58d71fbf3bd8e0edd920981db285bd7f.png" alt="Cover for Burner" style="max-width:120px; height:auto;" />
      </td>
      <td>1994</td>
      <td>Breadwinner</td>
      <td>Burner</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-metal" | relative_url }}">alternative metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-garde-metal" | relative_url }}">avant-garde metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>85</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/446369eb9d7e4c9ccce3afda50b48e76.png" alt="Cover for The Burner" style="max-width:120px; height:auto;" />
      </td>
      <td>1994-03-14</td>
      <td>Breadwinner</td>
      <td>The Burner</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/alternative-metal" | relative_url }}">alternative metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-garde-metal" | relative_url }}">avant-garde metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>52</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/788419bea2e2f536a1466759b4edfde9.png" alt="Cover for None" style="max-width:120px; height:auto;" />
      </td>
      <td>1994-11-08</td>
      <td>Meshuggah</td>
      <td>None</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>34</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8c6e2c7461584284096d21580a702b87.png" alt="Cover for Destroy Erase Improve" style="max-width:120px; height:auto;" />
      </td>
      <td>1995-05-05</td>
      <td>Meshuggah</td>
      <td>Destroy Erase Improve</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 13</li>
            <li> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a> # 22</li>
            <li> <a href="{{ "/tags/extreme-metal" | relative_url }}">extreme metal</a> # 26</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>30</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f4c327102e66e9fbca5f399a9174030d.png" alt="Cover for Chaosphere" style="max-width:120px; height:auto;" />
      </td>
      <td>1998-11-09</td>
      <td>Meshuggah</td>
      <td>Chaosphere</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 8</li>
            <li> <a href="{{ "/tags/extreme-metal" | relative_url }}">extreme metal</a> # 10</li>
            <li> <a href="{{ "/tags/swedish-metal" | relative_url }}">swedish metal</a> # 25</li>
            <li> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a> # 100</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>100</td>
      <td>
      </td>
      <td>2000-06-13</td>
      <td>The Dillinger Escape Plan</td>
      <td>Dillinger Escape Plan</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>3</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d93e010af3e060fca76aeddaed41a82d.png" alt="Cover for Catch 33" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>Meshuggah</td>
      <td>Catch 33</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 82 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 7</li>
            <li> <a href="{{ "/tags/extreme-metal" | relative_url }}">extreme metal</a> # 31</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>45</td>
      <td>
          <img src="https://i.discogs.com/wHRue6nyVrJcNaXJ_iZewGd0Cr7X-cbigajI5i_42fQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTM4Njkx/NS0xMTc1MTU3OTYx/LmpwZWc.jpeg" alt="Cover for Nothing [Re-Issue]" style="max-width:120px; height:auto;" />
      </td>
      <td>2002</td>
      <td>Meshuggah</td>
      <td>Nothing [Re-Issue]</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>18</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/124ebef46e38cf0e436e6c94cd8bd3ce.png" alt="Cover for Nothing" style="max-width:120px; height:auto;" />
      </td>
      <td>2002-08-06</td>
      <td>Meshuggah</td>
      <td>Nothing</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 5</li>
            <li> <a href="{{ "/tags/technical-death-metal" | relative_url }}">technical death metal</a> # 77</li>
            <li> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a> # 82</li>
            <li> <a href="{{ "/tags/progressive-death-metal" | relative_url }}">progressive death metal</a> # 99</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>35</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/7910154cbf764be8b180ebfafc4ef402.png" alt="Cover for A Great Artist" style="max-width:120px; height:auto;" />
      </td>
      <td>2003-07-29</td>
      <td>A Life Once Lost</td>
      <td>A Great Artist</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/groove-metal" | relative_url }}">groove metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>97</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/f4773165f41f4f3e95f944f0c51436f5.png" alt="Cover for Nietoleruje-Bije" style="max-width:120px; height:auto;" />
      </td>
      <td>2004</td>
      <td>Moja Adrenalina</td>
      <td>Nietoleruje-Bije</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-garde-metal" | relative_url }}">avant-garde metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a> # 68</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>91</td>
      <td>
          <img src="https://i.discogs.com/-4THuR8Mz77m7ty2tJEs2CUBUL37kZMrJELWX3OsOZs/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE3OTY2/NzEtMTU2MjUxMTk2/Mi02NzIzLmpwZWc.jpeg" alt="Cover for ...Breathing Is Irrelevant" style="max-width:120px; height:auto;" />
      </td>
      <td>2004</td>
      <td>Ion Dissonance</td>
      <td>...Breathing Is Irrelevant</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/grindcore" | relative_url }}">grindcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>87</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/86e9d6291d74f099f358c68857c18e43.png" alt="Cover for Ink Compatible" style="max-width:120px; height:auto;" />
      </td>
      <td>2004-04-21</td>
      <td>Spastic Ink</td>
      <td>Ink Compatible</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 51 / 100)"> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 25</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>40</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4994a8c0ec620505e4af46cef7585d5b.png" alt="Cover for I" style="max-width:120px; height:auto;" />
      </td>
      <td>2004-07-13</td>
      <td>Meshuggah</td>
      <td>I</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 4</li>
            <li> <a href="{{ "/tags/swedish-metal" | relative_url }}">swedish metal</a> # 11</li>
            <li> <a href="{{ "/tags/extreme-metal" | relative_url }}">extreme metal</a> # 18</li>
            <li> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a> # 72</li>
            <li> <a href="{{ "/tags/avant-garde-metal" | relative_url }}">avant-garde metal</a> # 72</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>60</td>
      <td>
      </td>
      <td>2005</td>
      <td>Mudvayne</td>
      <td>Lost and Found (Clean Version)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>70</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e3a4ba67fbe54bec92ae34119c742182.png" alt="Cover for Unorthodox Creative Criteria" style="max-width:120px; height:auto;" />
      </td>
      <td>2005-07-29</td>
      <td>Coprofago</td>
      <td>Unorthodox Creative Criteria</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>80</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/73f538ae645846ced722afbae62d5541.png" alt="Cover for Centralia" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-01-06</td>
      <td>Car Bomb</td>
      <td>Centralia</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 30</li>
            <li> <a href="{{ "/tags/noisecore" | relative_url }}">noisecore</a> # 44</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>51</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/aef3d4a8db20ed7aae26e929e247e234.png" alt="Cover for Minus The Herd" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-06-05</td>
      <td>Ion Dissonance</td>
      <td>Minus The Herd</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 37</li>
            <li> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a> # 53</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>23</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/91ed0de7822063d776fdd4d46e4110d4.png" alt="Cover for Skullgrid" style="max-width:120px; height:auto;" />
      </td>
      <td>2007-10-16</td>
      <td>Behold... the Arctopus</td>
      <td>Skullgrid</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 14</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>66</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bafdfbc6d3c542dc9551e05cec690018.png" alt="Cover for Cannibalised" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-02-11</td>
      <td>Biomechanical</td>
      <td>Cannibalised</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/industrial" | relative_url }}">industrial</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/thrash-metal" | relative_url }}">thrash metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 50</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>2</td>
      <td>
          <img src="http://coverartarchive.org/release/4766f9f5-3d4f-360d-a0dd-85d482d0e3ca/27922575264-250.jpg" alt="Cover for ObZen" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-02-27</td>
      <td>Meshuggah</td>
      <td>ObZen</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 3</li>
            <li> <a href="{{ "/tags/extreme-metal" | relative_url }}">extreme metal</a> # 4</li>
            <li> <a href="{{ "/tags/technical-death-metal" | relative_url }}">technical death metal</a> # 7</li>
            <li> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a> # 10</li>
            <li> <a href="{{ "/tags/death-metal" | relative_url }}">death metal</a> # 22</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>78</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0d3599fbdaa34d50ad22fc1a769e5585.png" alt="Cover for Silhouettes" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-04-21</td>
      <td>Textures</td>
      <td>Silhouettes</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 34</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>67</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2bb7b896240c4faea4ae0996cbd73d11.png" alt="Cover for The Hidden Words" style="max-width:120px; height:auto;" />
      </td>
      <td>2008-06-30</td>
      <td>Fellsilent</td>
      <td>The Hidden Words</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 24</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>98</td>
      <td>
      </td>
      <td>2009-04-28</td>
      <td>Animals as Leaders</td>
      <td>Animals As Leaders</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 10</li>
            <li> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a> # 42</li>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>38</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/db781d568e7b4115a372f1917eab9dd3.png" alt="Cover for The Doppler Effect" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-12-06</td>
      <td>The Algorithm</td>
      <td>The Doppler Effect</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>48</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d78e0f0939e8f767063ef156ac930ba8.png" alt="Cover for 8" style="max-width:120px; height:auto;" />
      </td>
      <td>2009-12-08</td>
      <td>Uneven Structure</td>
      <td>8</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a> # 12</li>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 29</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>99</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/828840fa4e3f23953ea3083d06a58212.png" alt="Cover for Terminal Code" style="max-width:120px; height:auto;" />
      </td>
      <td>2010</td>
      <td>Cruentus</td>
      <td>Terminal Code</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>88</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/64ffb23402f79fb592a26ee64a28ad51.png" alt="Cover for Nonsense" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-04-05</td>
      <td>Liquorworks</td>
      <td>Nonsense</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>59</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/08d76faf629f4fc694eab01b06626979.png" alt="Cover for Periphery" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-04-16</td>
      <td>Periphery</td>
      <td>Periphery</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 26</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>93</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/58b7b6632d8f4318ad9ac1b4275a3bd1.png" alt="Cover for Ineave" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-05-09</td>
      <td>deely</td>
      <td>Ineave</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>47</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/2bc25b47132e4f5fb15403a7d6ff5e37.png" alt="Cover for CRITICAL.ERROR" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-07-08</td>
      <td>The Algorithm</td>
      <td>CRITICAL.ERROR</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>83</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ac5c3df876504de9b22bb133a1b607a4.png" alt="Cover for The Concept of Dreaming" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-11-16</td>
      <td>Volumes</td>
      <td>The Concept of Dreaming</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 50</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>77</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/eab1931cdd6242478d2c55594df3349c.png" alt="Cover for We Are The Foundation" style="max-width:120px; height:auto;" />
      </td>
      <td>2010-11-27</td>
      <td>Monuments</td>
      <td>We Are The Foundation</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 99</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>96</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/91f6f794c40d4bc2bf46eee08848dfca.png" alt="Cover for Stranded" style="max-width:120px; height:auto;" />
      </td>
      <td>2011</td>
      <td>Of Legends</td>
      <td>Stranded</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hardcore" | relative_url }}">hardcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metalcore" | relative_url }}">progressive metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>90</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/537bbe3f789a80bcc4ef90bac2fc238a.png" alt="Cover for Unframed" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-02-02</td>
      <td>deely</td>
      <td>Unframed</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>32</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/34ff00e2e01a3c613a1eacfa77c95374.png" alt="Cover for One" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-03-21</td>
      <td>TesseracT</td>
      <td>One</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 15</li>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 16</li>
            <li> <a href="{{ "/tags/spiritual" | relative_url }}">spiritual</a> # 22</li>
            <li> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a> # 75</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>41</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/15713948d171475c9fe482e90e6d8385.png" alt="Cover for II" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-04-25</td>
      <td>Xerath</td>
      <td>II</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/groove-metal" | relative_url }}">groove metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/symphonic-metal" | relative_url }}">symphonic metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>26</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3caef46fbc0d4fd786814d37bdd91f9d.png" alt="Cover for Universal Language" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-05-24</td>
      <td>Angel Vivaldi</td>
      <td>Universal Language</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>79</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/08940969ef7745779d8c0288e4359101.png" alt="Cover for Carnival Is Forever" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-07-12</td>
      <td>Decapitated</td>
      <td>Carnival Is Forever</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/technical-death-metal" | relative_url }}">technical death metal</a></li>
            <li style="font-size: calc(1rem * 88 / 100)"> <a href="{{ "/tags/death-metal" | relative_url }}">death metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-death-metal" | relative_url }}">technical death metal</a> # 30</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>54</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/84e3218eca1d48eab9dba7ef69da4b1d.png" alt="Cover for Red Seas Fire" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-09-04</td>
      <td>Red Seas Fire</td>
      <td>Red Seas Fire</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>63</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0deff0cd9fcf4bdab56e1b8110864bfd.png" alt="Cover for Dualism" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-09-23</td>
      <td>Textures</td>
      <td>Dualism</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 53</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>64</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/71d81790b1066cc25071788c76a095d9.png" alt="Cover for Cassini" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-09-26</td>
      <td>Sithu Aye</td>
      <td>Cassini</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 74</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>86</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8c42459d74f84c1ba4e4a6e971baeae8.png" alt="Cover for Via" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-09-27</td>
      <td>Volumes</td>
      <td>Via</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/bounce" | relative_url }}">bounce</a> # 15</li>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 91</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>92</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/38f4863af7924c7b8ea49317e60c621c.png" alt="Cover for Weightless" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-11-04</td>
      <td>Animals as Leaders</td>
      <td>Weightless</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/instrumental-metal" | relative_url }}">instrumental metal</a> # 7</li>
            <li> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a> # 21</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>81</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0db5b740bd8942dfbe83b8eb4b29c9e6.png" alt="Cover for Ever Forthright" style="max-width:120px; height:auto;" />
      </td>
      <td>2011-12-13</td>
      <td>Ever Forthright</td>
      <td>Ever Forthright</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>89</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8859eb8a6cba4807b02a17b025c1aa7e.png" alt="Cover for Case of Noise" style="max-width:120px; height:auto;" />
      </td>
      <td>2012</td>
      <td>Materia</td>
      <td>Case of Noise</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 69</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>8</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c1b40db03340400eb85dffea9f006ef3.png" alt="Cover for Koloss" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-02-09</td>
      <td>Meshuggah</td>
      <td>Koloss</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 1</li>
            <li> <a href="{{ "/tags/extreme-metal" | relative_url }}">extreme metal</a> # 3</li>
            <li> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a> # 11</li>
            <li> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a> # 49</li>
            <li> <a href="{{ "/tags/avant-garde-metal" | relative_url }}">avant-garde metal</a> # 53</li>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 82</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>36</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/66208febefd947e5aa712832d0408c46.png" alt="Cover for Acid Mist Tomorrow" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-03-23</td>
      <td>Hypno5e</td>
      <td>Acid Mist Tomorrow</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>33</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/1788c5274c6b4d27af39d2e13308cc04.png" alt="Cover for Колесницы Богов" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-03-30</td>
      <td>The Korea</td>
      <td>Колесницы Богов</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>62</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/96efd350b584402d9783c1ae83b23cfa.png" alt="Cover for Grind the Ocean" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-04-09</td>
      <td>The Safety Fire</td>
      <td>Grind the Ocean</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>56</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3b0f4fe4a24b44b3ab9f19f5cb1e0c1e.png" alt="Cover for Perspective" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-05-18</td>
      <td>TesseracT</td>
      <td>Perspective</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 64 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>49</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/aebc8c7741d1481e8083f59e668d6361.png" alt="Cover for Gnosis" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-08-27</td>
      <td>Monuments</td>
      <td>Gnosis</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 46</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>75</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/9bbb49ebc7d446db866ce9b2ec8393ad.png" alt="Cover for Moments" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-08-27</td>
      <td>Stealing Axion</td>
      <td>Moments</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 58</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>76</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/08550ff2f35849bf85ddb58e82429060.png" alt="Cover for Invent the Universe" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-10-15</td>
      <td>Sithu Aye</td>
      <td>Invent the Universe</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 92</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>68</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/253e1f46ebe747bfc7bfe63793c33d2b.png" alt="Cover for Hacktivist" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-11-12</td>
      <td>Hacktivist</td>
      <td>Hacktivist</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 25</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>11</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/93945a2ea957424cc236f6c345293ad5.png" alt="Cover for Polymorphic Code" style="max-width:120px; height:auto;" />
      </td>
      <td>2012-11-19</td>
      <td>The Algorithm</td>
      <td>Polymorphic Code</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>31</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/8b2abd568bc6461ec6f558a7a16df255.png" alt="Cover for Inspire" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-04-21</td>
      <td>Polyphia</td>
      <td>Inspire</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 83</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>82</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/41088a68f96e4fa891d133d08b9d1609.png" alt="Cover for Exposition" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-05-15</td>
      <td>Red Seas Fire</td>
      <td>Exposition</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metalcore" | relative_url }}">progressive metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 66</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>58</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/431df5550e3895e7a8a159495c265134.png" alt="Cover for Mouth of Swords" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-09-02</td>
      <td>The Safety Fire</td>
      <td>Mouth of Swords</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>74</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/dded8a31188c4820c877bfb9db0fb182.png" alt="Cover for Travelers" style="max-width:120px; height:auto;" />
      </td>
      <td>2013-12-15</td>
      <td>Modern Day Babylon</td>
      <td>Travelers</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 86</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>94</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/e608b2599e404fcecf55993e5d5bc9ef.png" alt="Cover for The Amanuensis" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-06-20</td>
      <td>Monuments</td>
      <td>The Amanuensis</td>
      <td>
        <ul>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 23</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>27</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/18c5516243734b65cb67eab3c7ccf9da.png" alt="Cover for False Idols" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-08-01</td>
      <td>Hacktivist</td>
      <td>False Idols</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rapcore" | relative_url }}">rapcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a> # 13</li>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 76</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>43</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/eca4b2de12a742f4c2adc09f9d80d4ee.png" alt="Cover for Lion" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-09-02</td>
      <td>Anup Sastry</td>
      <td>Lion</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>15</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/af3ec986cbcb4f97cfe4825eb2ca9581.png" alt="Cover for Deceive and Defy" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-11-07</td>
      <td>Hacktivist</td>
      <td>Deceive and Defy</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rapcore" | relative_url }}">rapcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a> # 5</li>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 39</li>
            <li> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a> # 76</li>
            <li> <a href="{{ "/tags/rapcore" | relative_url }}">rapcore</a> # 84</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>73</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/858a7a27bcc5444fc4eab804bae9ef36.png" alt="Cover for Aeons" style="max-width:120px; height:auto;" />
      </td>
      <td>2014-11-11</td>
      <td>Stealing Axion</td>
      <td>Aeons</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/technical-metal" | relative_url }}">technical metal</a> # 55</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>95</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/774ea309060341eac17166c9dc3e9a54.png" alt="Cover for Juggernaut: Alpha" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-01-23</td>
      <td>Periphery</td>
      <td>Juggernaut: Alpha</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>20</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/af1a24273c0eefc00d79f137e5ca21da.png" alt="Cover for Kezia (Remaster)" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-11-02</td>
      <td>Protest the Hero</td>
      <td>Kezia (Remaster)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>22</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/d1b439f8aabfe61b1636ccea5e3a04ef.png" alt="Cover for Polysemy" style="max-width:120px; height:auto;" />
      </td>
      <td>2015-11-06</td>
      <td>Mestís</td>
      <td>Polysemy</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fusion" | relative_url }}">fusion</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/jazz" | relative_url }}">jazz</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/jazz-fusion" | relative_url }}">jazz fusion</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 64</li>
            <li> <a href="{{ "/tags/fusion" | relative_url }}">fusion</a> # 89</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>42</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/405b63cc87c229f7b48a9b463b03dc8a.png" alt="Cover for Brute Force" style="max-width:120px; height:auto;" />
      </td>
      <td>2016</td>
      <td>The Algorithm</td>
      <td>Brute Force</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 27</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>13</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0128260661f395facc0b2380e852e829.png" alt="Cover for Phenotype" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-02-05</td>
      <td>Textures</td>
      <td>Phenotype</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>46</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/031c2d79bde0af97ee47a21a2d850a52.png" alt="Cover for Bloom" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-03-02</td>
      <td>Anup Sastry</td>
      <td>Bloom</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>61</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/113644af77f06a44b228722966819b61.png" alt="Cover for The Infinite Nothing" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-04-01</td>
      <td>Entheos</td>
      <td>The Infinite Nothing</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/deathcore" | relative_url }}">deathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-deathcore" | relative_url }}">progressive deathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/technical-death-metal" | relative_url }}">technical death metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/progressive-deathcore" | relative_url }}">progressive deathcore</a> # 68</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>25</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c4123b93ecd8149219c2651b5bd44c67.png" alt="Cover for Set Course for Andromeda" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-05-04</td>
      <td>Sithu Aye</td>
      <td>Set Course for Andromeda</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 72</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>28</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/be943e0f767a80f488e4c9de7010cdaf.png" alt="Cover for Polaris &#x2F; Errai" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-09-16</td>
      <td>TesseracT</td>
      <td>Polaris &#x2F; Errai</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/ambient" | relative_url }}">ambient</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 77</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>50</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3bd80e425953beca54a25a051b16558f.png" alt="Cover for Cassini (5th Anniversary Remaster)" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-09-26</td>
      <td>Sithu Aye</td>
      <td>Cassini (5th Anniversary Remaster)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>6</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/3c1b0891d727f3c9070ba22c0bc40398.png" alt="Cover for DOOM (Original Game Soundtrack)" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-09-29</td>
      <td>Mick Gordon</td>
      <td>DOOM (Original Game Soundtrack)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/soundtrack" | relative_url }}">soundtrack</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 13</li>
            <li> <a href="{{ "/tags/industrial-metal" | relative_url }}">industrial metal</a> # 61</li>
            <li> <a href="{{ "/tags/soundtrack" | relative_url }}">soundtrack</a> # 77</li>
            <li> <a href="{{ "/tags/industrial" | relative_url }}">industrial</a> # 95</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>21</td>
      <td>
          <img src="https://i.discogs.com/pHPefXPAraUIbmS2wWcFmC7A2izqfJU02u7y3MEHPG0/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTkxMjYx/NDUtMTQ3NTIyNTI0/My0xOTM5LmpwZWc.jpeg" alt="Cover for The Violent Sleep Of Reason" style="max-width:120px; height:auto;" />
      </td>
      <td>2016-10-07</td>
      <td>Meshuggah</td>
      <td>The Violent Sleep Of Reason</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 56.00000000000001 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/extreme-metal" | relative_url }}">extreme metal</a> # 11</li>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 26</li>
            <li> <a href="{{ "/tags/avant-garde-metal" | relative_url }}">avant-garde metal</a> # 39</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>55</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/096beb4e64a81d43320b60ba15358c7e.png" alt="Cover for Blind World" style="max-width:120px; height:auto;" />
      </td>
      <td>2017</td>
      <td>Ghost Iris</td>
      <td>Blind World</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metal" | relative_url }}">metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metalcore" | relative_url }}">progressive metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>37</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/57b21290c4d236c68b155aa2229161ca.png" alt="Cover for Ascendancy" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-02-17</td>
      <td>Kadinja</td>
      <td>Ascendancy</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metalcore" | relative_url }}">progressive metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/progressive-metalcore" | relative_url }}">progressive metalcore</a> # 73</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>72</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c8b1e265381fbf53671724b86922f4d9.png" alt="Cover for Look At Yourself" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-03-03</td>
      <td>Emmure</td>
      <td>Look At Yourself</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/nu-metal" | relative_url }}">nu metal</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/moshcore" | relative_url }}">moshcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/moshcore" | relative_url }}">moshcore</a> # 4</li>
            <li> <a href="{{ "/tags/nu-metalcore" | relative_url }}">nu metalcore</a> # 10</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>69</td>
      <td>
          <img src="https://i.discogs.com/Av2Ay84y5QJYlvIo9EDPUGdjdNfKcVYEcQJRViM74uU/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEwMTYx/MDUwLTE0OTI2OTE0/NjEtMjc4Ny5qcGVn.jpeg" alt="Cover for Synchromysticism" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-04-20</td>
      <td>Yowie</td>
      <td>Synchromysticism</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/avant-garde-metal" | relative_url }}">avant-garde metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/instrumental" | relative_url }}">instrumental</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/noise-rock" | relative_url }}">noise rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>39</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/bf24a55fa93812631373c9c52b4caa9c.png" alt="Cover for Senpai EP II: The Noticing" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-05-08</td>
      <td>Sithu Aye</td>
      <td>Senpai EP II: The Noticing</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>14</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/72dc7968383ede2a61a64d06df6f25e5.png" alt="Cover for 2 Rotten" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-05-31</td>
      <td>Hacktivist</td>
      <td>2 Rotten</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/hip-hop" | relative_url }}">hip-hop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rap" | relative_url }}">rap</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/rapcore" | relative_url }}">rapcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a> # 4</li>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 37</li>
            <li> <a href="{{ "/tags/mathcore" | relative_url }}">mathcore</a> # 75</li>
            <li> <a href="{{ "/tags/rapcore" | relative_url }}">rapcore</a> # 82</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>19</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/0a42f674713433136bf3ab89ea113b32.png" alt="Cover for Smile" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-06-23</td>
      <td>TesseracT</td>
      <td>Smile</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>7</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/4e8679d9891a621b80ce2dfaed9c2ca6.png" alt="Cover for The Way Forward" style="max-width:120px; height:auto;" />
      </td>
      <td>2017-12-01</td>
      <td>Intervals</td>
      <td>The Way Forward</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 19</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>16</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/40248e94a476fdf84d167beb1c95dc8f.png" alt="Cover for Luminary" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-02-08</td>
      <td>TesseracT</td>
      <td>Luminary</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 42</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>17</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/c6a9fc00940798b1236112ead4f9615e.png" alt="Cover for King" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-02-16</td>
      <td>TesseracT</td>
      <td>King</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 43</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>4</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/cdf65ca7a448d831a6710f91637e7e0d.png" alt="Cover for Sunhead" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-07-27</td>
      <td>Plini</td>
      <td>Sunhead</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/fusion" | relative_url }}">fusion</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/jazz" | relative_url }}">jazz</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/jazz-fusion" | relative_url }}">jazz fusion</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/fusion" | relative_url }}">fusion</a> # 27</li>
            <li> <a href="{{ "/tags/jazz-fusion" | relative_url }}">jazz fusion</a> # 47</li>
            <li> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a> # 67</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>1</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/efd6544886af88d4241bc934f6c78afe.png" alt="Cover for Ego Death (feat. Steve Vai)" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-10-12</td>
      <td>Polyphia</td>
      <td>Ego Death (feat. Steve Vai)</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/pop" | relative_url }}">pop</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-pop" | relative_url }}">progressive pop</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 1</li>
            <li> <a href="{{ "/tags/progressive-pop" | relative_url }}">progressive pop</a> # 7</li>
            <li> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a> # 10</li>
            <li> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a> # 26</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>24</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/009ad4a6097bcf31ce39b26d5d6377f4.png" alt="Cover for Compiler Optimization Techniques" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-11-02</td>
      <td>The Algorithm</td>
      <td>Compiler Optimization Techniques</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/8-bit" | relative_url }}">8-bit</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/electronic" | relative_url }}">electronic</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/psytrance" | relative_url }}">psytrance</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/synthwave" | relative_url }}">synthwave</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a> # 10</li>
            <li> <a href="{{ "/tags/8-bit" | relative_url }}">8-bit</a> # 25</li>
            <li> <a href="{{ "/tags/psytrance" | relative_url }}">psytrance</a> # 59</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>57</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5b18b3d4aa442a3e17ec5c55036e6c9a.png" alt="Cover for Homebound" style="max-width:120px; height:auto;" />
      </td>
      <td>2018-12-17</td>
      <td>Sithu Aye</td>
      <td>Homebound</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>9</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/354dedfbb0db2a03fa95c14ae8c0eef5.png" alt="Cover for Macro" style="max-width:120px; height:auto;" />
      </td>
      <td>2019</td>
      <td>Jinjer</td>
      <td>Macro</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/nu-metalcore" | relative_url }}">nu metalcore</a> # 17</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>53</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/451ac43ecc2bdf78d499d8f9a88028f1.png" alt="Cover for Super 90&#39;" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-01-18</td>
      <td>Kadinja</td>
      <td>Super 90&#39;</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metalcore" | relative_url }}">progressive metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>65</td>
      <td>
          <img src="https://i.discogs.com/m4bviPWWMt3hpzzh0NeLyjk5PyeTh_sbOwBUIK3DQq4/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTEzNTk1/ODYxLTE1NTcxODAz/OTYtOTIyMC5qcGVn.jpeg" alt="Cover for Familiaris" style="max-width:120px; height:auto;" />
      </td>
      <td>2019-05-03</td>
      <td>Snooze</td>
      <td>Familiaris</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>29</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/ed39b1832fe93443188e8d797762b8be.png" alt="Cover for Fabula &amp; Syuzhet" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-08-21</td>
      <td>Protest the Hero</td>
      <td>Fabula &amp; Syuzhet</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>12</td>
      <td>
          <img src="https://i.discogs.com/6p425JUH0MpUSHuMYVi9t2EeNXiweFDD3s4qaqz-fiM/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE2MjI4/MDg1LTE2MTM5MjQz/OTEtMzU0Mi5qcGVn.jpeg" alt="Cover for Circadian" style="max-width:120px; height:auto;" />
      </td>
      <td>2020-11-13</td>
      <td>Intervals</td>
      <td>Circadian</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 28</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>71</td>
      <td>
          <img src="https://i.discogs.com/FpGyK7ELflHhG3epbBpt1V-QD5NFw6-Q_tkcIZIvplQ/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTE3MTc4/NjQwLTE2MTIwMTg3/MTYtNzkyMS5qcGVn.jpeg" alt="Cover for Senpai III" style="max-width:120px; height:auto;" />
      </td>
      <td>2021-01-08</td>
      <td>Sithu Aye</td>
      <td>Senpai III</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-rock" | relative_url }}">math rock</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-rock" | relative_url }}">progressive rock</a></li>
        </ul>
      </td>
      <td>
        <ul>
        </ul>
      </td>
    </tr>
    <tr>
      <td>5</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/350f291398357b3468102492b2914422.png" alt="Cover for Immutable" style="max-width:120px; height:auto;" />
      </td>
      <td>2022-04-01</td>
      <td>Meshuggah</td>
      <td>Immutable</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/djent" | relative_url }}">djent</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/progressive-metal" | relative_url }}">progressive metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/djent" | relative_url }}">djent</a> # 11</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>44</td>
      <td>
          <img src="https://i.discogs.com/ZqCYSd26y2tJeom6UKEUK7Nnm9QDscjw44N6CRITpgo/rs:fit/g:sm/q:40/h:150/w:150/czM6Ly9kaXNjb2dz/LWRhdGFiYXNlLWlt/YWdlcy9SLTIzMzQ5/MDExLTE2NTc0MjMz/OTYtMjExNS5qcGVn.jpeg" alt="Cover for Data Renaissance" style="max-width:120px; height:auto;" />
      </td>
      <td>2023-05-19</td>
      <td>The Algorithm</td>
      <td>Data Renaissance</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/experimental-metal" | relative_url }}">experimental metal</a> # 23</li>
        </ul>
      </td>
    </tr>
    <tr>
      <td>10</td>
      <td>
          <img src="https://lastfm.freetls.fastly.net/i/u/34s/5cdb99a63e4498063511f694ba34a2f3.png" alt="Cover for Half Living Things" style="max-width:120px; height:auto;" />
      </td>
      <td>2024-04-05</td>
      <td>Alpha Wolf</td>
      <td>Half Living Things</td>
      <td>
        <ul>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/math-metal" | relative_url }}">math metal</a></li>
            <li style="font-size: calc(1rem * 100 / 100)"> <a href="{{ "/tags/metalcore" | relative_url }}">metalcore</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/nu-metal" | relative_url }}">nu metal</a></li>
            <li style="font-size: calc(1rem * 66 / 100)"> <a href="{{ "/tags/nu-metalcore" | relative_url }}">nu metalcore</a></li>
        </ul>
      </td>
      <td>
        <ul>
            <li> <a href="{{ "/tags/nu-metalcore" | relative_url }}">nu metalcore</a> # 12</li>
        </ul>
      </td>
    </tr>
  </tbody>
</table>

### Related genres

- [progressive metal]({{ "/tags/progressive-metal" | relative_url }})
- [djent]({{ "/tags/djent" | relative_url }})
- [mathcore]({{ "/tags/mathcore" | relative_url }})
- [metal]({{ "/tags/metal" | relative_url }})
- [metalcore]({{ "/tags/metalcore" | relative_url }})
- [progressive rock]({{ "/tags/progressive-rock" | relative_url }})
- [death metal]({{ "/tags/death-metal" | relative_url }})
- [technical death metal]({{ "/tags/technical-death-metal" | relative_url }})
- [deathcore]({{ "/tags/deathcore" | relative_url }})
- [technical metal]({{ "/tags/technical-metal" | relative_url }})
